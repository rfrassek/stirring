...
\begin{remark}[Equilibrium case]
	...
	\end{equation}
i.e. the product over sites of multinomial distribution with parameters $\nu=1$ and $p_{a}=\beta_{a}$. This is a direct consequence of the fact that $\beta_{N}=1-(\beta_{1}+\ldots+\beta_{N-1})$. 
\end{remark}
...
\begin{remark}
	...
	\end{equation}
where the expectation is take with respect to the $\mu_{\text{NESS}}$ of the process $\bm{\sigma}=(\sigma_{1},\ldots,\sigma_{L})$. 
...
\begin{comment}
	...
	\begin{comment}
		...
		\begin{equation}
			...
			\end{align}
		In the last equality we have used equation \eqref{ABS_1}. 
			%	\begin{equation}
				%		\begin{split}
			...
			A formula for $|\Psi\rangle$ can be also obtain using the conservation of probability in the non-equilibrium steady state. Consider a generic $\bm{\tau}\in \Omega$, then we have 
				\begin{align}
					...
					=&\sum_{\bm{\sigma}\in \Omega}\prod_{x=1\,:\,\tau_{x}\neq N}^{L}\mathbbm{1}_{\{\sigma_{x}=\tau_{x}\}}\prod_{x=1\,:\,\tau_{x}=N}^{L}\mathbbm{1}_{\{\sigma_{x}=N\}}\bm{\sigma}|\Psi\rangle\nonumber\\
