...
As already pointed out in Subsection \ref{statementDualitySubsection}, the dual process voids the graph by piling up particles of species $\{1,\ldots,N-1\}$ in the extra sites and replacing them by types $N$. This property allows to characterize the steady state non-equilibrium distribution (assuming that it is unique) by the probability that dual particles are absorbed in the extra-sites, then called \textit{absorption probabilities}. Here we consider a chain of length $L$ where the end sites $1$ and $L$ are connected with two reservoirs. Each site can host $\nu\geq 1$ particles, chosen among the $N$ types. The process $\left(\bm{n}(t)\right)_{t\geq0}$ is defined on the space $\Omega=\bigtimes_{x=1}^{L} \Omega_{x}$ and the generator reads
\begin{equation}
...
\end{equation}
We specialize the results of Subsection \ref{statementDualitySubsection} to this chain. The process is dual with respect to $(\bm{\xi}(t))_{t\geq0} $ defined on the enlarged configuration space
\begin{equation}	
...
\end{equation}
Therefore, up to a constant, $\mathbb{E}_{\mu^{\text{stat}}}\left[D(\bm{n},\bm{\hat{\xi}})\right]$ gives the non-equilibrium stationary state $m-$points correlations among non-empty species of types $s_{k}\in\{1,\ldots,N-1\}$ situated at sites $y_{k}\in\{1,\ldots,L\}$ with $k\in \{1,\ldots,m\}$. This expectation can be computed by \eqref{ExptationSS} once the absorption probabilities are known. The issue is that these absorption probabilities fulfil some difference equations that are not easy to solve. For example, we report here the solution for the simplest case of one point correlations, since the single dual particle behaves as an independent random walker. 
...
\end{equation}
 namely the joint probability of having, for all $k=1,\ldots,m$, $c_{k}$ particles of species $s_{k}$ at site $y_{k}$ are absorbed at left and $1-c_{k}$ absorbed at right. Therefore, by computing these absorption probabilities we determine the arbitrary non-equilibrium steady state correlations of the integrable multi-species stirring process. 
...
\end{equation}
This Markov chain is irreducible and positive recurrent, therefore there exists a unique stationary measure, that will be reached when time goes to infinity, regardless of the initial distribution. 
...
The MPA gives an abstract form of the steady state in terms of an algebra of operators. However, to write explicitly the steady state one should provide a representation of this algebra. Another approach is to directly use the commutators and and the boundary actions compute correlations. This path has been followed in \cite{vanicat2017exact} to compute one-point and two-point correlations. 
...
\begin{align*}
    	...
    	\\&+\left.
    	\ldots+	\sum_{q_{1},\ldots,q_{L}=1\,:\,q_{i}\neq q_{j}\,\text{if}\,i\neq j}^{L}\left(\prod_{t=1}^{L}(1-\delta_{s_{q_{t}},N})\right)e_{\tau_{1},\ldots,\tau_{L+1}}^{\varphi_{1}^{L},\ldots,\varphi_{L+1}^{L}}
    	\right. \\&+ \left. 
    	\sum_{q_{1},\ldots,q_{L+1}=1\,:\,q_{i}\neq q_{j}\,\text{if}\,i\neq j}^{L}\left(\prod_{x=1}^{L+1}(1-\delta_{\tau_{x},N})\right)\beta_{\tau_{x}}e_{\tau_{1},\ldots,\tau_{L+1}}^{\varphi_{1}^{L+1},\ldots,\varphi_{L+1}^{L+1}} \right\}
    	\\&=
...
\paragraph{Proof of formula \eqref{exactCorrelations}} Consider $m$ particles of non-empty types $s_{i}\in\{1,\ldots,N-1\}$ in positions $y_{i}\in\{1,\ldots,L\}$ for all $i=1,\ldots,m$. We aim to compute the non-equilibrium stationary state correlation $\langle n_{s_{1}}^{y_{1}}\cdots n_{s_{m}}^{y_{m}} \rangle_{stat}$. By using duality, we showed in \eqref{CorrelationByABS-prob} that
 \begin{equation}
...
The two points correlations with respect the non equilibrium distribution of the occupation variable of the species $s_{1},s_{2}\in \{1,\ldots,N-1\}$ at coordinates $x_{1},x_{2}\in \{1,\ldots,L\}$ with $x_{1}< x_{2}$ reads 
\begin{equation}
	\begin{split}
...
The three points correlations with respect the non equilibrium distribution of the occupation variable of the species $s_{1},s_{2},s_{3}\in \{1,\ldots,N-1\}$ at coordinates $y_{1},y_{2},y_{3}\in \{1,\ldots,L\}$ such that $y_{1}<y_{2}<y_{3}$
\begin{equation}
...
\end{align}
\subsection{Charges}
Using the matrix form of $U_{0}(x)$ written in \eqref{matrixForm-U}, the transfer matrix $T(x)$ reads
\begin{equation}
	\begin{split}
...
\end{equation}
\end{comment}
Arguing similarly we write the elements of $U_{0}(x)$  as polynomials in $x$ 
\begin{equation}
...
\end{equation}
We will need the charge expansion of $T(x)$. As it will be clear in the following, only the first three coefficients for the diagonal elements of $U_{0}(x)$ and only the first one for the non-diagonal elements of $U_{0}(x)$ will be needed. Therefore, using \eqref{RURU}, we compute: 
\begin{itemize}
	\item For $D_{N,N}(x)$: 
\begin{equation}
	D_{N,N}^{2L+1}=I^{tot}
...
\end{equation}
\end{itemize}
\textbf{Remark}: when restricted to the case of 2 species, we obtain equations (2.24)-(2.30) of \cite{frassek2020eigenstates}. \\ \\
...
\end{align}
\begin{align}
Q^{2L+1}&=ND_{N,N}^{2L+1}+A^{2L}-\sum_{i=1}^{N-1}D_{ii}^{2L}=N+2L-1
\end{align}
\begin{align}
...
\end{equation}
where, for the sake of notation we denoted $\Delta_{i}=(\alpha_{i}-\beta_{i})$. Therefore we can write 
\begin{equation}
Q^{2L}=Q^{2L}_{0}+Q_{+}^{2L}
\end{equation}
...
\end{equation} }
\end{comment}
\paragraph{Exchange relations between charges} Since, in general, the coefficients of the polynomial form of the elements of $U_{0}(x)$ and the charges do not commute, it is useful to find explicit exchange relations among them. Using \eqref{EX1}, \eqref{EX2}, \eqref{EX3}, \eqref{EX4},\eqref{EX5} and \eqref{EX6} we obtain:
\begin{itemize}
	\item 
...
\begin{equation}
	[H^{''},T(x)]=0
\end{equation}
Since $T(x)=\sum_{a=0}^{2L}Q^{2L}x^{a}$, the first non-diagonal charge $Q_{2L}$ commutes with $H^{''}$ and thus we can equivalently look for a matrix $W$ such that 
\begin{equation}
	Q^{2L}W=WQ_{0}^{2L}
\end{equation}
...
In particular, by taking as equilibrium eigenvector in \eqref{Nice-W2} the vacuum state 
\begin{equation}
...
\end{equation}
Power expanding the matrix $W$ with respect to $\Delta_{1}=(\alpha_{1}-\beta_{1})$ we obtain 
...
\begin{equation}
	\lim_{\epsilon\to 0}\epsilon\left((\Lambda_{m}+\epsilon)I-Q_{0}^{2L}\right)^{-1}|\Psi_{m}\rangle=\text{diag}\left\{0,\ldots,0,1,0,\ldots,0\right\}|\Psi_{m}\rangle
\end{equation}
...
In this case we do not have a closed formula for all non-equilibrium eigenvectors as in the case of $gl(2)$. Indeed, the exchange relation \eqref{ExQ0B-charges} has an extra creation operator
\begin{equation}
...
Performing a multi-variable power expansion of formula \eqref{WInCharges} with respect to $\Delta_{i} B_{i}^{2L-1}$ for all $i\in\{1,\ldots,N-1\}$  we obtain 
\begin{equation}
	\begin{split}
		W=&\lim_{\epsilon\to 0}\epsilon\sum_{\bm{m}\,:\,|\bm{m}|=L}\sum_{k_{1}=0}^{L}\ldots\sum_{k_{N-1}=0}^{L}\mathbbm{1}_{\{k_{1}+\cdots+k_{N-1}\leq L\}}2^{k_{1}+\ldots+k_{N-1}}\Delta_{1}^{k_{1}}\cdots\Delta_{N-1}^{k_{N-1}}\\&SYM_{k_{1},\ldots,k_{N-1}}\left(\left\{\left(\Lambda_{\bm{m}}^{Q}+\epsilon-Q_{0}^{2L}\right)^{-1}B_{1}^{2L-1}\right\},\ldots,\left\{\left(\Lambda_{\bm{m}}+\epsilon-Q_{0}^{2L}\right)^{-1}B_{N-1}^{2L-1}\right\}\right)
		\\&
		\left(\Lambda_{\bm{m}}+\epsilon-Q_{0}^{2L}\right)^{-1}|\Psi_{\bm{m}}^{0}\rangle \langle \Psi_{\bm{m}}^{0}|
	\end{split}
\end{equation} 
Observing that 
\begin{equation}
...
Using \eqref{W-charges-Bad}, it is possible to find an explicit expression of the steady state $|\Psi^{''}\rangle$ of $H^{''}$ when $N>2$ by acting with $W$ on the vacuum state of the Hamiltonian $H_{0}$, i.e. 
...
The aim of this section is studying some extension of the analysis previously done when integrability property is lost. We fix $\nu\geq 1$ and, for the sake of simplicity, we keep the geometry of a chain. We provide the following extensions: 
\begin{enumerate}
	...
	\item \textit{Reaction-diffusion process}: a further dynamics is added. Particle can now change type. The model is carefully constructed with the goal of having the average density evolving as a system of $N-1$ coupled difference-differential equation, providing a generalization of the model studied in \cite{casini2022uphill}. Duality for the boundary driven process is proved. Subsection \ref{Subsection-RD}
\end{enumerate}
...
\begin{equation}
	...
	\end{split}
\end{equation}
In the thermalized bulk generator particles on a bond $x$ and $x+1$ are redistribute according to the measure $\nu_{TH}(\bm{r}|\bm{\epsilon})$, while in the thermalized boundary generators the configuration at site $1$ (and $L$) $n_{1}$ is replaced by $\bm{r}$, according to a rate depending on the reservoirs parameters. 
