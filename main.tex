...
\begin{equation}
	WH_{0}=H^{''}W
\end{equation}
Formula \eqref{W-extended} may be evaluated by perturbation theory (see \cite{frassek2020duality}). Moreover, since the process is integrable it is possible to express $W$ in function of the conserved charges of the system. This expression allows a simpler treatment of the mapping and allows also to retrieve the steady state of $H^{''}$ by applying $W$ to the ground state of $H_{0}$. To write formula \eqref{W-extended} through the conserved charges of the system, we use Quantum Inverse Scattering Method (QISM) (see \ref{SubsectionQISM}). In Subsection \ref{SubsectionGL2} we first construct $W$ for the simpler case of $gl(2)$, then, in Subsection \ref{SubsectionGLN} we adapt the proof to the general $gl(N)$. 
...
\begin{equation}\label{matrixForm-U}
	...
	\end{pmatrix}
\end{equation}
where each of the above element acts on the quantum space $V$. These element satisfy some \textit{exchange relations} that can be found by \eqref{RURU} and they are given by: 
\begin{itemize}
...
\begin{equation}
	...
	\end{split}
\end{equation}
where, for all $k\in\{0,\ldots,2L+2\}$, we call the matrices $Q_{k}$ the \textit{conserved charges} of the system. We also introduce 
\begin{equation}
	T_{0}(x)=(N+x)D_{N,N}(x)-\sum_{i=1}^{N-1}xD_{ii}(x)\quad\text{such that}\quad T(x)=T_{0}(x)+\sum_{i=2}^{N}(2x+N)(\alpha_{i}-\beta_{i})B_{i}(x)
\end{equation}
Arguing similarly we write the elements of $U_{0}(x)$  as polynomials in $x$ as 
\begin{equation}
...
\end{equation}
 In the following we will need the charge expansion of $T(x)$. As it will be clear in the following, only the first three coefficients for the diagonal elements of $U_{0}(x)$ and only the first one for the non-diagonal elements of $U_{0}(x)$ will be needed. Therefore, using \eqref{RURU}, we compute: 
\begin{itemize}
	\item For $D_{N,N}(x)$: 
\begin{equation}
	D_{N,N}^{2L+1}=I^{tot}
...
\end{align}
The charges $Q_{2L+2}$ and $Q_{2L+1}$ are proportional to the identity matrix, while $Q_{2L}$ contains non diagonal matrices and the boundary parameters $\alpha_{i},\beta_{i}$ $\forall i=1,\ldots,N-1$.\\
...
Equation \eqref{Nice-W2} gives an explicit formula in term of the matrices $B_{1}^{2L-1}$ of the matrix $W$. We stress the fact that these closed formula for every non-equilibrium eigenvectors comes from the simple commutation relations between charges. We also notice that \eqref{Nice-W2} was found with other arguments also in \cite{frassek2020duality} and \cite{frassek2020eigenstates}, here we present a different technique to obtain it. 
...
Following the result in the simpler $gl(2)$ case, we write the multi-variable power expansion with respect to $\Delta_{i} B_{i}^{2L-1}$ for all $i\in\{1,\ldots,N-1\}$. Using also the multi-variate version of \eqref{veryUsefulSeries}, we obtain
\begin{equation}
	\begin{split}
