...
\begin{equation}
 \mu^{x}_{rev}\sim \text{Multinomial}\left(\twoj,\rho_{1},\ldots,\rho_{N}\right)\quad \text{where}\quad \rho_{a}=\frac{\alpha_{a}}{|\alpha|}\co
\end{equation}
namely
\begin{equation}
...
\subsection{The integrable chain and the non-equilibrium steady state}
...
{\color{blue}
\begin{equation}
  \mathcal{S}_{2}^{-1} |\tau\rangle= |\tau\rangle+\delta_{N,\tau}\sum_{a=1}^{N-1}\beta_a|a\rangle
\end{equation} 
\begin{equation}
  \left(\mathcal{S}_{2}^{-1}\right)^{\otimes L} |\bm{\tau}\rangle=\left( |\tau_1\rangle+\delta_{N,\tau_1}\sum_{a=1}^{N-1}\beta_a|a\rangle\right)\otimes\ldots\otimes \left( |\tau_L\rangle+\delta_{N,\tau_L}\sum_{a=1}^{N-1}\beta_a|a\rangle\right)
\end{equation} 
\begin{equation}
  \langle \bm{\mu}|\left(\mathcal{S}_{2}^{-1}\right)^{\otimes L} |\bm{\tau}\rangle=\prod_{i=1}^L\left[ \delta_{\mu_i,\tau_i}+\delta_{N,\tau_i}\beta_{\mu_i} (1-\delta_{\mu_i,N})\right]
\end{equation} 
...
we obtain 
    \begin{equation}
\Psi^{''}(\bm{\tau})=\frac{\Gamma(2+\sum_{x=1}^{L}\delta_{\tau_{x},N})}{\Gamma(L+2)}\prod_{y=1}^{L}\left(\lambda_{\tau_{y}}\left(2+L-y-\sum_{j=y}^{L}(1-\delta_{\tau_{j},N})\right)\right)^{1-\delta_{\tau_{y},N}}
\end{equation}
Introducing
\begin{equation}
	c_{y}\in\{0,1\} \qquad \forall y\in \{1,\ldots L\}
\end{equation}
...
\newline \newline
	...
	\begin{comment}
		...
		\end{equation}} 
	\end{comment}
indeed, in each coordinate $q_{r}$ with $r=1,\ldots b$ the species $N$ is replaced by all other possible species from $1$ to $N-1$.  Finally, considering all possible $\tau_{1},\ldots,\tau_{L}\in \{1,\ldots,N\}$, we obtain \eqref{steadyStateH}. 
		\begin{flushright}
			$\square$
		\end{flushright}
\begin{comment}
