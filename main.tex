...
\end{comment}
\subsubsection{The steady state of $J^{''}$}\label{subsectionSSHsec}
...
\end{equation}
\paragraph{Proof of formula \eqref{ABS-vect}} As a consequence of \eqref{SteadyStateHatTilde}, we only need to show how the transformation $\mathcal{S}_{2}^{-1}$ acts on the vector $|\Psi^{'}\rangle$. The matrix $S_{2}^{-1}$, acting on a single site, is given by 
\begin{equation}
...
\end{equation}
We iterate this tensor product for the chain of length $L$, and we write the raw with indices arbitrary $\bm{s}=s_{1},\ldots,s_{L}\in \{1,\ldots,N\}$ as
\begin{equation}
	\begin{split}
  ...
  \ldots+\left(\prod_{x=1}^{L}(1-\delta_{s_{x},1})\right)\prod_{x=1}^{L}\left(\beta_{s_{x}}\right)^{1-\delta_{s_{x},1}}e_{s_{1},\ldots,s_{L}}^{1,\ldots,1}
	\end{split}
\end{equation}
The product $\mathcal{S}_{2}^{-1}|\Psi^{''}\rangle$ gives a vector whose component are denoted by $\Psi^{'}(\bm{s})$ that are given by 
...
we obtain 
    \begin{equation}
\Psi^{''}(\bm{s})=\frac{\Gamma(2+\sum_{i=1}^{L}\delta_{s_{x},1})}{\Gamma(L+2)}\prod_{x=1}^{L}\left(\lambda_{s_{x}}\left(2+L-x-\sum_{j=x}^{L}(1-\delta_{s_{x},1})\right)\right)^{1-\delta_{s_{x},1}}
\end{equation}
...
\begin{equation}\label{elementsABS}
	\begin{split}
		\Psi^{'}(\mathbf{s})=\sum_{c_{1}=0}^{1-\delta_{s_{1},1}}\ldots\sum_{c_{L}=0}^{1-\delta_{s_{L},1}}\frac{\Gamma(2+L-\sum_{a=1}^{L}c_{a})}{\Gamma(L+2)}\prod_{x=1}^{L}\left(\lambda_{s_{x}}\left(2+L-x-\sum_{j=x}^{L}c_{j}\right)\right)^{c_{i}}\left(\beta_{s_{x}}\right)^{(1-c_{i})(1-\delta_{s_{x},1})}
	\end{split}
\end{equation} 
