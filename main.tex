...
\end{equation}
where for every $\bm{\tau}=(\tau_{1},\ldots,\tau_{L})$
\begin{equation}\label{SteadyState-coeff-implicit}
	\begin{split}
		\Psi(\bm{\tau})=\sum_{b=0}^{L}(-1)^{b}\sum_{1\leq q_{1}<\ldots<q_{b}\leq L}^{L}\left(\prod_{k=1}^{b}\delta_{\tau_{q_{k}},N}\right)\sum_{i_{q_{1}},\ldots i_{q_{b}}=1}^{N-1}\mathbb{E}_{\mu_{\text{NESS}}}\left[\prod_{k=1}^{b}\mathbbm{1}_{\{\sigma_{q_{k}}=i_{q_{k}}\}}\prod_{x=1\,:\,\tau_{x}\neq N}^{L}\mathbbm{1}_{\{\sigma_{x}=\tau_{x}\}}\right]
	\end{split}
\end{equation}
where the last expectation is taken with respect to the $\mu^{\text{NESS}}$ of the process $\bm{\sigma}$ and is given in \eqref{exactCorrelations}.
\end{proposition} %Therefore, 
%\begin{equation}
...
\begin{align}
	...
	=&\sum_{\bm{\sigma}\in \Omega}\Psi^{'}(\bm{\sigma})\sum_{b=0}^{L}(-1)^{b}\sum_{1\leq q_{1}<\ldots<q_{b}\leq L}\left(\prod_{x=1\,:\,x\notin\{q_{1},\ldots,q_{b}\}}^{L}\delta_{\tau_{x},\sigma_{x}}\right)\left(\prod_{x\in \{q_{1},\ldots,q_{b}\}}\delta_{\tau_{x},N}\sum_{i_{x}=1}^{N-1}\delta_{\sigma_{x},i_{x}}\right)\nonumber\\
	=&\sum_{b=0}^{L}(-1)^{b}\sum_{1\leq q_{1}<\ldots<q_{b}\leq L}\left(\prod_{k=1}^{b}\delta_{\tau_{q_{k}},N}\right)\sum_{i_{q_{1}},\ldots,i_{q_{b}}=1}^{N-1}\mathbb{E}_{\mu_{\text{NESS}}}\left[\prod_{k=1}^{b}\mathbbm{1}_{\{\sigma_{q_{k}}=i_{q_{k}}\}}\prod_{x=1\,:\,\tau_{x}\neq N}^{L}\mathbbm{1}_{\{\sigma_{x}=\tau_{x}\}}\right]
\end{align}
In the last equality we exploited the fact that considering any $\tau_{x}\in \{1,\ldots,N\}$ and $i_{x}\in \{1,\ldots,N-1\}$, and considering a configuration $\bm{\sigma}=(\sigma_{1},\ldots,\sigma_{L})$ such that 
\begin{equation}
	\begin{cases}
		\sigma_{x}=i_{x}\qquad &\text{if}\quad x\in \{q_{1},\ldots,q_{b}\}\\
		\sigma_{x}=\tau_{x}\qquad &\text{if}\quad \text{otherwise}
	\end{cases}
\end{equation}
we have that 
\begin{equation}
	 \Psi^{'}(\bm{\sigma})=\mathbb{E}_{\mu_{\text{NESS}}}\left[\prod_{k=1}^{b}\mathbbm{1}_{\{\sigma_{q_{k}}=i_{q_{k}}\}}\prod_{x=1\,:\,\tau_{x}\neq N}^{L}\mathbbm{1}_{\{\sigma_{x}=\tau_{x}\}}\right]
\end{equation}
as showed in Section \ref{correlation-section}. 
	%	\begin{equation}
	%		\begin{split}
...
\end{comment}
 Finally, considering all possible $\tau_{1},\ldots,\tau_{L}\in \{1,\ldots,N\}$, we obtain \eqref{SteadyState-coeff-implicit}. 
		\begin{flushright}
			$\square$
		\end{flushright}
	\begin{remark}[Proof of \eqref{SteadyState-coeff-implicit} using the conservation of probability]
		Equation \eqref{SteadyState-coeff-implicit} can be proved also using the conservation of probability in the non-equilibrium steady state. Consider a generic $\bm{\tau}\in \Omega$, then we have 
\begin{align}
...
\end{align}
Introducing the summation over $b$, the coordinates $(q_{k})_{k\in {1,\ldots,b}}$ and the indices $i_{q_{k}}$ as in the previous proof and arguing as in Section \ref{correlation-section}, one find \eqref{SteadyState-coeff-implicit}. 
\begin{comment}
