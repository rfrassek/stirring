...
Furthermore, reservoirs are attached at each site of the graph, creating and removing particle. The process becomes integrable when the graph is a finite chain with two reservoirs attached to the first and the last sites respectively and the maximal number of allowed particles is fixed to be $\nu=1$,  see \cite{vanicat2017exact}. {\color{blue}The choice of the boundary reservoirs introduced in \cite{vanicat2017exact} is further motivated by the fact that they allow to determine an absorbing dual process, regardless of the geometry on which the process is defined and regardless of the maximal number of particle hosted at each site ($\nu$ can any natural number)}.\\
{\color{blue}In this paper we concentrate our effort in studying the symmetric multispecies model. In literature, also multispecies asymmetric models have been introduced \cite{schutz2017kardar,crampe2016integrable, finn2018matrix} and integrability results have been proved. Moreover, different transition mechanism (beyond the diffusive one) have been recently studied \cite{chatterjee2023multi}. Finally, the presence of different types of particles have also been modelled via a "multi-layer" dynamics (see \cite{redig2022ergodic,floreani2022switching}). As far as we know, in these contexts integrability does not hold.} 
...
and generalizations of the model, including instantaneous thermalization limits and reaction-diffusions. In particular, a thermalized boundary-driven version of the multispecies stirring process is introduced, showing that it has an absorbing duality. Moreover, a further interaction mechanism of reaction type is added in order to have closed evolution equation for average particle density (inspired by \cite{casini2022uphill,schutzReaction}). Again, an absorbing duality is proved and the duality function for the thermalized and the reaction diffusion process turns out to be the same of the multispecies stirring process.
...
{\color{magenta} Cristian I think it is good to keep some of the following citations, especailly the one to Schutz...
 \cite{schutz2017kardar, crampe2016integrable, finn2018matrix, floreani2022switching, casini2023density, redig2022ergodic, redig2023equilibrium, chatterjee2023multi,  franceschini2022orthogonal,floreani2023non,carinci2023open}
...
For  $\nu=1$ we denote the ${gl}(N)$  generators   as  $(e_{AB})_{A,B\in\{1,\ldots,N\}}$ obeying $(e_{AB})_{CS}=\delta_{AC}\delta_{BD}$.
...
% However, they would lead to duality matrices where the number of dual particles is greater than the number of original ones, loosing the simplification of the dynamics of the dual process. 
 ...
 \begin{equation}\label{HadamardFormula}
     \exp{(-\mathcal{B})}A\exp{(\mathcal{B})}=\mathcal{A}-\left[\mathcal{B},\mathcal{A}\right]+\frac{1}{2!}\left[B\mathcal{B},\left[\mathcal{B},\mathcal{A}\right]\right]-\frac{1}{3!}\left[\mathcal{B},\left[\mathcal{B},\left[\mathcal{B},\mathcal{A}\right]\right]\right]+\ldots
 \end{equation}
...
\end{equation}
{\color{blue}where the expectation is taken with respect to the non-equilibrium steady state, then it computed by using the correlations of equation \eqref{exactCorrelations}.}
\todo{Explain this expectation...}
\end{corollary}
...
\end{equation} 
where $\mathcal{P}_{x_1}(1)$ is the probability that a single random walk started at $x_1$ is absorbed {\color{blue}at site $0$}.\todo{site 0?} Then, using \eqref{exactCorrelations}, we have
...
\end{align}
\todo{should this all be $Y$ in the identities?[DONE]}
...
of polynomials in the left and right boundary densities with coefficients given by the absorption probabilities, as claimed in Corollary~\ref{Corolollary-ABS_Corr-abstract}.
	...
	\todo{added ref... What about Section 4.4.5?[PREVIOUS SECTION 4.4.5 NOW MOVED IN THE APPENDIX A]}
	\end{itemize}
...
\end{equation}
		...
		\end{align}
		\todo{can we remove q index in sum?[SUB-INDICES q REMOVED]}
		...
		\begin{equation}\label{notation-PSI}
			\Psi^{'}(\tau_{1},\ldots,\underbrace{a_{1}}_{q_{1}},\ldots,\underbrace{a_{b}}_{q_{b}},\ldots,\tau_{L})
		\end{equation}
		means that, for all $b\in \{0,\ldots,L\}$ and for all $1\leq q_{1}<\ldots<q_{b}\leq L$ {\color{blue} at sites $q_{k}$ for $k=1,\ldots,b$ we have a particle of type $a_{k}$, while at the other sites ($x\notin\{q_{1},\ldots,q_{b}\}$) the occupation $\tau_{x}$ is left unchanged. }
		\todo{?[EXPLAINED]}
...
\end{pmatrix}\dt
\end{equation}
{\color{blue}As a further check we report the steady state correlations and the vectors $|\Psi^{''}\rangle$, $|\Psi^{'}\rangle$ and $|\Psi\rangle$ for the SSEP, i.e. the situation where the process has only one species of particles and holes, with maximal occupancy $\nu=1$. }
...
\end{equation} 
\textbf{Proof of formula \eqref{correlations-SSEP}:} Starting from \eqref{exactCorrelations} we exploit indistinguishability of particles and we introduce $q=\sum_{k=1}^{m}c_{x_{k}}$. There are many values of $c$'s that give the same $q$, therefore we define {\color{blue} $q$ labels denoted by } %labels denoted by $\ell_{1},\ldots,\ell_{q}\in \{1,\ldots,m\}$ such that  
$1\leq \ell_{1}<\ell_{2}<\ldots<\ell_{q}\leq m$. 
\todo{?[BLUE SENTENCE REWRITTEN]}
To each of these label we associate the coordinates $x_{\ell_{1}},\ldots,x_{\ell_{q}}\in\{x_{1},\dots,x_{m}\}$ that are such $c_{x_{\ell_{k}}}=1$ for all $k\in \{1,\ldots,q\}$ and $c_{x}=0$ for $x\notin \{x_{\ell_{1}},\ldots,x_{\ell_{q}}\}$. Therefore, for any fixed $c_{x_{1}},\ldots,c_{x_{m}}$, we find $q$ and $\ell_{1},\ldots,\ell_{q}$ from which we fix $x_{\ell_{1}},\ldots,x_{\ell_{q}}$. Then, we obtain  
	\begin{equation}
...
\end{equation}
\todo{is there $c_j$ and $c_k$ missing?[DONE]}
...
\begin{equation}\label{dualSpace-chain}
	\widetilde{\Omega_{L}}=\mathbb{N}_{0}^{N-1}\otimes \Omega_{L}\otimes \mathbb{N}_{0}^{N-1}\dt
\end{equation}
...
\begin{align*}
	&\left[\left(\sum_{a=1}^{N-1}E_{aN}^{x}\otimes\sum_{a=1}^{N-1}E_{aN}^{x+1}\right), \sum_{D,K=1}^{N}\left(E_{h_{c}(K)D}^{x}\otimes E_{h_{c}(D)K}^{x+1} -E_{DD}^{x}\otimes E_{KK}^{x+1}\right)\right]
	\\=&
...
%	\\=&
	\sum_{a,b=1}^{N-1}\sum_{D,k=1}^{N}\left\{\left(E_{DN}^{x}-E_{h_{c}(k)N}^{x}\right)\otimes \left(E_{DN}^{x+1}-E_{h_{c}(K)N}^{x+1}\right)\right\}=0\dt
\end{align*}
...
\begin{equation}
	\left[\sum_{a=1}^{N-1}E_{aN}^{x}\otimes \sum_{b=1}^{N-1}E_{bN}^{x},\;\sum_{D,K=1}^{N}\left(E_{K a}^{x}\otimes \mathbbm{1}^{x+1}-E_{DD}^{x}\otimes \mathbbm{1}^{x+1}\right)\right]=0\dt
\end{equation}
\begin{flushright}
	$\square$
\end{flushright}
