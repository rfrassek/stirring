...
\begin{align*}
	\langle W|\widetilde{X}_{N}^{n_{N}}|V\rangle&=\langle W|\widetilde{X}_{N}\widetilde{X}_{N}^{n-1}|V\rangle=\langle W|\widetilde{X}_{N}^{n-1}|V\rangle +\langle W|\frac{1}{(\alpha_{a}-\beta_{a})}\widetilde{X}_{a}\widetilde{X}_{N}^{n-1}|V\rangle
	\\&=
...
=&\frac{\Gamma(2+\sum_{x=1}^{L}\delta_{\tau_{x},N})}{\Gamma(L+2)}\prod_{x=1}^{L}\frac{\Gamma(2+\sum_{j=x+1}^{L}\delta_{\tau_{j},N})}{\Gamma(1+\sum_{j=x}\delta_{\tau_{j},N})}\nonumber\\
...
\begin{flushright}
    $\square$
\end{flushright}
\begin{remark} By change the summation indices form $c_x\in \{\delta_{\tau_{x},N},1\}$ to $c_{x}\in \{0,1-\delta_{\tau_{x},N}\}$ and by  the fact that occupation of each site is bounded by $1$ we rewrite \eqref{ABS} as 
