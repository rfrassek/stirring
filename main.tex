...
\todo{mention that there is an appendix. In particular Appendix B could be interesting to stress.[DONE, SEE BLUE TEXT]}
...
\todo{explain extra index [IS THE EXPLANATION OF REMARK 4 ENOUGH?]}  
...
\end{equation}
\todo{Why not merge sum or is there a typo? [FIXED,THERE WAS A TYPO]}
...
Indeed, 
\begin{enumerate}
	\item \textit{\underline{Stirring generator}}: {\color{blue}this is the edge generator $\mathcal{L}_{x,x+1}$ of the multispecies stirring process introduced in Section \ref{sec1} (with $y=x+1$). As already pointed out, in this nearest neighbour chain, its action consists in exchanging the particles or the holes and the particles of two adjacent sites. Here we show that its action on the occupation variable of any species gives a discrete gradient in the occupation variable itself. Indeed, we have that }
\todo{add text here [DONE]}
\begin{equation}
