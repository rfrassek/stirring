...
\begin{align*}
	\mathcal{L}_{x,x+1}n_{d}^{x}=&\sum_{a=1}^{N}n_{A}^{x}n_{d}^{x+1}-\sum_{bB=1}^{N}n_{d}^{x}n_{B}^{x+1}\\
...
\begin{equation}
	...
	\end{split}
\end{equation}
In the brackets of the right-hand-side of the above equation we have: a contribution $-n_{d}^{x}n_{K}^{x+1}$, when $A=d$ (here we call $K=h_{c}(B)$); a contribution $n_{A}^{x}n_{q}^{x+1}$, when $h_{c}(B)=d$. Thus we obtain 
\begin{align}
		\mathcal{L}_{x,x+1}^{c}n_{d}^{x}&=\sum_{A=1}^{N}n_{A}^{x}n_{b}^{x+1}-\sum_{K=1}^{N}n_{d}^{x}n_{K}^{x+1}
		\\&=
		\sum_{k=1}^{N-1}n_{k}^{x}n_{b}^{x+1}+\left(\nu-\sum_{i=1}^{N-1}n_{i}^{x}\right)n_{b}^{x+1}-\sum_{K=1}^{N-1}n_{d}^{x}n_{K}^{x+1}-n_{d}^{x}\left(\nu-\sum_{i=1}^{N-1}n_{i}^{x+1}\right)
		\\&=
		\nu(n_{b}^{x+1}-n_{d}^{x})
\end{align}
