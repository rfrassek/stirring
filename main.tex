...
where the notation
		...
		\begin{equation}
			...
			\end{cases}
		\end{equation}
		In the second equality we have rewritten the product by introducing the summation over $b$, the coordinates $q_{1},\ldots,q_{b}$ and the occupations variables $i_{q_{1}},\ldots,i_{q_{k}}$. In the last equality we have summed over all possible $\sigma\in \Omega^{'}$. From Section \ref{correlation-section} we recall that the components of $|\Psi^{'}\rangle$ are the correlation in the non-equilibrium steady state. Thus, we have that  
			\begin{equation}
			\Psi(\bm{\tau})=\sum_{b=0}^{L}(-1)^{b}\sum_{1\leq q_{1}<\ldots<q_{b}\leq L}\left(\prod_{k=1}^{b}\delta_{\tau_{q_{k}},N}\right)\sum_{i_{q_{1}},\ldots,i_{q_{b}}=1}^{N-1}\mathbb{E}_{\mu}\left[\prod_{k=1}^{b}\mathbbm{1}_{\{\sigma_{q_{k}}=i_{q_{k}}\}}\prod_{x=1\,:\,x\notin \{q_{1},\ldots,q_{b}\}}^{L}\mathbbm{1}_{\{\sigma_{x}=\tau_{x}\}}\right]
		\end{equation}
			%	\begin{equation}
