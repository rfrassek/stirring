...
$$}
	\todo{To me this looks like it is zero in (3.4.12). Can you explain the notation?}
where in the last equality we exploited the fact that 
\begin{equation}\label{prova}			\mathbbm{1}_{\{Y_{x}=N\}}=1-\sum_{a_{x}=1}^{N-1}\mathbbm{1}_{\{Y_{x}=a_{x}\}}\dt
\end{equation}
{\color{blue}
Here I mean 
\begin{equation}
...
\end{align} 
\todo{indices are strange. What happens here?}
