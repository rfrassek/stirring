...
\end{equation}
conditions \eqref{condition1} and \eqref{condition2} are easilly satisfied. To fulfill condition \eqref{condition3}% we can think about $m_{1}:=L-m_{2}-m_{3}$ as the number of empty states in the whole chain. We can further introducte $m:=m_{2}+m_{3}$ and then write $m_{1}=L-m$. Therefore we have an invertivle map between holes and particles, therefore we can think about $\left(B_{2}(x)+B_{3}(x)\right)$ as a lowering operator and 
