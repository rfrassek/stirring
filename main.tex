...
%\end{equation}
\textbf{Remark}: In case of $N=2$ we can argue as in the case of $|\Psi^{''}\rangle$ and we obtain 
\begin{equation}
	|\Psi^{'}\rangle= 
\end{equation}
...
\begin{equation}\label{elementsABS}
	\begin{split}
		\Psi^{'}(\bm{\tau})=&\sum_{c_{1}=0}^{1-\delta_{\tau_{1},N}}\ldots\sum_{c_{L}=0}^{1-\delta_{\tau_{L},N}}\mathbbm{1}_{\{c_{1}+\ldots+c_{L}>0\}}\frac{\Gamma(2+L-\sum_{x=1}^{L}c_{x})}{\Gamma(2+L)}\\&\prod_{y=1}^{L}\left(\lambda_{\tau_{y}}\left(2+L-y-\sum_{j=y}^{L}c_{j}\right)\right)^{c_{y}}\left(\beta_{\tau_{y}}\right)^{(1-c_{y})(1-\delta_{\tau_{y},N})}
	\end{split}
\end{equation} 
...
\\ \\
\textbf{Remark}: If we stop distinguishing the types of particles we retrieve the closed formula for correlations in the non-equilibrium steady state found in \cite{frassek2020eigenstates}. Indeed, introducing $q=\sum_{j=1}^{m}c_{i}$ in the colour-blind situation \eqref{exactCorrelations} becomes
\begin{align}
