...
\begin{equation}\label{siteDualGenerator}
    \widetilde{\mathcal{L}}_{x}f(\bm{\xi})=|\alpha^{x}|\sum_{a=1}^{N-1}\xi_{a}^{x}\left(f(\bm{\xi}-\bm{\delta}_{a}^{x}+\bm{\delta}_{1}^{x}+\bm{\delta}_{a}^{u(x)})-f(\bm{\xi})\right)
\end{equation}
\newline
...
% However, they would lead to duality matrices where the number of dual particles is greater than the number of original ones, loosing the simplification of the dynamics of the dual process. 
 ...
 \end{equation}
In the following we need the application of the Hadamard formula with $B=\sum_{a=1}^{N-1}E_{a1}$ defined in \eqref{equationE} and with $A$ equal to some of the generator $E_{ab}$ of the Lie algebra. Therefore we compute 
\begin{enumerate}
...
\begin{equation}\label{transformationV}
 \mathcal{S}_{1}^{x}:=\exp{\left(\sum_{a=1}^{N-1}e_{Na}^{x}\right)}%=\mathbbm{1}+\sum_{a=2}^{N}e_{1a}
\end{equation}
...
We introduce the second transformation as 
 \begin{equation}
 	\mathcal{S}_{2}^{x}:=\exp{\left(-\sum_{a=1}^{N-1}\beta_{a}e_{aN}^{x}\right)}%=\mathbbm{1}-\sum_{a=2}^{N}\beta_{a}e_{a1}
 \end{equation}
...
therefore 
\begin{align*}
	\widetilde{X}_{N}^{n_{1}}\ldots\widetilde{X}_{N-1}^{n_{N-1}}\widetilde{X}_{N}^{n_{N}}=\widetilde{X}_{1}^{n_{1}}\prod_{a=1}^{N-1}\left(\widetilde{X}_{a}+n_{N}\lambda_{a}\right)^{n_{a}}
\end{align*}
...
\begin{equation}
\begin{split}
\langle n^{x_{1}}_{a_{1}}\cdots n^{x_{m}}_{a_{m}}\rangle_{\text{stat}}=&
\sum_{c_{1},\ldots,c_{m}=0}^{1}
f(L,c_{1},\ldots,c_{m})\prod_{q=1}^{m}\left(\sum_{t_{k}=0}^{c_{k}}(-1)^{c_{q}-t_{q}}\alpha_{a_{q}}^{t_{q}}\beta_{a_{q}}^{1-t_{q}}g_{q}(L,x_{q},c_{q},\ldots,c_{m})\right)\\
=&\sum_{c_{1},\ldots,c_{m}=0}^{1}
...
=&\sum_{t_{1},\ldots,t_{m}=0}^{1}\prod_{q=1}^{m}\alpha_{q}^{t_{q}}\beta_{q}^{1-t_{q}}\sum_{c_{1}=t_{1}}^{1}\ldots\sum_{c_{m}=t_{m}}^{1}(-1)^{c_{q}-t_{q}}f(L,c_{1},\ldots,c_{m})g_{q}(L,x_{q},c_{q},\ldots,c_{m})
\end{split}
\end{equation}
...
\paragraph{Proof of formula \eqref{exactCorrelations}} Consider $m$ particles of non-empty types $a_{i}\in\{2,\ldots,N\}$ in positions $x_{i}\in\{1,\ldots,L\}$ for all $i=1,\ldots,m$. We aim to compute the non-equilibrium stationary state correlation $\langle n_{a_{1}}^{x_{1}}\cdots n_{a_{m}}^{x_{m}} \rangle_{stat}$. By using duality, we showed in \eqref{CorrelationByABS-prob} that
 ...
 \end{equation} 
Using formula \eqref{ABS} for a configuration where all $\tau_{x_{i}}=a_{i}$  for $i=1,\ldots,m$ and $\tau_{j}=N$ for $j\in\{1,\ldots,L\}\setminus \{x_{1},\ldots,x_{m}\}$, we obtain 
\begin{equation}
 \langle\hat{\bm{\xi}}\rangle_{stat,dual}=\Psi^{''}(\tau_{1},\ldots,\tau_{L})=\sum_{c_{1}=0}^{1}\ldots\sum_{c_{L}=0}^{1}\frac{\Gamma(2+L-\sum_{a=1}^{m}c_{k})}{\Gamma(2+L)}\prod_{k=1}^{m}\lambda_{\tau_{k}}^{c_{k}}\beta_{\tau_{k}}^{1-c_{k}}\left(2+L-x_{k}-\sum_{j=k}^{m}c_{j}\right)^{c_{k}}
\end{equation}
Introducing $f(L,c_{1},\ldots,c_{m})$ and $g_{k}(L,x_{k},c_{k},\ldots,c_{m})$ we have \eqref{exactCorrelations}.
\begin{comment}
{\color{blue}
...
The average with respect to the non equilibrium distribution of the occupation variable of the species $a_{1}\in \{1,\ldots,N-1\}$ at coordinate $x_{1}\in \{1,\ldots,L\}$ reads
\begin{equation}
...
The three points correlations with respect the non equilibrium distribution of the occupation variable of the species $a_{1},a_{2},a_{3}\in \{1,\ldots,N-1\}$ at coordinates $x_{1},x_{2},x_{3}\in \{1,\ldots,L\}$ such that $x_{1}<x_{2}<x_{3}$
\begin{equation}
...
\begin{equation}\label{GoalWH}
	H^{''}W=WH_{0}
\end{equation}
 We denote by $\bm{m}=(m_{1},\ldots,m_{N})$ an $N-$dimensional vector where each $m_{i}$ means the total occupation over the chain of each species $i\in\{1,\ldots,N\}$. Therefore, we call $\mathcal{E}_{\bm{m}}$ and $|\Psi_{\bm{m}}^{0}\rangle$ the eigenvalue and the eigenvector of $H^{0}$ that correspond to a certain $\bm{m}$. We prove that 
...
\end{equation}
\paragraph{Construction of matrix W}
...
\begin{equation}
	(z-H^{''})R(H^{''},z)=I
\end{equation}
In order to be well defined $z$ must not be an eigenvalue of $H^{''}$. We chose $z=\mathcal{E}_{\bm{m}}+\epsilon$ 
\begin{equation}
...
\begin{equation}
	\lim_{\epsilon\to 0}\epsilon\sum_{\bm{m}\,:\, |\bm{m}|=L}  \Lambda_{\bm{m}}R(H^{''},\mathcal{E}_{\bm{m}}+\epsilon)|\Psi_{\bm{m}}^{0}\rangle \langle \Psi_{\bm{m}}^{0}|= H^{''}\lim_{\epsilon\to 0}\epsilon\sum_{\bm{m}\,:\, |\bm{m}|=L}  R(H^{''},\mathcal{E}_{\bm{m}}+\epsilon)|\Psi_{\bm{m}}^{0}\rangle \langle \Psi_{\bm{m}}^{0}|
\end{equation}
thus 
\begin{equation}
...
\end{equation}
with $C_{1},C_{2}>0$. \\
The double row monodromy is given by
\begin{equation}
...
\begin{equation}
	...
	\end{split}
\end{equation}
therefore, it is a polynomial in $x$ with exponents from $0$ to $2L+2$ 
\begin{equation}
	\begin{split}
		T^{\Delta}(x)=
	...
	\end{split}
\end{equation}
where, for all $k\in\{0,\ldots,2L+2\}$, we call these $Q_{k}$ the \textit{conserved charges} of the system. We also introduce 
\begin{equation}
...
\end{equation}
 Only the first three coefficients of the diagonal elements and the first coefficient of the non diagonal elements do survive when $x\to \infty$, therefore we only compute them. We obtain:
\begin{itemize}
	\item For $D_{N,N}(x)$: 
...
Consider the matrix $Q_{0}^{2L}$ defined in \eqref{Q0Exp}. All terms are diagonal except the last one. However, this last non-diagonal term acts diagonally on the states. Indeed, can be identified with the second Casimir of the $gl(N-1)$ subalgebra. We denote it by $\hat{C}_{2}^{N-1}$ and it reads
\begin{equation}
\hat C_2^{N-1}= \sum_{i,j=1}^{N-1}e_{ij}^{tot}e_{ji}^{tot}
\end{equation} 
We denote its eigenvector by $C_{2}^{N-1}$. On the Bethe states (and on the decomposition into irreducible representation of the spin chain) it acts diagonally with eigenvalues $C_{2}^{N-1}$ given in terms of shifted weights, i.e. 
\begin{equation}
...
\left[D_{j,j}^{2L},B_{i}^{2L-1}\right]=0\qquad \left[B_{i}^{2L-1},B_{j}^{2L-1}\right]=0\qquad \left[B_{i}^{2L-1},D_{j,i}^{2L}\right]=0%\qquad \left[D_{12}^{2L},B_{2}^{2L-1}\right]=0\qquad \left[D_{11}^{2L},B_{2}^{2L-1}\right]=0
\end{equation}
...
In the case of $gl(2)$ we have that for every magnon number $m\in\{0,\ldots,L\}$ non-equilibrium eigenvectors $|\Psi_{m}\rangle$ of $H^{''}$, are written in terms of the eigenvectors $|\Psi_{m}^{0}\rangle$ of the equilibrium Hamiltonian $H_{0}$ by the following formula 
...
\end{equation}
Power expanding the matrix $W$ with respect to $\Delta=(\alpha-\beta)$ we obtain 
...
\begin{equation}\label{exchangeQk}
	\begin{split}
		...
		&=(B_{1}^{2L-1})^{k}\prod_{j=1}^{k}\left(Q_{0}^{2L}-2j(D_{2,2}^{2L}+D_{1,1}^{2L})+4j(j-1)N\right)\\
...
\begin{equation}\label{W-charges-Bad}
	...
	\end{split}
\end{equation}
where $\Lambda_{\bm{m}}$ are the eigenvectors of $Q_{0}^{2L}$ corresponding to a configuration $\bm{m}$ of the chain. \\
Indeed, since the exchange relation \eqref{ExQ0B-charges} has an extra creation operator
\begin{equation}
	-2\sum_{k=1\,:\,k\neq i}^{N-1}B_{k}^{2L-1}D_{k,i}^{2L}
\end{equation}
that gives rise to a proliferation of terms that does not vanish, thus only part of the argument done in the $N=2$ case can be followed. However, as we will show in subsection \ref{subsection-NONSS-W}, a closed formula for the steady state can be written, since when acting on the vacuum the exchange relations do remain simple. 
...
Using \eqref{W-charges-Bad}, it is possible to find an explicit expression of the steady state $|\Psi^{''}\rangle$ of $H^{''}$ when $N>2$ by actin with $W$ on the vacuum state 
...
\begin{equation}\label{SteadyState-W-Good}
	...
	\end{split}
\end{equation}
\textbf{Remark}: relation \eqref{SteadyState-W-Good} holds only for the action of $W$ on the vacuum state $|\Psi_{0}^{0}\rangle$. Acting on different equilibrium eigenstates, exchange relations between $B_{i}^{2L-1}$ and $Q_{0}^{2L}$ are more difficult and we cannot rewrite \eqref{W-charges-Bad} in such a simple form. 
\subsubsection{Proof of \eqref{SteadyState-W-Good}}
To prove \eqref{SteadyState-W-Good} we do four steps. 
...
\end{align}
We have that, using \eqref{GoodRelation}, \eqref{Dij-ref} and \eqref{Dii-ref},
\begin{align}
...
\begin{equation}\label{intermediate-W-ss}
	...
	\end{split}
\end{equation}
Using \eqref{veryUsefulSeries} and observing that  we have that $2j(D_{1,1}^{2L}+D_{N,N}^{2L})-4j(j-1)$ is diagonal we have that, for all $j\in{1,\ldots,k_{1}+\ldots+k_{N-1}}$ 
\begin{align}
...
In Subsection \ref{subsection-exact} and \ref{SectionNonLocalTrasformation} we have found three similarity transformations, namely $S_{1}$, $S_{2}$ and $W$ between operators $H$, $H^{'},H^{''}$ and $H_{0}$. Using them simultaneously one can map the non-equilibrium Hamiltonian $H$ onto the equilibrium one as
\begin{equation}
	H\;\xleftrightarrow{S_{1}}\;H^{'}\;\xleftrightarrow{S_{2}}\;H^{''}\xleftrightarrow{W}\;H_{0}
\end{equation} 
...
\end{equation}
As a consequence we have that, for a fixed configuration $\bm{m}$, the non-equilibrium eigenvectors can be mapped to the equilibrium ones as
\begin{equation}
...
The aim of this section is studying some extension to the analysis previously done when integrability property is lost. 
In particular, Sections \ref{sectionIntegrabiliy} and \ref{Section-Mapping} we have shown that, on the geometry of a chain and with hard-core interaction the model is exactly solvable thanks to integrability.  
This nice property is lost when we allow an arbitrary maximal occupation $\nu>1$. For the sake of simplicity we still study the geometry of chain and we provide the following extensions: 
\begin{enumerate}
	...
	\item \textit{Instantaneous thermalization}: starting from the multi-species stirring dynamics, we can define the thermalized process, where particle are redistribute in each bond with a measure that is the reversible one conditioned to the conservation of the number of particles on this bond. Subsection \ref{Subsection-Thermalized}.
...
The dual process has absorbing boundaries, i.e. all dual particles are eventually absorbed at $0$ or at $L+1$ with a certain probability. This implies that, the chain is eventually void and the steady state correlation of the original process can be written via the absorption probabilities of the dual process as explained in the following. 
We call $\mu_{stat}$ the stationary measure of the the process $(\bm{n}(t))_{t\geq 0}$. We assume that it is initially distribute with a configuration $\bm{n}$. For every $m\in\{1,\ldots,L\}$ we consider the dual configuration  $\bm{\widehat{\xi}}=\delta_{\tau_{1}}^{y_{1}}+\ldots+\delta_{s_{m}}^{y_{m}}$ with $y_{x}<y_{x+1}$  and $s_{x}\in \{1,\ldots,N-1\}$ for every $x\in \{1,\ldots,m-1\}$. The following chain of equalities holds
...
\end{equation}
and where $D(\bm{n},\bm{\xi})$ is the duality function defined in \eqref{dualityFunctionChain}. Here, we denote by $|\hat{\xi}_{a}|=\sum_{x=1}^{L}\hat{\xi}_{a}^{x}$ and by $i_{a}$ the number of dual particles of species $a\in\{1,\ldots,N-1\}$ absorbed at left, thus we denote by $|\hat{\xi}_{a}|-i_{a}$ the number of particles of the same type absorbed at right.\\
...
\end{equation}
Therefore, up to a constant, $\mathbb{E}_{\mu^{\text{stat}}}\left[D(\bm{n},\bm{\hat{\xi}})\right]$ gives the non-equilibrium stationary state $m-$points correlations between non-empty species of types $s_{i}\in\{1,\ldots,N-1\}$ situated at sites $y_{x}\in\{1,\ldots,L\}$ with $x\in \{1,\ldots,m\}$. This expectation is computed by \eqref{ExptationSS} once the absorption probabilities is known. The issue is that these absorption probabilities fulfil some difference equations that are not easy to solve. For example, we report here the solution for the one points correlations. 
\paragraph{One point correlations for the non-integrable case}
We have that $\widehat{\bm{\xi}}=\delta_{s}^{y}$, then 
\begin{equation}
...
\begin{equation}\label{OnePtsCORR}
	...
	\end{split}
\end{equation}
we want to determine $\mathcal{P}(1,\delta_{s}^{y})=\mathbb{P}_{\delta_{s}^{y}}\left(\xi_{\infty}=i_{s}\delta_{0}+(1-i_{s})\delta_{L+1}\right)$ is the probability of a random walker on the chain $\{1,\ldots,L\}$ with absorbing boundaries. Therefore, this $\mathcal{P}(1,\delta_{s}^{y})$ fulfils the following equations 
\begin{equation}\label{gamblers-ruin}
	...
	\end{cases}
\end{equation}
where $\Delta_{x}^{L}$ is the discrete Laplace operator on the chain of length $L$. The equations \eqref{gamblers-ruin} can be solved. Therefore, using \eqref{OnePtsCORR} and $\mathcal{P}(1,\delta_{s}^{y})=1-\mathcal{P}(0,\delta_{s}^{y})$ we obtain 
\begin{equation}
...
\end{enumerate}
The aim of this section is to answer to these questions. We first define the process on a chain in order to obtain average occupation evolution equations with $N-1$ discrete Laplacians and a reaction term. Second we construct the Lie algebraic description and we use it to prove absorbing duality in this general case. 
...
\begin{equation}\label{edgeGeneratorRD}
	\mathcal{L}_{x,x+1}^{rd}=\sigma_{11}\mathcal{L}_{x,x+1}+\sigma_{12}\sum_{c=1}^{N-2}\mathcal{L}_{x,x+1}^{c}+(\Upsilon-2\nu\sigma_{12})\mathcal{L}_{x,x+1}^{m}
\end{equation}
...
Moreover,
\begin{equation}
	\mathcal{L}_{x,x+1}^{m}f(\bm{n})=\sum_{a,b=1}^{N}n_{a}^{x}\left(f(\bm{n}-\bm{\delta}_{a}^{x}+\bm{\delta}_{b}^{x})-f(\bm{n})\right)
\end{equation}
...
\textbf{Remark}: observe that the site generator $\mathcal{L}_{left}^{rd}$ has the form of the one of the multi-species stirring process \eqref{siteGenerator}. We wrote it with the uppercase 'rd' in order to underline the fact that we need a specific choice of the reservoirs parameters $\alpha_{a}$, as specified above. 
...
For the boundary transitions we have that, analogously to what happens for the multispecies stirring process, the reservoirs parameters must be such that $\beta_{a}=\alpha_{a}$ and $\rho_{a}=\frac{\alpha_{a}}{|\alpha|}$. As a consequence we have the result. 
...
\begin{equation}
	\bm{\xi}=\left(\xi_{2}^{0},\ldots,\xi_{N}^{0}\right)\otimes\left(\xi_{1}^{x},\ldots,\xi_{N}^{x}\right)^{\otimes L}\otimes \left(\xi_{1}^{L+1},\ldots,\xi_{N}^{L+1}\right)
\end{equation}
where the component $\xi_{a}^{x}$ is interpreted as the number of dual particles of type $a\in \{1,\ldots,N\}$ at site $x$, 
and the component $\xi_{a}^{0}$  gives the number of dual particles of type $a\in \{1,\ldots,N-1\}$ at 
the extra-site $0$ connected to $1$ and similarly for $xi_{a}^{L+1}$. \\
The duality function is given by
\begin{equation}
	D(\bm{n},\bm{\xi})=\prod_{a=1}^{N-1}\left(\rho_{a}^{L}\right)^{\xi_{a}^{0}}\left(\prod_{x=1}^{L}\frac{(\nu -\sum_{a=2}^{N}\xi_{a}^{x})!}{\nu!}\prod_{a=1}^{N-1}\frac{n_{a}^{x}!}{(n_{a}^{x}-\xi_{a}^{x})!}\right)\prod_{a=1}^{N-1}\left(\rho_{a}^{R}\right)^{\xi_{a}^{L+1}}
\end{equation}
%where we denote the \textit{density} of the species $a\in \{2,\ldots,N\}$ imposed by the reservoir at site $x\in V$  by 
%\begin{equation}
%	\rho_{a}^{x}:=\frac{\alpha_{a}^{x}}{|\alpha^{x}|}
...
\end{equation}
Similarly, we will define the dual Hamiltonian $\widetilde{H}^{rd}$. For the edge dynamics, the original and the dual Hamiltonians do coincide, while the boundary ones are again $\widetilde{H}_{left},H_{right}$ defined in \eqref{siteDualHamiltonian}, but with the parameters introduced in \eqref{boundaryParamRD}.\\ We aim to show
\begin{equation}\label{DualityRelationRD}
	(H^{rd})^{T}D=D\widetilde{H}^{rd}
\end{equation}
Therefore, we need to prove that, for arbitrary $x\in\{1,\ldots,L\}$
\begin{align}
	(\mathcal{H}_{x,x+1}^{rd})^{T}D&=D\mathcal{H}_{x,x+1}^{rd}\label{edgeDualityRelationRD}\\
	H_{left}^{T}D&=D\widetilde{H}_{left}D\label{siteDualityRelationRD}
\end{align}
and similarly for $H_{right}$.
The duality matrix $D$ is again \eqref{dualityMatrix}. As already pointed out, $H_{left}$ and $\widetilde{H}_{left}$ have the same structure (up to a specific choice of the reservoirs parameters) therefore the duality relation for the boundaries is proved. However, $\mathcal{H}_{x,x+1}^{rd}$ has two more terms $\sum_{c=1}^{N-2}\mathcal{H}_{x,x+1}^{c}$ and $\mathcal{H}_{x,x+1}^{m}$. Therefore, we only need to show that 
\begin{align}
...
\begin{equation}
	...
	\begin{equation}\label{S1-L1}
		\mathcal{S}_{1}^{-1}=\sum_{\tau_{1}=1}^{N}e_{\tau_{1}}^{\tau_{1}}+(-1)\delta_{\tau_{1},N}\sum_{d_{1}=2}^{N}e_{1}^{d_{1}}
	\end{equation}
	...
	\begin{equation}
		\begin{split}
			...
			\\&
			+\left.\sum_{b=1}^{L}(-1)^{b}\sum_{q_{1}=1}^{L}\sum_{q_{2}=q_{1}+1}^{L}\ldots\sum_{q_{b}=q_{b-1}+1}^{L}\left(\prod_{r=1}^{b}\delta_{s_{q_{r}},N}\right)\sum_{d_{q_{1}}=2}^{N}\ldots\sum_{d_{q_{r}}=2}^{N}e_{\eta_{1},\ldots,\eta_{L}}^{\theta_{1},\ldots,\theta_{L}}\right\} 
		\end{split}
	\end{equation}
	where we have introduced, $\forall x\in \{1,\ldots,L\}$
	\begin{equation}
		...
		\begin{align*}
			...
			\sum_{\tau_{1}=1}^{N}\ldots\sum_{\tau_{L}=1}^{N}\left\{e_{\tau_{1},\ldots,\tau_{L}}^{\tau_{1},\ldots,\tau_{L}}\right. \\&+\left. \sum_{b=1}^{L}(-1)^{b}\sum_{q_{1}=1}^{L}\sum_{q_{2}=q_{1}+1}^{L}\ldots\sum_{q_{b}=q_{b-1}+1}^{L}\left(\prod_{r=1}^{b}\delta_{s_{q_{r}},N}\right)\sum_{d_{q_{1}}=2}^{N}\ldots\sum_{d_{q_{r}}=2}^{N}e_{\theta_{1},\ldots,\theta_{L}}^{\eta_{1},\ldots,\eta_{L}}\right\}
			\\&\otimes
			\left\{ \sum_{\tau_{1}=1}^{N}e_{\tau_{L+1}}^{\tau_{L+1}}+(-1)\delta_{\tau_{L+1},N}\sum_{d_{L+1}=2}^{N}e_{1}^{d_{L+1}}\right\}
			\\&=
			\sum_{\tau_{1}=1}^{N}\ldots\sum_{\tau_{L+1}=1}^{N}\left\{e_{\tau_{1},\ldots,\tau_{L+1}}^{\tau_{1},\ldots,\tau_{L+1}}\right. \\&+\left. \sum_{b=1}^{L+1}(-1)^{b}\sum_{q_{1}=1}^{L+1}\sum_{q_{2}=q_{1}+1}^{L+1}\ldots\sum_{q_{b}=q_{b-1}+1}^{L+1}\left(\prod_{r=1}^{b}\delta_{s_{q_{r}},N}\right)\sum_{d_{q_{1}}=2}^{N}\ldots\sum_{d_{q_{b}}=2}^{N}e_{\theta_{1},\ldots,\theta_{L+1}}^{\eta_{1},\ldots,\eta_{L+1}}\right\}
		\end{align*}
		\begin{comment}
		...
		\begin{equation}
			\begin{split}
				...
				\\&
				+\sum_{b=1}^{L}(-1)^{b}\sum_{q_{1}=1}^{L}\sum_{q_{2}=q_{1}+1}^{L}\ldots\sum_{q_{b}=q_{b-1}+1}^{L}\left(\prod_{r=1}^{b}\delta_{s_{q_{r}},N}\right)\sum_{d_{q_{1}}=2}^{N}\ldots\sum_{d_{q_{r}}=2}^{N}\Psi{'}(\eta_{1},\ldots,\eta_{L})
			\end{split}
		\end{equation}
