...
We argue as in the previous proof. For any given $\bm{\tau}\in \Omega^{'}$ we have that 
\begin{align}
...
\end{align}
We introduce the dual configuration $\bm{\xi}(\bm{\tau})=(N,\tau_{1},\ldots,\tau_{L},N)$ where two extra-sites occupied by the species $N$ have been attached to the vector $\bm{\tau}$, transforming it in a vector of dimension $L$ to one of dimension $L+2$. More explicitly, this dual configuration is given by
\begin{equation}
	\begin{cases}
...
\end{equation}
 ...
 \end{equation} 
 where the above average is taken with respect to the non-equilibrium steady state of the process $\bm{\sigma}$. Equation \eqref{interpretation-PsiP} will be useful in the exact formula for the steady state. Given $\bm{\tau}$ as before, we can introduce $\ell=\sum_{x=1}^{L}(1-\delta_{\tau_{x},N})$ the number of occupied sites. Then we denote by $x_{1},\ldots,x_{\ell}$, such that $x_{k}<x_{k+1}$ for all $k\in \{1,\ldots,\ell-1\}$, the coordinates where a particle is present. Finally, we call 
 \begin{equation}
 	\Omega_{\ell}^{'}(\bm{\tau}):=\left\{\bm{\sigma}\in \Omega^{'}\;:\; \sigma_{x_{1}}=\tau_{x_{1}},\ldots,\sigma_{x_{\ell}}=\tau_{x_{\ell}}\right\}
 \end{equation} 
Then, we have that
\begin{equation}
	\Psi(\bm{\tau})=\mu_{x_{1},\ldots,x_{\ell}}(\bm{\tau})
\end{equation}
where 
\begin{equation}
	\mu_{x_{1},\ldots,x_{\ell}}(\bm{\tau})=\sum_{\bm{\sigma}\in \Omega_{\ell}^{'}(\bm{\tau})}\mu(\bm{\sigma})
\end{equation}
is marginal on $m$ sites of the steady state distribution.  
\end{remark}
	...
	\begin{comment}
		...
		\begin{equation}
			...
			\begin{align}
				...
				=&\sum_{b=0}^{L}(-1)^{b}\sum_{1\leq q_{1}<\ldots<q_{b}\leq L}^{L}\left(\prod_{k=1}^{b}\delta_{\tau_{q_{k}},N}\right)\sum_{i_{q_{1}},\ldots i_{q_{b}}=1}^{N-1}\Psi^{'}(\tau_{1},\ldots,\underbrace{i_{q_{1}}}_{q_{1}},\ldots,\underbrace{i_{q_{b}}}_{q_{b}},\ldots,\tau_{L})
			\end{align}
where the notation
		\begin{equation}\label{notation-PSI}
			\Psi^{'}(\tau_{1},\ldots,\underbrace{i_{q_{1}}}_{q_{1}},\ldots,\underbrace{i_{q_{b}}}_{q_{b}},\ldots,\tau_{L})
		...
		\begin{equation}
			...
			\end{cases}
		\end{equation}
		In the second equality we have rewritten the product by introducing the summation over $b$, the coordinates $q_{1},\ldots,q_{b}$ and the occupations variables $i_{q_{1}},\ldots,i_{q_{k}}$. In the last equality we have summed over all possible $\sigma\in \Omega^{'}$. From Section \ref{correlation-section} we recall that the components $\Psi^{'}(\bm{\tau})$ of $|\Psi^{'}\rangle$ can be written as in equation \eqref{interpretation-PsiP}. Thus, we have that  
			\begin{equation}
			\Psi(\bm{\tau})=\sum_{b=0}^{L}(-1)^{b}\sum_{1\leq q_{1}<\ldots<q_{b}\leq L}\left(\prod_{k=1}^{b}\delta_{\tau_{q_{k}},N}\right)\sum_{i_{q_{1}},\ldots,i_{q_{b}}=1}^{N-1}\mathbb{E}_{\mu}\left[\prod_{k=1}^{b}\mathbbm{1}_{\{\sigma_{q_{k}}=i_{q_{k}}\}}\prod_{x=1}^{L}\left(\mathbbm{1}_{\{\sigma_{x}=\tau_{x}\}}\right)^{1-\delta_{\tau_{x},N}}\right]
		\end{equation}
			%	\begin{equation}
...
Formula \eqref{SteadyState-coeff-implicit} can be also obtained without inverting the matrix $S_{1}$, but just using the conservation of probability in the non-equilibrium steady state. Consider a generic $\bm{\tau}\in \Omega^{'}$, then we have 
			...
			\end{align}
		where in the up to last line we exploited the fact that 
