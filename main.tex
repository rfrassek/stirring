...
\end{equation}
In this setting the extra-sites are denoted by $0$ and $L+1$, since connected with sites $1$ and $L$ of the chain respectively. Moreover, the matrix $R_{x}$ reduces to the identity. Therefore, the duality matrix reads as
\begin{equation}
	D=\mathcal{D}_{0}\otimes\prod_{x=1}^{L}\exp{E^{x}}\otimes \mathcal{D}_{L+1}
\end{equation}
and the dual Hamiltonian 
\begin{equation}
	\widetilde{H}=H_{0}+\sum_{x=1}^{L-1}H_{x,x+1}+H_{L+1}
\end{equation}
with the operators \eqref{edgeHamiltonian} and \eqref{siteDualHamiltonian}. In the long-time horizon, the dual process voids the chain, and all particles are eventually absorbed at extra-site $0$ or $L+1$. \\
...
\end{equation}
 This matrix $\mathcal{S}_{1}$ is invertible and we introduce 
...
\end{equation}
The boundary Hamiltonians are \eqref{siteDualHamiltonian} without the extra-site bosonic creation operators $(a^{\dagger})_{a}^{0}$ and $(a^{\dagger})_{a}^{L+1}$, i.e.
\begin{equation}
...
\end{equation}
The boundary Hamiltonians are the following 
\begin{equation}
...
\end{equation}
We obtain the new operators of the MPA that by transforming the original one by $S$
...
\end{equation} 
The vector $|\Psi^{''}\rangle$ is written as:
...
\begin{equation}\label{ABS}
		\begin{split}
			\Psi^{'}(\bm{s})=\sum_{i=1}^{L}\sum_{c_{i}=0}^{1-\delta_{s_{x},1}}\frac{\Gamma(2+L-\sum_{x=1}^{L}c_{x})}{\Gamma(L+2)}\prod_{l=1}^{L}\left(\lambda_{s_{l}}\left(2+L-l-\sum_{j=l}^{L}c_{j}\right)\right)^{c_{l}}\beta_{s_{l}}^{(1-c_{l})(1-\delta_{s_{l},1})}
		\end{split}
	\end{equation} 
and where
...
\end{equation}
\end{comment}
The product $\mathcal{S}_{2}^{-1}|\Psi^{''}\rangle$ gives a vector whose component are denoted by $\Psi^{'}(\bm{s})$. 
...
we obtain 
    \begin{equation}
\Psi^{''}(\bm{s})=\frac{\Gamma(2+\sum_{x=1}^{L}\delta_{s_{x},1})}{\Gamma(L+2)}\prod_{x=1}^{L}\left(\lambda_{s_{x}}\left(2+L-x-\sum_{j=x}^{L}(1-\delta_{s_{x},1})\right)\right)^{1-\delta_{s_{x},1}}
\end{equation}
Introducing
\begin{equation}
	c_{x}=(1-\delta_{s_{x},1}) \qquad \forall x\in \{1,\ldots L\}
\end{equation}
...
\begin{equation}\label{elementsABS}
	\begin{split}
		\Psi^{'}(\mathbf{s})=\sum_{c_{1}=0}^{1-\delta_{s_{1},1}}\ldots\sum_{c_{L}=0}^{1-\delta_{s_{L},1}}\frac{\Gamma(2+L-\sum_{x=1}^{L}c_{x})}{\Gamma(L+2)}\prod_{x=1}^{L}\left(\lambda_{s_{x}}\left(2+L-x-\sum_{j=x}^{L}c_{j}\right)\right)^{c_{i}}\left(\beta_{s_{x}}\right)^{(1-c_{i})(1-\delta_{s_{x},1})}
	\end{split}
\end{equation} 
...
This probability \eqref{Prob-m} $\Psi(\bm{s})$ of a chain of length $m$ with each coordinates $y_{i}$ occupied by $s_{x}\neq 1$, i.e. 
\begin{equation}
...
\begin{equation}\label{Corr-mABS}
	\Psi_{\bm{y}}(s_{1},\ldots,s_{m})=\Psi^{'}_{\bm{y}}(s_{1},\ldots,s_{m})
\end{equation}
Thus, by applying \eqref{ABS_intermediate} we have 
\begin{equation}
	\Psi_{\bm{y}}(s_{1},\ldots,s_{m})=\sum_{i=1}^{L}\sum_{c_{i}=0}^{1}\frac{\Gamma(2+L-\sum_{a=1}^{m}c_{k})}{\Gamma(2+L)}\prod_{k=1}^{m}\lambda_{s_{k}}^{c_{k}}\beta_{s_{k}}^{1-c_{k}}\left(2+L-y_{k}-\sum_{j=k}^{m}c_{j}\right)^{c_{k}}
\end{equation}
...
In \cite{casini2022uphill} it has been defined a process whose average occupation evolution equation are given by two Laplacians and a linear reaction term (mutation). This model consists in a sum of three Markov generators: a stirring, a stirring-mutation and a pure mutation one. For the lattice $\mathbb{Z}$, self-duality has been showed. In view of what have been proved in the previous section of this work, some questions arise naturally:
\begin{enumerate}
	\item Is it possible to extend this last duality result to the boundary-driven case in order to have an absorbing dual process?
...
\end{enumerate}
The aim of this section is to give an answer to these questions. 
\subsection{Linear reaction-diffusion process on a finite graph}
...
\end{cases}
\end{equation}
Observe that $g_{c}(\cdot)$ is surjective and bijective, and thus invertible. \\
Moreover,
\begin{equation}
...
\paragraph{Action on the occupation variable of the edge generator}
...
\paragraph{Action on the occupation variable of the site generator}
...
\begin{equation}\label{actionBoudaryRD}
	...
	\end{split}
\end{equation}
\subsection{Reaction-diffusion process on a chain}
...
\end{equation}
On the occupation variable $n_{d}^{L}$ the action is similar, but with the upper case $1$ replaced by $L$
 In the spirit of \cite{casini2022uphill} we can write he the Master's equation for the evolution of the average density occupation, obtaining a system of $N-1$ difference-differential equation where the discrete Laplacians of all species are coupled and linear reaction terms are present. Therefore, this model is a generalization to $N$ species and to maximal occupation $\nu$ of the one studied in \cite{casini2022uphill}. This answers the first question of the beginning of this section. 
...
\newline
\textbf{Remark}: the generator $\mathcal{L}^{rd}$ has a dual with respect to the same duality function of $\mathcal{L}$ of the multi-species stirring process. Moreover, the dual process has absorbing boundaries while it performs the same edge dynamics of $\mathcal{L}^{rd}$. As we will show this follows from the same $gl(N)$ algebraic structure. 
\subsubsection{Proof of duality relation for reaction-diffusion process}
...
\end{align}
where the duality matrix $D$ is again \eqref{dualityMatrix}. As already pointed out, $H_{x}$ and $\widetilde{H}_{x}$ are the same of the stirring process, while $H_{x,y}^{rd}$ has two more terms $\sum_{c=1}^{N-2}H_{x,y}^{c}$ and $H_{x,y}^{m}$. Therefore, we only need to show that 
\begin{align}
...
\begin{flushright}
	$\square$
\end{flushright}
With this prove we answered to the second and third questions stated at the beginning of this section. 
...
\textbf{Reamrk}: The basis element the we have defined above, can also be thought as proper compositions of the rising and lowering operators of th Heisenberg Lie algebra. Indeed, \begin{equation}\label{creationOperators}
	\begin{cases}
