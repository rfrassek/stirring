...
\begin{equation}
    \bm{\xi}=\left(\xi_{1}^{x},\ldots,\xi_{N}^{x}\right)\otimes (\xi_{1}^{u(x)},\ldots,\xi_{N}^{u(x)} )_{x\in V}
\end{equation}
...
\end{align}
\todo{what happens to $q_N$?} {\color{blue} we used the orthogonality between $\langle q_{1}^{u(x)} ,\ldots,q_{N}^{u(x)}|\xi_{1}^{u(x)},\ldots,q_{N}^{u(x)}\rangle=1$ on the extra-site $u(x)$}
...
\end{equation}
{\color{blue}where the expectation is taken with respect to the non-equilibrium steady state distribution, i.e. it is given in \eqref{exactCorrelations}.}
