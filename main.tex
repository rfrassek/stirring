...
\begin{equation}
	...
	\end{cases}\dt
\end{equation}
This is a new configuration where the occupations $\tau_{q_{1}},\ldots,\tau_{q_{i}}$ at sites $q_{1},\ldots,q_{i}$ are replaced with $N$'s, while the occupations on the other sites are left untouched. The dependence of $\varphi_{x}^{i}(q_{1},\ldots,q_{i})$ with respect to $q_{1},\ldots,q_{i}$ is usually understood for the sake of notation. By considering a fixed $\langle\bm{\tau}|$, and by using \eqref{goodTP-S2} we obtain 
