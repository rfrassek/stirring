...
\begin{equation}\label{notation2}
    ...
    \end{equation}
    Assuming that \eqref{InverseS2} is true for $L$, we prove that it holds for $L+1$
\begin{align*}
...
This is proved by induction. Indeed, for $L=1$ it is trivially true, by \eqref{S1-L1}. We assume that it is true for $L$ and we prove that it holds for $L+1$. 
\begin{align*}
...
\subsection{Linear reaction-diffusion process with coupled discrete Laplacians}
...
where
\begin{equation}
    \mathcal{L}_{x,y}^{rd}=\sigma_{11}\mathcal{L}_{x,y}+\sigma_{12}\sum_{c=1}^{N}\mathcal{L}_{x,y}^{c}+(\Upsilon-2\nu\sigma_{12})\mathcal{L}_{x,y}^{m}
\end{equation}
with $\mathcal{L}_{x,y}$ is the generator the generator \eqref{edgeGenerator} and, for any function $f:\Omega\to \mathbb{R}$
\begin{equation}
...
\begin{equation}
    ...
    k+c-N\quad &\text{if}\quad k+c>N\\
    1\quad &\text{if}\quad k=1
\end{cases}\qquad \forall k\in \{1,\ldots N\}
\end{equation}
and where
\begin{equation}
...
\begin{equation}
	\mathcal{L}_{x}f(\bm{n})=\sum_{a,b=1}^{N}\alpha_{a}^{x}n_{b}^{x}\left(f(\bm{n}-\delta_{b}^{x}+\delta_{a}^{x})-f(\bm{n})\right)
\end{equation}
where the boundary parameters are defined as
\begin{equation}
	\begin{split}
	\alpha_{a}^{x}&=\sigma_{11}\nu\rho_{a}^{x}+\sigma_{12}\nu\sum_{b=2\,:\,b\neq a}^{N}\rho_{b}^{x}\qquad \text{if} \quad k\in \{2,\ldots,N\}\\
	\alpha_{1}^{x}&=\nu\left(\sigma_{11}+(N-1)\sigma_{12}\right)-\sum_{a=2}^{N}\alpha_{a}^{x}
	\end{split}
\end{equation}
The action on the occupation variable $n_{d}^{x}$ of particle type $d\in \{2,\ldots,N\}$ at site $x\in V $
is the following
\begin{equation}\label{actionGraphRD}
\begin{split}
    \mathcal{L}_{x,y}^{rd}n_{d}^{x}&=\nu \sigma_{11}(n_{d}^{y}-n_{d}^{x})+\nu\sigma_{12}\sum_{c=2\,:	,c\neq d}^{N}(n_{c}^{y}-n_{d}^{x})+(\Upsilon-2\nu\sigma_{12})\sum_{c=2}^{N}(n_{c}^{x}-n_{\alpha}^{x})
\end{split}
\end{equation}
Indeed
\begin{equation}
    \begin{split}
\mathcal{L}_{x,y}^{c}n_{a}^{x}&=\sum_{a,b=1}^{N}n_{a}^{x}n_{b}^{y}\left(n_{d}^{x}-\delta_{a}^{x}+\delta_{g_{c}(b)}^{x}+\delta_{g_{c}(a)}^{y}-\delta_{b}^{y}-n_{\alpha}^{x}\right)=\sum_{a=1}^{N}n_{a}^{x}n_{q}^{y}-\sum_{\ell=1}^{N}n_{\alpha}^{x}n_{\ell}^{y}
\\&=
\sum_{a=2}^{N}n_{a}^{x}n_{q}^{y}-\left(\nu-\sum_{i=2}^{N}n_{i}^{x}\right)n_{q}^{y}-\sum_{\ell=2}^{N}n_{\alpha}^{x}n_{\ell}^{y}-n_{\alpha}^{x}\left(\nu-\sum_{i=2}^{N}n_{i}^{y}\right)
\\&=
\nu(n_{q}^{y}-n_{\alpha}^{x})
    \end{split}
\end{equation}
where we call $q\in \{2,\ldots,N\}$ such that $g_{i}(q)=\alpha$ and $\ell\in \{2,\ldots,N\}$ such that $g_{i}(\ell)\neq \alpha$. Summing over all indices $c\in\{2,\ldots,N\}\,:\,c\neq d$ we obtain \eqref{actionGraphRD}.\\
As a consequence, considering \eqref{actionGraphRD} on the geometry of a chain of finite length $L\in \mathbb{N}$, the action of the generator \eqref{RDGenerator} on the occupation variable $n_{\alpha}^{x}$ gives
\begin{equation}\label{DifferenceEquation}
\mathcal{L}^{rd}n_{d}^{x}=\nu\sigma_{11}\Delta_{L}n_{d}^{x}+\nu\sigma_{12}\sum_{c=2\,:\,l\neq d}^{N}\Delta_{L}n_{c}^{x}+\Upsilon\sum_{c=2}^{N}(n_{c}^{x}-n_{\alpha}^{x})
\end{equation}
where with the symbol $\Delta_{N}$ we denote the discrete Laplacian operator on the chain, i.e. $\Delta_{L}n_{a}^{x}=n_{a}^{x+1}+n_{a}^{x-1}-2n_{a}^{x}$, $\forall x\in \{2,\ldots,L-1\}$ and $\forall a\in \{2,\ldots,N\}$. In the spirit of \cite{casini2022uphill} we can write he the Master's equation for the evolution of the average density occupation, obtaining a system of $N-1$ difference differential equation where the discrete Laplacian of all species are coupled and linear reaction term are present. 
\subsection{Reversible measure} 
The process defined by generator \eqref{RDGenerator} has reversible product measure when all the reservoirs have the same parameter for each site and each species, i.e. $\alpha_{a}^{x}=\alpha$, $\forall a\in \{1,\ldots,N\}$ and $x\in V$. This measure is given by
\begin{equation}
...
\begin{equation}
    H^{rd}=\sum_{x,y\in \mathcal{E}}\omega_{x,y}\left(\sigma_{11}H_{x,y}+\sigma_{12}\sum_{c=1}^{N}H_{x,y}^{c}+(\Upsilon-2\sigma_{12})H_{x,y}^{m}\right)+\sum_{x\in V}\Gamma_{x}H_{x}
\end{equation}
