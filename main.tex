...
For the single species case, the exclusion process with open boundaries has been of crucial importance in our understanding of the structural properties of non-equilibrium state states, such as long-range correlations and non-local density large deviation functions. The results available for the exclusion process are rooted in the algebraic properties of the corresponding integrable quantum spin chain, i.e. the XXX Heisenberg Hamiltonian with $\mathfrak{su}(2)$ spins of value 1/2.
In this paper, we investigate the multispecies analog of the exclusion process in the boundary-driven context, i.e. the stirring process with open boundaries. The bulk part of the model with $N$ species is associated with the integrable Hamiltonian having  $\mathfrak{gl}(N)$ symmetry.

Our strategy to solve for the non-equilibrium steady state is of a probabilistic nature and relies on the use of duality. This approach, starting from the pioneering approach of [Presutti] and [Spohn] has been substantially developed in recent years to study stochastic processes in the boundary-driven set-up. The boundary-driven process is mapped to
a dual process with absorbing extra sites and the problem of describing the stationary state
...
For a recent example see [Frassek,Giardinà].

The remainder of this paper is organized as follows. In Section \ref{} we define the open multispecies stirring
process and provide its Lie algebraic description. In Section \ref{} we prove duality (Theorem \ref{}) 
...
system has a general spatial structure described by a graph. In Section \ref{} we obtain a closed formula 
for the stationary correlations (Theorem \ref{}) for the case of a one-dimensional chain with at most one 
particle per site, which is driven to steady-state by two reservoirs acting at the left and right boundaries. 
Here the computation of the absorption probabilities relies on the previously formulated Matrix Product Ansatz,
which is in fact also simplified by the use of duality. Finally in Section \ref{} we discuss a few extensions 
and generalizations of the model, including instantaneous thermalization limits, ......
...
Acknowledgments: GNFM, PRIN, FAR UNIMORE,...
...
\begin{remark}\label{Remark-extension-graph-nu}
	...
	Thus we chose to present them in such a context.
\end{remark}
...
Observing that $c_{x}(1-\delta_{\tau_{x},N}) =c_{x}-\delta_{\tau_{x},N}$, we finally have
...
We argue similarly as in the Section \ref{correlation-section}. For any $\bm{\tau}\in \Omega^{'}$ we have that 
\begin{align}
...
\end{equation}
 ...
 \end{equation} 
 This last equation will be used in Section \ref{appB}. {\color{blue}Given $\bm{\tau}$ as before, we can introduce the number of occupied sites $\ell=\sum_{x=1}^{L}(1-\delta_{\tau_{x},N})$. We denote by $x_{1},\ldots,x_{\ell}$, such that $x_{k}<x_{k+1}$ for all $k\in \{1,\ldots,\ell-1\}$, the coordinates where a particle of any (non-empty) type is present. Finally, we call 
 \begin{equation}
...
By the knowledge of the closed formula for correlations \eqref{exactCorrelations}, we find the probability mass-function in the non-equilibrium steady state by using Corollary \ref{Corollary-NESS-ABS}. Therefore, the vector $|\Psi\rangle$ is also determined. For the sake of completeness we show here that $|\Psi\rangle$ can be equivalently determined by inverting the similarity transformation $S_{1}$, using $|\Psi\rangle=S_{1}^{-1}|\Psi^{'}\rangle$ (cf. \eqref{S1-Inverse}). 
...
\begin{comment}
	...
	\begin{equation}
		...
		\begin{align}\label{implicit-PSI-PSIp}
			...
			=&\sum_{b=0}^{L}(-1)^{b}\sum_{1\leq q_{1}<\ldots<q_{b}\leq L}\sum_{\bm{\sigma}\in \Omega^{'}}\Psi^{'}(\bm{\sigma})\left(\prod_{x=1\,:\,x\notin\{q_{1},\ldots,q_{b}\}}^{L}\delta_{\tau_{x},Y_x}\right)\left(\prod_{x\in \{q_{1},\ldots,q_{b}\}}\delta_{\tau_{x},N}\sum_{a_{x}=1}^{N-1}\delta_{\sigma_{x},a_{x}}\right)\nonumber\\
...
\begin{remark}[Equilibrium case]
	...
	\end{equation}
As it has to, this is the probability of the configuration $\bm{\tau}$ under the reversible measure \eqref{reversibleMeasure}, with site marginals distributed as $\text{Multinomial}(1,\alpha_{1},\ldots,\alpha_{N})$. 
\end{remark}
\begin{comment}
...
\end{equation} 
\textbf{Proof of \eqref{correlations-SSEP}:} Starting from \eqref{exactCorrelations} we exploit indistinguishability of particles and we introduce $q=\sum_{k=1}^{m}c_{x_{k}}$. There are many values of $c$'s that give the same $q$, therefore we define $q$ %labels denoted by $\ell_{1},\ldots,\ell_{q}\in \{1,\ldots,m\}$ such that  
...
Furthermore, sing the properties of the factorials, we have that 
...
\begin{enumerate}
	\item \textit{Arbitrary correlations}: exploiting duality, the steady state correlations can be expressed in terms of absorption probabilities of the dual particles. These probability are hard to find in this partial exclusion situation, however correlation can be formally written. Section \ref{Subsection-ss-nonI}. \textbf{[Do we need this or it is enough Remark \ref{Remark-extension-graph-nu}?]}
