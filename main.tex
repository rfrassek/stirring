...
\begin{equation}
	D(\bm{n},\bm{\xi})=\prod_{a=1}^{N-1}\left(\rho_{a}^{L}\right)^{\xi_{a}^{0}}\prod_{x=1}^{L}\frac{(\nu-\sum_{a=1}^{N-1}\xi_{a}^{x})!}{\nu!}\prod_{a=1}^{N-1}\frac{n_{a}^{x}!}{(n_{a}^{x}-\xi_{a}^{x})!}\prod_{a=1}^{N-1}\left(\rho_{a}^{R}\right)^{\xi_{a}^{L+1}}\dt
\end{equation}
...
\begin{align}
	...
	\left(\mathcal{L}_{\text{left}}^{TH}D(\cdot,\bm{\xi})\right)(\bm{n})&=\left(\widetilde{\mathcal{L}}_{\text{left}}^{TH}D(\bm{n},\cdot)\right)(\bm{\xi})
\end{align}
...
\end{align}
We consider the first addend in the curly bracket of the \eqref{ProvaBordoTH}. We add and remove $\xi_{a}^{1}$ at the exponent of $\left(\frac{\alpha_{a}}{|\alpha|}\right)$ and then we multiplying by $\prod_{\ell=1}^{N-1}\left(\frac{\alpha_{N}}{|\alpha|}\right)^{-r_{\ell}^{1}+r_{\ell}^{1}}$. Thus, we have
...
\end{equation}
The only terms that survive are: in one case the ones with $a=d$ and $b\neq d$, giving a contribution $-n_{d}^{x}n_{b}^{x+1}$; in the other case the ones with $a\neq d$ and $b=d$, giving a contribution $n_{a}^{x}n_{d}^{x+1}$. Thus we obtain 
\begin{align*}
	\mathcal{L}_{x,x+1}n_{d}^{x}=&\sum_{a=1\,:\,a\neq d}^{N}n_{a}^{x}n_{d}^{x+1}-\sum_{b=1\,:\,b\neq d}^{N}n_{d}^{x}n_{b}^{x+1}\\
	=&\sum_{a=1\,:\,a\neq d}^{N-1}n_{a}^{x}n_{d}^{x+1}+(\nu-\sum_{i=1}^{N-1}n_{i}^{x})n_{d}^{x+1}-\sum_{b=1\,:\,b\neq d}^{N-1}n_{d}^{x}n_{b}^{x+1}-n_{d}^{x}(\nu-\sum_{i=1}^{N-1}n_{i}^{x+1})\\
