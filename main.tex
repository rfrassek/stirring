...
\textbf{Proof of formula \eqref{psiII-SSEP}:} We specialize equation \eqref{resulEsteady} to the colour-blind case. \todo{can we just say $N=2$ as we discussed several times now}We introduce $q=\sum_{x=1}^{L}\delta_{\tau_{x},1}$. For each $q\in \{1,\ldots,L\}$ we call $1\leq\ell_{1}<\ldots,\ell_{q}\leq L$ the coordinates where particles of type $1$ are present. Thus, arguing as in the proof of \eqref{correlations-SSEP} and using \eqref{Gamma-product} we have that 
\begin{align}
...
In the colour-blind case we have that \todo{can we just say $N=2$ as we discussed several times now}
...
\begin{equation}\label{boundaryThermalized}
	\begin{split}
		...
		\end{split}
	\end{equation}
	\todo{is the last term in the product?}
...
\end{equation}
\todo{Why not merge sum or is there a typo?}
...
\end{equation}
\todo{shall we remove all site indices given that there is the tensor product?}
\begin{flushright}
	$\square$
\end{flushright}
...
\todo{Why do we never refer to this appendix?}
