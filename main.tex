...
\langle N,\ldots,N,\underbrace{a_1}_{x_{1}},N,\ldots,N,\underbrace{a_m}_{x_{m}},N,\ldots,N|\Psi^{'}\rangle\dt
 \end{split}
\end{equation} 
{\color{blue}Here the notation means that the entries $a_k\neq N$ in \eqref{eq:EE} are at position $x_k$ for $k\in\{1,\ldots,m\}$.} 
\end{remark}

\todo{ok? adopt notation?}
...
\end{equation}
		...
		\begin{align}\label{implicit-PSI-PSIp}
			...
			=&\sum_{b=0}^{L}(-1)^{b}\sum_{1\leq q_{1}<\ldots<q_{b}\leq L}^{L}\left(\prod_{k=1}^{b}\delta_{\tau_{q_{k}},N}\right)\sum_{a_{1},\ldots a_{b}=1}^{N-1}\Psi^{'}(\tau_{1},\ldots,\underbrace{a_{1}}_{q_{1}},\ldots,\underbrace{a_{b}}_{q_{b}},\ldots,\tau_{L})
		\end{align}
