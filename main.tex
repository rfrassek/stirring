...
\begin{equation}
    ...
    \end{cases}
\end{equation}
\textbf{Remark}: In case of $N=2$ we can introduce $q=\sum_{x=1}^{L}\delta_{\tau_{x},1}$. Moreover, we call $\ell_{1},\ldots,\ell_{q}$ the coordinates where particles of type $1$ are present. Thus, the coefficient of the above formula can be written as
\begin{align}
&\frac{\Gamma\left(2+\sum_{x=1}^{L}\delta_{\tau_{x},N}\right)}{\Gamma\left(2+L\right)}\left(\prod_{x=1}^{L}\left[\lambda_{\tau_{x}}\left(1+\sum_{j=x}^{L}\delta_{\tau_{j},2}\right)\right]^{1-\delta_{\tau_{x},N}}\right)%=\lambda_{1}^{q}\prod_{x=1}^{L}\left(2+L-y_{x}-\sum_{j=x}^{L}\delta_{\tau_{j},1}\right)^{\delta_{\tau_{x},1}}
=\lambda_{1}^{q}\prod_{k=1}^{q}\frac{\left(1+L-\ell_{k}-\sum_{j=k+1}^{q}\delta_{\tau_{x_{j}},1}\right)}{(2+L-k)}
\end{align}
Moreover, for each $q\in \{1,\ldots,L\}$ using the coordinates $1\leq\ell_{1}<\ldots,\ell_{q}\leq L$, calling $\bm{\ell}=(\ell_{1},\ldots,\ell_{q})$ and, by varying $q$, we have a one-to-one mapping with $\bm{\tau}$. Therefore, we can write 
\begin{equation}
...
\begin{equation}\label{ABS}
		\begin{split}
			\Psi^{'}(\bm{\tau})=\sum_{c_{1}=0}^{1-\delta_{\tau_{1},N}}\ldots\sum_{c_{L}=0}^{1-\delta_{\tau_{L},N}}\frac{\Gamma(2+L-\sum_{z=1}^{L}c_{z})}{\Gamma(L+2)}\prod_{x=1}^{L}\left(\lambda_{\tau_{x}}\left(2+L-x-\sum_{x=x}^{L}c_{j}\right)\right)^{c_{x}}\beta_{\tau_{y}}^{(1-c_{x})(1-\delta_{\tau_{x},N})}
		\end{split}
	\end{equation} 
...
\begin{flushright}
    $\square$
\end{flushright}
\textbf{Remark}: In the case of $N=2$ we can argue similarly as we did in the Remark of Subsection \ref{subsectionSSHsec}. If we stop distinguishing types of non-empty particles we have that \eqref{ABS} reads
\begin{align}
	\Psi^{'}(\bm{\tau})=&\sum_{q=0}^{L}(\alpha-\beta)^{q}\beta^{L-q}\frac{\Gamma(L+2-q)}{\Gamma(L+2)}\sum_{c_{1}=0}^{1}\ldots\sum_{c_{L}=0}^{1}\mathbbm{1}_{\{c_{1}+\cdots+c_{L}=q\}}
	\\&\prod_{k=1}^{q}\left(L+2-x_{k}-\sum_{i=k}^{L}c_{i}\right)\dt
\end{align}
where $x_{k}$ denotes the position of the particle $k$ among $q$. For any fixed $q$ we must consider all possible values of $(c_{i})_{i\in\{1,\ldots,m\}}$ such that its sum is equal to $q$. For each of these $(c_{i})_{i\in\{1,\ldots,m\}}$ we add a term given by  
\begin{equation}
	\prod_{k=1}^{L}\left(L+2-x_{k}-\sum_{i=k}^{L}c_{i}\right)^{c_{k}}\dt
\end{equation}
We can introduce $\ell_{1},\ldots,\ell_{q}\in\{1,\ldots,m\}$ such that $\ell_{1}<\ell_{2}<\cdots<\ell_{q}$ and write 
\begin{equation}
	\sum_{c_{1}=0}^{1}\ldots\sum_{c_{L}=0}^{1}\mathbbm{1}_{\{c_{1}+\cdots+c_{L}=q\}}\prod_{k=1}^{L}\left(L+2-x_{k}-\sum_{i=k}^{L}c_{i}\right)^{c_{k}}=\sum_{1\leq \ell_{1}<\ldots<\ell_{q}\leq L}\prod_{k=1}^{q}\left(L+2-\ell_{k}-(q-k+1)\right)\dt
\end{equation}
...
\Psi^{'}(\bm{\ell},q)=\sum_{q=0}^{L}(\alpha-\beta)^{q}\beta^{L-q}\sum_{1\leq \ell_{1}<\ldots<\ell_{q}\leq L}\prod_{k=1}^{q}\frac{\left(L+1-\ell_{k}-q+k\right)}{(2+L-k)}\dt
\end{equation}
...
\begin{comment} introducing $q=\sum_{j=1}^{m}c_{i}$ in the colour-blind situation \eqref{exactCorrelations} becomes
\begin{align}
