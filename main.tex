...
{\color{blue}\textbf{Remark}: 
    ...
    \begin{flushright}
        $\square$
    \end{flushright}
The process described by the generator \eqref{Generator} is reversiblewith respect to the homogeneus product measure \begin{equation}\label{reversibleMeasure}
\mu_{rev}=\bigotimes_{x\in V}\mu^{rev}_{x}
\end{equation}
...
By the Lie algebra above we describe the process characterized by the generator \eqref{Generator}. The state space \eqref{stateSpace} is the vector space with basis elements 
...
We can write this Hamiltonian as a function of the coproduct of the second Casimir
\begin{equation}
    C_{2}=\sum_{a,b=1}^{N}E_{ab}E_{ba}
\end{equation}
that acts diagonally as $C_{2}|\mathbf{n}\rangle=\twoj(\twoj+N)|\mathbf{n}\rangle$. \\
...
We now state our duality result. The process $|n\rangle$ with Hamiltonian \eqref{} is dual in the sense of \eqref{DualityRelation}, with respect to a process $|\xi\rangle$ defined on the enlarged state space
\begin{equation}
    \widetilde{\Omega}:=\Omega\times \mathbb{N}_{0}^{V}
\end{equation}
with elements
\begin{equation}
    |\xi\rangle=\bigotimes_{x\in V}\left(|r_{1}^{x},\ldots,r_{N}^{x}\rangle\otimes |m_{2}^{\widetilde{x}},\ldots,m_{N}^{\widetilde{x}}\rangle\right)
\end{equation}
...
$(a^{\dagger})_{k}^{\widetilde{x}}$ as a copy of $a^{\dagger}$ acting on the extra-site $\widetilde{x}$ and on the species $k\in\{2,\ldots,N\}$. \\
The Hamiltonians \eqref{Hamiltonian} and \eqref{DualHamiltonian} are dual with respect to the duality matrix $D$ defined as 
\begin{equation}
...
\end{equation}
%The key idea behind the proof of \eqref{edgeDualRealtion} is that it is possible to write the Hamiltonian \eqref{} as a function of the second Casimir of the $gl(N)$ Lie algebra. Finding a symmetry $S$ of this element, will imply of having a symmetry of the Hamiltonian. By the reversibility of the process one can make $S$ act on a proper matrix (called cheap duality) and obtain 
\paragraph{Proof of \eqref{edgeDualRealtion}}
The proof of the edge duality relation relies on two facts: first the existence of a diagonal matrix, arising from the reversible measure, that is already a duality for the Hamiltonian (in literature called 'cheap' duality matrix, see \cite{}); second the possibility of finding a proper symmetry of the Hamiltonian.  \\We first introduce the cheap duality matrix 
...
\end{equation}
We now look for a symmetry of the Hamiltonian. As shown in \eqref{}, it is possible to write the Hamiltonian \eqref{edgeHamiltonian} as a function of the coproduct of the second Casimir of $gl(N)$. We say that a matrix $A\in gl(N)$ is symmetry of the Casimir $C_{2}$ if
\begin{equation}
    [A,C_{2}]=0
\end{equation}
...
\end{equation}
 $C_{2}$ is a central element of $gl(N)$, then it commutes with every element of the algebra. 
...
\paragraph{Proof of \eqref{siteDualRelation}} The proof of the site duality relations relies on the Hadamard Lemma that, $\forall E,A \in gl(N)$, reads as 
...
\begin{equation}
    \begin{split}
H_{x}^{T}=\sum_{k,l=0}^{N}\alpha_{k}^{x}\left(E_{kl}^{x}-E_{ll}^{x}\right)^{T}=R_{x}\sum_{k,l=0}^{N}\alpha_{k}^{x}\left(E_{lk}^{x}-E_{ll}^{x}\right)R_{x}^{-1}
    \end{split}
\end{equation}
we multiply both sides for $R_{x}\exp{(E^{x})}$
\begin{equation}
    H_{x}^{T}R_{x}\exp{(E^{x})}=R_{x}\sum_{k,\ell=0}^{N}\alpha_{k}^{x}\left(E_{lk}^{x}-E_{ll}^{x}\right)R_{x}^{-1}R_{x}\exp{(E^{x})}=R_{x}\sum_{k,l=0}^{N}\alpha_{k}^{x}\left(E_{lk}^{x}-E_{ll}^{x}\right)\exp{(E^{x})}
\end{equation}
we insert the identity $I=\exp{(E^{x})}\exp{(-E^{x})}$ in the right hand side
\begin{equation}\label{intermediateTransposeSite}
H_{x}^{T}R_{x}\exp{(E^{x})}=R_{x}\exp{(E^{x})}\exp{(-E^{x})}\sum_{k,l=0}^{N}\alpha_{k}^{x}\left(E_{lk}^{x}-E_{ll}^{x}\right)\exp{(E^{x})}
\end{equation}
we apply the Hadamard's formula \eqref{HadamardFormula} to 
\begin{equation}
    \exp{(-E^{x})}\sum_{k,l=0}^{N}\alpha_{k}^{x}\left(E_{lk}^{x}-E_{ll}^{x}\right)\exp{(E^{x})}
\end{equation}
We compute the following quantities by the commutation relations \eqref{}:
\begin{itemize}
    \item for all $k\neq 1$ 
\begin{equation}
    \begin{split}
        \exp{(-E^{x})}E_{k1}^{x}\exp{(E^{x})}=E_{k1}^{x},
    \end{split}
\end{equation}
\begin{equation}
    \begin{split}
    \exp{(-E^{x})}E_{kk}^{x}\exp{(E^{x})}=E_{kk}^{x}+E_{k1}^{x}
    \end{split}
\end{equation}
and
\begin{equation}
\begin{split}
    \exp{(-E^{x})}E_{1k}^{x}\exp{(E^{x})}=E_{1k}^{x}+E_{11}^{x}-\sum_{a=2}^{N}\left(E_{a1}^{x}+E_{ak}^{x}\right)
\end{split}
\end{equation}
Indeed we have the following commutators at the first order:
\begin{align*}
\sum_{a=2}^{N}\left[E_{a1}^{x},E_{kk}\right]=\sum_{a=2}^{N}\left(E_{ak}^{x}\delta_{k1}-E_{k1}^{x}\delta_{ak}\right)
\end{align*}
and 
\begin{align*}
\sum_{a=2}^{N}\left[E_{a1}^{x},E_{1k}\right]=\sum_{a=2}^{N}\left(E_{ak}^{x}\delta_{11}-E_{11}^{x}\delta_{ak}\right)=\sum_{a=2}^{N}E_{ak}^{x}-E_{11}^{x};
\end{align*}
and the following commutators at the second order:
\begin{align*}
&\sum_{b=2}^{N}\sum_{a=2}^{N}\left[E_{b1}^{x},E_{ak}^{x}\right]-\sum_{b=2}^{N}\left[E_{b1}^{x},E_{11}^{x}\right]=\sum_{a,b=2}^{N}\left(E_{bk}^{x}\delta_{a1}-E_{a1}^{x}\delta_{bk}\right)-\sum_{b=2}^{N}\left(E_{b1}\delta_{11}-E_{11}\delta_{b1}\right)\\=&-2\sum_{a=2}^{N}E_{a1}^{x};
\end{align*}
from the third on-wards all commutators zero. 
\item for all $k,l\neq 1$
\begin{equation}
        \begin{split}
    \exp{(-E^{x})}E_{lk}^{x}\exp{(E^{x})}=E_{kl}^{x}+E_{l1}^{x}
    \end{split}
\end{equation}
Indeed the first commutator gives
\begin{align*}
\sum_{a=2}^{N}\left[E_{a1}^{x},E_{lk}\right]=\sum_{a=2}^{N}\left(E_{ak}^{x}\delta_{l1}-E_{l1}^{x}\delta_{ak}\right)=-E_{l1}^{x};
\end{align*}
...
indeed the first commutator gives
\begin{align*}
\sum_{a=2}^{N}\left[E_{a1}^{x},E_{lk}\right]=\sum_{a=2}^{N}\left(E_{ak}^{x}\delta_{11}-E_{l1}^{x}\delta_{a1}\right)=\sum_{a=2}^{N}E_{a1}^{x};
\end{align*}
and the from the second commutator on-wards they are all zero.
\end{itemize}
Therefore we have
\begin{equation}
\begin{split}
    &\exp{(-E^{x})}\sum_{k=1}^{N}\sum_{l=1}^{N}\alpha_{k}^{x}\left(E_{lk}^{x}-E_{ll}^{x}\right)\exp{(E^{x})}\\&=
     \exp{(-E^{x})}\sum_{k=1}^{N}\alpha_{k}^{x}\left(E_{1k}^{x}-E_{11}^{x}\right)\exp{(E^{x})}
     + \exp{(-E^{x})}\sum_{k=1}^{N}\sum_{l=2}^{N}\alpha_{k}^{x}\left(E_{lk}^{x}-E_{ll}^{x}\right)\exp{(E^{x})}
     \\&=
     \sum_{k=1}^{N}\alpha_{k}^{x}\left(E_{1k}+E_{11}^{x}-\sum_{a=2}^{N}(E_{jk}^{x}+E_{l1}^{x})-E_{11}^{x}+\sum_{j=2}^{N}E_{j1}^{x}\right)+\sum_{k=1}^{N}\sum_{l=2}^{N}\left(E_{lk}^{x}+E_{\ell 1}^{x}-E_{\ell\ell}^{x}-E_{l1}^{x}\right)
     \\&=
     \sum_{k=1}^{N}\alpha_{k}^{x}\left(E_{1k}^{x}-\sum_{j=2}^{N}E_{jj}^{x}\right)
    \end{split}
\end{equation}
Thus, multiplying and dividing by $\sum_{i=1}^{N}\alpha_{i}^{x}$, we rewrite \eqref{intermediateTransposeSite} as
\begin{equation}\label{siteHadamardI}
H_{x}^{T}R_{x}\exp{(E^{x})}=R_{x}\exp{(E^{x})}\sum_{i=1}^{N}\alpha_{i}^{x}\sum_{k=1}^{N}\frac{\alpha_{k}^{x}}{\sum_{i=1}^{N}\alpha_{i}^{x}}\left(E_{1k}^{x}-\sum_{j=2}^{N}E_{jj}\right)
\end{equation}
We multiply both sides of \eqref{siteHadamard} by 
\begin{equation}
...
\\=&
D_{x}\sum_{m_{2}^{\widetilde{x}},\ldots,m_{N}^{\widetilde{x}}=0}^{\infty}\prod_{k=2}^{N}\left(\frac{\alpha_{k}^{x}}{\sum_{i=1}^{N}\alpha_{i}^{x}}\right)^{m_{k}^{\widetilde{x}}}\langle m_{2}^{\widetilde{x}},\ldots,m_{N}^{\widetilde{x}}|\sum_{i=1}^{N}\alpha_{i}^{x}\sum_{k=1}^{N}\frac{\alpha_{k}^{x}}{\sum_{i=1}^{N}\alpha_{i}^{x}}\left(E_{1k}^{x}-\sum_{j=2}^{N}E_{jj}\right)
    \end{split}
\end{equation}
Recalling the action of the bosonic creation operator acting at site $\widetilde{x}$ and on the species $k\in \{2,\ldots\}$ we have that 
...
\sum_{m_{2}^{\widetilde{x}},\ldots,m_{N}^{\widetilde{x}}=0}^{\infty}\prod_{k=2}^{N}\left(\frac{\alpha_{k}^{x}}{\sum_{i=1}^{N}\alpha_{i}^{x}}\right)^{m_{k}^{\widetilde{x}}}\langle m_{2}^{\widetilde{x}},\ldots,m_{k}^{\widetilde{x}},\ldots,m_{N}^{\widetilde{x}}|(a^{\dagger})_{k}^{\widetilde{x}}E_{1k}^{x}
    \end{split}
\end{equation}
where we performed a change of summation variable. Therefore, inserting this in \eqref{siteHadamardII} we obtain 
\begin{equation}
    \begin{split}
     &H_{x,y}^{T}D_{x}\sum_{m_{2}^{\widetilde{x}},\ldots,m_{N}^{\widetilde{x}}=0}^{\infty}\prod_{k=2}^{N}\left(\frac{\alpha_{k}^{x}}{\sum_{i=1}^{N}\alpha_{i}^{x}}\right)^{m_{k}^{\widetilde{x}}}\langle m_{2}^{\widetilde{x}},\ldots,m_{N}^{\widetilde{x}}|
\\=&
D_{x}\sum_{m_{2}^{\widetilde{x}},\ldots,m_{N}^{\widetilde{x}}=0}^{\infty}\prod_{k=2}^{N}\left(\frac{\alpha_{k}^{x}}{\sum_{i=1}^{N}\alpha_{i}^{x}}\right)^{m_{k}^{\widetilde{x}}}\langle m_{2}^{\widetilde{x}},\ldots,m_{N}^{\widetilde{x}}|\sum_{i=1}^{N}\alpha_{i}^{x}\sum_{k=1}^{N}\left((a^{\dagger})_{k}^{\widetilde{x}}E_{1k}^{x}-\sum_{j=2}^{N}E_{jj}\right)   
    \end{split}
\end{equation}
Since the duality matrix \eqref{dualityMatrix} is tensor product over sites, this implies \eqref{siteDualRelation}. 
\begin{flushright}
$\square$
\end{flushright}
\textbf{Remark}:
The duality result can be state also in terms of infinitesimal generators \eqref{Generator}. In this case, the dual process, is denoted by $(\xi(t))_{t\geq 0}$, where for each $t\geq 0$ and for each $x\in V$ we denote
\begin{equation}
\xi^{x}(t)=\left(r_{1}^{x}(t),\ldots,r_{N}^{x}(t),m_{2}^{\widetilde{x}}(t),\ldots,m_{N}^{\widetilde{x}}(t)\right)
\end{equation}
where we denote $r_{k}^{x}$ the occupation of dual particle of type $k\in \{1,\ldots,N\}$ at site $x$ and with $m_{l}^{\widetilde{x}}$ the occupation of dual particle of type $k\in \{1,\ldots,N\}$ at extra-site $\widetilde{x}$ connected to $x\in V$. This dual process has values in the enlarged state space
\begin{equation}
...
\begin{equation}
    \widetilde{\mathcal{L}}_{x}f(\xi)=\sum_{i=1}^{N}\alpha_{i}^{x}\sum_{k=2}^{N}\xi_{k}^{x}\left(f(\xi-\delta_{k}^{x}+\delta_{1}^{x}+\delta_{k}^{\widetilde{x}})-f(\xi)\right)
\end{equation}
The duality relation \eqref{DualityRelation} reads
\begin{equation}\label{dualityGenerators}
    \left(\mathcal{L}D(\cdot,\xi)\right)(n)=\left(\widetilde{\mathcal{L}}D(n,\cdot)\right)(\xi)
\end{equation}
where 
\begin{equation}\label{dualityElements}
	D(n,\xi)=\langle n|D|\xi\rangle=\prod_{x\in V}\left(\frac{(2j-\sum_{k=2}^{N}r_{k}^{x})!}{\nu!}\prod_{k=2}^{N}\frac{n_{k}^{x}!}{(n_{k}^{x}-r_{k}^{x})!}\left(\frac{\alpha_{k}}{\sum_{i=1}^{N}\alpha_{1}}\right)^{m_{k}^{\varepsilon(x)}}\right)
\end{equation}
...
\begin{align*}
	 &\langle n^{x}|D_{x}\otimes D_{\widetilde{x}}|\xi^{x}\rangle\\&=\langle n_{1}^{x},\ldots,n_{N}^{x}|\sum_{m_{2}^{\widetilde{x}},\ldots,m_{N}^{\widetilde{x}}=0}^{\infty}\prod_{k=2}^{N}\left(\frac{\alpha_{k}}{\sum_{i=1}^{N}\alpha_{i}^{x}}\right)^{m_{k}^{\varepsilon(x)}}\langle m_{2},\ldots,m_{N}|_{x}
...
\begin{equation}
    \mathcal{L}_{x,u}^{i}f(n)=\sum_{k,l=1}^{N}n_{k}^{x}n_{l}^{y}\left(f(n-\delta_{k}^{x}+\delta_{g_{i}(l)}^{x}+\delta_{g_{i}(k)}^{y}-\delta_{l}^{y})-f(n)\right)
\end{equation}
with the mapping 
\begin{equation}
    g_{i}(k)=\begin{cases}
...
and where
\begin{equation}
    \mathcal{L}_{x,y}^{m}f(n)=\sum_{l,k=0}^{N}n_{k}^{x}\left(f(n-\delta_{k}^{x}+\delta_{l}^{x})-f(n)\right)
\end{equation}
...
\end{equation}
where with the symbol $\Delta_{N}$ we denote the discrete Laplacian operator on the chain, i.e. $\Delta_{L}n_{k}^{x}=n_{k}^{x+1}+n_{k}^{x-1}-2n_{k}^{x}$, $\forall x\in \{2,\ldots,L-1\}$ and $\forall k\in \{2,\ldots,N\}$. In the spirit of \cite{} we can write he the Master's equation for the evolution of the average density occupation, obtaining a system of $N-1$ difference differential equation where the discrete Laplacian of all species are coupled and linear reaction term are present. \\ 
...
\end{equation}
Therefore, following a proof similar to the one in subsection \ref{subsectionDualityProof}, \eqref{DualityRelationRD} can be proved. 
