...
\begin{equation}
    \begin{split}
H_{x}^{T}=\sum_{k,l=0}^{N}\alpha_{k}^{x}\left(E_{k\ell}^{x}-E_{\ell\ell}^{x}\right)^{T}=R_{x}\sum_{k,l=0}^{N}\alpha_{k}^{x}\left(E_{lk}^{x}-E_{ll}^{x}\right)R_{x}^{-1}
    \end{split}
\end{equation}
...
\begin{split}
    &\exp{(-E^{x})}\sum_{k=1}^{N}\sum_{l=1}^{N}\alpha_{k}^{x}\left(E_{lk}^{x}-E_{ll}^{x}\right)\exp{(E^{x})}\\&=
     ...
     \\&=
     \sum_{k=2}^{N}\alpha_{k}^{x}\left(E_{1k}+E_{11}^{x}-\sum_{a=2}^{N}(E_{ak}^{x}+E_{a1}^{x})-E_{11}^{x}+\sum_{j=2}^{N}E_{j1}^{x}\right)+\sum_{k=1}^{N}\sum_{l=2}^{N}\alpha_{k}^{x}\left(E_{lk}^{x}+E_{\ell 1}^{x}-E_{\ell\ell}^{x}-E_{l1}^{x}\right)
     \\&=
...
\end{flushright}
\textbf{Remark}:
The duality result can be state also in terms of infinitesimal generators \eqref{Generator}. In this case, the dual process, is denoted by $(\bm{\xi}(t))_{t\geq 0}$, where for each $t\geq 0$ and for each $x\in V$ we denote
\begin{equation}
...
\textbf{Remark}: in the reverible situation, i.e. when $\forall x\in V$ $\alpha_{k}^{x}=\alpha_{k}$, the average of the element of the duality matrix with respect to the reversible measure \eqref{reversibleMeasure}, satisfies
\begin{equation}
