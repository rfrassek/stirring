...
\begin{equation}
	\begin{split}
		\Psi(\bm{\tau})=\sum_{b=0}^{L}(-1)^{b}\sum_{1\leq q_{1}<\ldots<q_{b}\leq L}^{L}\left(\prod_{k=1}^{b}\delta_{\tau_{q_{k}},N}\right)\sum_{i_{q_{1}},\ldots i_{q_{b}}=1}^{N-1}\mathbb{E}_{\mu_{\text{NESS}}}\left[\prod_{k=1}^{b}\mathbbm{1}_{\{\sigma_{x_{q_{k}}}=i_{q_{k}}\}}\prod_{x=1\,:\,\tau_{x}\neq N}^{L}\left(\mathbbm{1}_{\{\sigma_{x}=\tau_{x}\}}\right)\right]
	\end{split}
\end{equation}
[STILL TO DOUBLE CHECK AND TO WRITE BETTER...]
where the last expectation is taken with respect to the $\mu^{\text{NESS}}$ of the process $\bm{\sigma}$. Therefore, 
\begin{equation}
...
\begin{equation}
S_{1}^{-1}|\bm{\mu}\rangle=\left(|\mu_{1}\rangle+(-1)(1-\delta_{\mu_{1},N})|N\rangle\right)\otimes \ldots \otimes \left(|\mu_{L}\rangle+(-1)(1-\delta_{\mu_{L},N})|N\rangle\right)
\end{equation}
therefore, projecting on $\langle \bm{\tau}|$ we have
\begin{equation}\label{Tensor-S1}
	\langle \bm{\tau}|	S_{1}^{-1}|\bm{\mu}\rangle=\prod_{x=1}^{L}\left(\delta_{\mu_{x},\tau_{x}}+(-1)\delta_{\tau_{x},N}\sum_{i_{x}=1}^{N-1}\delta_{\mu_{x},i_{x}}\right)
\end{equation}
\begin{comment}
