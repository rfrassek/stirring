...
{\color{blue}
	...
	\end{equation}
\begin{equation}
	\Psi^{'}(\bm{\tau})=\frac{1}{(1+L)!}\sum_{c_{1}=\delta_{\tau_{1},N}}^{1}\ldots\sum_{c_{L}=\delta_{\tau_{L},N}}^{1}\prod_{x=1}^{L}\lambda_{\tau_{x}}^{1-c_{x}}\left(1+\sum_{j=x}^{L}c_{j}\right)\beta_{\tau_{x}}^{c_{x}-\delta_{\tau_{x},N}}
\end{equation}
	\begin{equation}
...
\end{equation}
\begin{equation}
\langle n^{y_{1}}_{s_{1}}\cdots n^{y_{m}}_{s_{m}}\rangle_{\text{stat}}=\frac{1}{(1+L)!}\sum_{c_{1},\ldots,c_{m}=0}^{1}\prod_{k=1}^{m}(\alpha_{s_{k}}-\beta_{s_{k}})^{1-c_{k}}\beta_{s_{k}}^{c_{k}}\left(1+\sum_{j=k}^{m}c_{j}\right)
\end{equation}
To check!
}\\
