...
\begin{equation}
	...
	\end{split}
\end{equation}
\paragraph{Action on the occupation variable $n_{d}^{x}$ for the edge generator $\mathcal{L}_{x,y}^{rd}$}
...
\end{equation}
while for every $c\in \{2,\ldots,N\}$ we have 
\begin{equation}
    \begin{split}
...
\end{equation}
Then, summing over all indices $c\in\{2,\ldots,N\}\,:\,c\neq d$, we obtain \eqref{actionGraphRD}.
\paragraph{Action on the occupation variable $n_{d}^{x}$ for the site generator $\mathcal{L}_{x}^{rd}$}
...
\begin{equation}
	...
	\\&=
	\alpha_{d}^{x}\left(\nu-n_{d}^{x}\right)-n_{d}^{x}\left(\nu(\sigma_{11}+(N-1)\sigma_{12})-\alpha_{d}^{x}\right)
	\\&=
	\end{split}
\end{equation}
