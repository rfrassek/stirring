...
\end{equation} 
where $S_{1}$ makes $H^{'}$ with both diagonal boundaries and $S_{2}$ makes $H^{''}$ with left triangular boundary and right diagonal one. Thanks to these transformations the commutators \eqref{bulk} and the action on the boundary vectors \eqref{rightBoundary}, \eqref{leftBoundary} of the operators $(X_{a})_{a\in\{1,\ldots,N\}}$ of the MPA will become simple, therefore, the steady state $|\widetilde{\Psi}\rangle$ of $H^{''}$ will be written explicitly. Then, reversing the transformations we will retrieve $|\Psi\rangle$. The first transformation $S_{1}$ is closely related to the dual matrix. Moreover, we will see that $H^{'}$ is $\widetilde{H}$, up to the extra-site term described by the bosonic creation operator in \eqref{siteDualHamiltonian}. Therefore, $H^{'}$ is not stochastic. This informally explains the role of duality. The second transformation $S_{2}$ allows to diagonalize the right boundary, paying the price of having the left one depending on the difference between the densities of the two reservoirs. \\
This idea is exposed as follows. We first define the similarity transformations $S_{1},S_{2}$ and the Hamiltonians $H^{'},H^{''}$. Second, we compute the steady state of $H^{''}$ thanks to the simpler commutators. Finally, we invert the similarities and retrieve the steady state of $H^{'}$ and $H$ respectively.  
...
The steady state $|\widehat{\Psi}\rangle$ of $H^{'}$ fulfils the equations
\begin{equation}
	H^{'}|\widehat{\Psi}\rangle=0
\end{equation}
therefore, it is easy to see that 
\begin{equation}
	|\Psi\rangle =\mathcal{S}_{1}^{-1}|\widehat{\Psi}\rangle
\end{equation}
...
\subsubsection{The steady state of $\mathbb{H}$}\label{subsectionSSHsec}
...
$\square$
\end{flushright}
\subsubsection{The steady state of $\widehat{H}^{T}$}\label{subsectionSSdual}
