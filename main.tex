...
\end{equation}
where  $ \langle m_{1},\ldots,m_{N}|$ is the row vector obtained by transposing $|m_{1},\ldots,m_{N}\rangle$ and $\delta_{\xi_{a},n_{a}}$ is the Kronhecker delta. 
...
\begin{equation}
	\begin{split}
			H_{L}^{''}&=\exp{\left(-\sum_{c=2}^{N}\beta_{c}e_{c1}^{L}\right)}\sum_{a=2}^{N}\left(\beta_{a}e_{a1}^{L}-e_{aa}^{L}\right)\exp{\left(-\sum_{c=2}^{N}\beta_{c}e_{c1}^{L}\right)}
			\\&=
...
\begin{equation}\label{notation2}
    ...
    \begin{equation}\label{InverseS2}
    	\begin{split}
    		\left(\mathcal{S}_{2}^{-1}\right)^{\otimes L}&=\sum_{\tau_{1}=1}^{N}\ldots\sum_{\tau_{L}=1}^{N}\left\{e_{\tau_{1},\ldots,\tau_{L}}^{\tau_{1},\ldots,\tau_{L}}+\sum_{q_{1}=1}^{L}(1-\delta_{\tau_{q_{1}},1})e_{\tau_{1},\ldots,\tau_{q_{1}},\ldots,\tau_{L}}^{\tau_{1},\ldots,1,\ldots,\tau_{L}}\beta_{\tau_{q_{1}}}\right. 
    		\\&+\left. \sum_{q_{1},q_{2}=1\,:\,q_{1}\neq q_{2}}^{L}(1-\delta_{\tau_{q_{1}},1})(1-\delta_{s_{q_{2}},1})e_{\tau_{1},\ldots,\tau_{q_{1}},\ldots,s_{q_{2}}\ldots,\tau_{L}}^{\tau_{1},\ldots,1,\ldots,1,\ldots,\tau_{L}}\beta_{\tau_{q_{1}}}\beta_{s_{q_{2}}}\right.
    		\\&+\left.
...
\begin{align*}
    	...
    	\ldots+	\sum_{q_{1},\ldots,q_{L-1}=1\,:\,q_{i}\neq q_{j}\,\text{if}\,i\neq j}^{L}\left(\prod_{t=1}^{L-1}(1-\delta_{s_{q_{t}},1})\right)e_{\tau_{1},\ldots,s_{q_{L-1}\tau_{L}}}^{1,\ldots,1,\tau_{L}}
    	\right. \\&+ \left. 
    	\left(\prod_{x=1}^{L}(1-\delta_{\tau_{x},1})\right)\prod_{x=1}^{L}\beta_{\tau_{x}}e_{\tau_{1},\ldots,\tau_{L}}^{1,\ldots,1} \right\}\otimes \sum_{\tau_{L+1}=1}^{N}\left\{e_{\tau_{L+1}}^{\tau_{L+1}}+(1-\delta_{\tau_{L+1},1})\sum_{q_{1}=1}^{1}\beta_{\tau_{q_{1}}}e_{\tau_{q_{1}}}^{1}\right\}
    	\\&=
    	\sum_{\tau_{1}=1}^{N}\ldots\sum_{s_{L+}=1}^{N}\left\{e_{\tau_{1},\ldots,\tau_{L+1}}^{\tau_{1},\ldots,\tau_{L+1}}+\sum_{q_{1}=1}^{L+1}(1-\delta_{\tau_{q_{1}},1})e_{\tau_{1},\ldots,\tau_{q_{1}},\ldots,\tau_{L+1}}^{\tau_{1},\ldots,1,\ldots,\tau_{L+1}}\beta_{\tau_{q_{1}}}\right. 
    	\\&+\left. \sum_{q_{1},q_{2}=1\,:\,q_{1}\neq q_{2}}^{L+1}(1-\delta_{\tau_{q_{1}},1})(1-\delta_{s_{q_{2}},1})e_{\tau_{1},\ldots,\tau_{q_{1}},\ldots,s_{q_{2}}\ldots,\tau_{L+1}}^{\tau_{1},\ldots,1,\ldots,1,\ldots,\tau_{L+1}}\beta_{\tau_{q_{1}}}\beta_{s_{q_{2}}}\right.
    	\\&+\left.
    	\ldots+	\sum_{q_{1},\ldots,q_{L}=1\,:\,q_{i}\neq q_{j}\,\text{if}\,i\neq j}^{L}\left(\prod_{t=1}^{L}(1-\delta_{s_{q_{t}},1})\right)e_{\tau_{1},\ldots,\tau_{q_{L}}\tau_{L+1}}^{1,\ldots,1,\tau_{L+1}}
    	\right. \\&+ \left. 
    	\left(\prod_{x=1}^{L+1}(1-\delta_{\tau_{x},1})\right)\prod_{x=1}^{L+1}\beta_{\tau_{x}}e_{\tau_{1},\ldots,\tau_{L+1}}^{1,\ldots,1} \right\}
    	\\&=
...
\begin{equation}
	\begin{split}
\left(\mathcal{S}_{2}^{-1}\right)^{\otimes L}_{\bm{\tau}}&=\,e_{\tau_{1},\ldots,\tau_{L}}^{\tau_{1},\ldots,\tau_{L}}+\sum_{q_{1}=1}^{L}(1-\delta_{\tau_{q_{1}},1})e_{\tau_{1},\ldots,\tau_{q_{1}},\ldots,\tau_{L}}^{\tau_{1},\ldots,1,\ldots,\tau_{L}}\beta_{\tau_{q_{1}}}\\&+\sum_{q_{1},q_{2}=1\,:\,q_{1}\neq q_{2}}^{L}(1-\delta_{\tau_{q_{1}},1})(1-\delta_{s_{q_{2}},1})e_{\tau_{1},\ldots,\tau_{q_{1}},\ldots,s_{q_{2}}\ldots,\tau_{L}}^{\tau_{1},\ldots,1,\ldots,1,\ldots,\tau_{L}}\beta_{\tau_{q_{1}}}\beta_{s_{q_{2}}}
  \\&+
  \ldots+\left(\prod_{x=1}^{L}(1-\delta_{\tau_{x},1})\right)\prod_{x=1}^{L}\left(\beta_{\tau_{x}}\right)^{1-\delta_{\tau_{x},1}}e_{\tau_{1},\ldots,\tau_{L}}^{1,\ldots,1}
	\end{split}
\end{equation}
...
\begin{equation}\label{ABS_intermediate}
	\begin{split}
		\Psi^{'}(\bm{\tau})&=(\mathcal{S}_{2}^{-1})_{\bm{\tau}}^{\otimes L}|\Psi^{''}\rangle\\&= \Psi^{''}(\bm{\tau})+\sum_{q_{1}= 1}^{L}(1-\delta_{\tau_{q_{1}},1})\Psi^{''}(\tau_{1},\ldots,\overbrace{1}^{\tau_{q_{1}}},\ldots,\tau_{L})\beta_{q_{1}}\\&+
		\sum_{q_{1},q_{2}= 1}^{L}(1-\delta_{\tau_{q_{1}},1})(1-\delta_{s_{q_{2}},1})\Psi^{''}(\tau_{1},\ldots,\overbrace{1}^{q_{1}},\ldots,\overbrace{1}^{q_{2}},\ldots,\tau_{L})\beta_{a_{1}}\beta_{a_{2}}\\&+\ldots+\Psi^{''}(1,\ldots,1)\left(\prod_{x=1}^{L}(1-\delta_{\tau_{x},1})\right)\prod_{x=1}^{L}\left(\beta_{\tau_{x}}\right)^{1-\delta_{\tau_{x},1}}
	\end{split}
\end{equation}
For a fixed configuration $\bm{\tau}=(\tau_{1},\ldots,s_{N})$, using equation \eqref{ResulsBasis} and observing that 
\begin{equation}
...
\begin{equation}
	\mathcal{L}_{1}f(\bm{n})=\sum_{a,b=0}^{N}\alpha_{a}n_{b}^{1}\left(f(\bm{n}-\bm{\delta}_{b}^{x}+\bm{\delta}_{a}^{x})-f(\bm{n})\right)\qquad \mathcal{L}_{L}f(\bm{n})=\sum_{a,b=0}^{N}\beta_{a}n_{b}^{L}\left(f(\bm{n}-\bm{\delta}_{b}^{x}+\\bm{delta}_{a}^{x})-f(\bm{n})\right)
\end{equation}
...
\begin{equation}
	\begin{split}
		\widetilde{\mathcal{L}}_{1}f(\bm{\xi})&=|\alpha|\sum_{a=2}^{N}\xi_{a}^{1}\left(f(\bm{\xi}-\bm{\delta}_{a}^{1}+\bm{\delta}_{a}^{0}+\bm{\delta}_{a}^{1})f(\bm{\xi})\right)\\\widetilde{\mathcal{L}}_{L}f(\bm{\xi})&=\sum_{i=1}^{L}\beta_{i}\sum_{a=2}^{N}\xi_{a}^{L}\left(f(\bm{\xi}-\bm{\delta}_{a}^{L}+\bm{\delta}_{a}^{L+1}+\bm{\delta}_{a}^{L})-f(\bm{\xi})\right)
	\end{split}
\end{equation}
...
\begin{align}
	\left(\mathcal{L}_{1}^{TH}D(\cdot,\bm{\xi})\right)(\bm{n})&=\left\{\prod_{a=2}^{N}\left(\frac{\alpha_{k}}{|\alpha|}\right)^{\xi_{a}^{0}+\xi_{a}^{1}}\prod_{x=2}^{N}d_{x}(n^{x},\xi^{x})\prod_{d=2}^{N}(\rho_{a}^{R})^{\xi_{d}^{L+1}}-D(\bm{n},\bm{\xi})\right\}
	\\&=
...
\begin{equation}
\begin{split}
        \widetilde{\mathcal{L}}_{1}f(\bm{\xi})&=|\alpha|\sum_{a=2}^{N}\xi_{a}^{1}\left(f(\bm{\xi}-\delta_{a}^{1}+\delta_{a}^{0}+\delta_{a}^{1})f(\bm{\xi})\right)\\\widetilde{\mathcal{L}}_{L}f(\bm{\xi})&=\sum_{i=1}^{L}\beta_{i}\sum_{a=2}^{N}\xi_{a}^{L}\left(f(\bm{\xi}-\delta_{a}^{L}+\delta_{a}^{L+1}+\delta_{a}^{L})-f(\bm{\xi})\right)
        \end{split}
\end{equation}
where $0$ and $L+1$ are the extra-sites. The duality function on the chain is
\begin{equation}\label{dualityFunctionChain}
