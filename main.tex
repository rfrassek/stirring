...
\begin{align}
	...
	\left(\mathcal{L}_{y}^{TH}D(\cdot,\bm{\xi})\right)(\bm{n})&=\left(\widetilde{\mathcal{L}}_{y}^{TH}D(\bm{n},\cdot)\right)(\bm{\xi})\qquad y\in\{1,L\}\label{boundaryDualityTH}
\end{align}
...
\begin{equation}\label{pezzo2}
	\begin{split}
		&D(\bm{n},\bm{\xi})\sum_{r_{1}=0}^{\nu}\ldots\sum_{r_{N}=0}^{\nu}\mathbbm{1}_{\{r_{1}+\ldots+r_{N}=\nu\}}\frac{\nu!}{r_{1}!\cdots r_{N}!}\left(\prod_{k=1}^{N-1}\left(\frac{\alpha_{k}}{\alpha_{1}}\right)^{r_{k}}\right)\left(\frac{\alpha_{N}}{|\alpha|}\right)^{\nu}\left(\prod_{a=1}^{N-1}\left(\frac{\alpha_{N}}{|\alpha|}\right)^{r_{a}-r_{a}}\right)\\=&D(\bm{n},\bm{\xi})
	\end{split}
\end{equation}
