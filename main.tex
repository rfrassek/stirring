...
\begin{equation}\label{EX3}
	\begin{split}
		D_{j,j}(x)B_{i}(y)&=B_{i}(x)D_{j,j}(y)+\frac{1}{2}h_{3}(x,y)\left(B_{i}(x)D_{i,i}(y)+B_{i}(y)D_{N,N}(x)\right)
		\\&-
		\frac{1}{2}h_{3}(x,y)\left(B_{i}(y)D_{i,i}(x)+B_{i}(x)D_{N,N}(y)\right)
		\\&+
		h_{1}(x,y)\sum_{k=1\,:\, k\neq i}^{N-1}\left(B_{k}(x)D_{k,i}(y)-B_{k}(y)D_{k,i}(x)\right)
	\end{split}
\end{equation}
\end{itemize}
where
...
\begin{equation}
	D_{N,N}(x)=\sum_{a=0}^{2L+1}D_{N,N}^{a}x^{a}\quad D_{ii}(x)=\sum_{a=0}^{2L+1}D_{ii}^{a}x^{a}\quad B_{i}(x)=\sum_{a=0}^{2L-1}B_{i}^{a}x^{a}
\end{equation}
The coefficients that we need are the following:
\begin{equation}
	D_{N,N}(x)=D_{N,N}^{2L+1}x^{2L+1}+D_{N,N}^{2L}x^{2L+1}+D_{N,N}^{2L-1}x^{2L-1}+\ldots
\end{equation}
where 
\begin{equation}
	D_{N,N}^{2L+1}=I^{tot}
...
\begin{equation}
	\begin{split}
		D_{N,N}^{2L-1}=3e_{NN}^{tot}-\sum_{j=1}^{N-1}e_{jj}^{tot}+2\sum_{a\neq b}e_{NN}^{[a]}e_{NN}^{[b]}
	\end{split}
\end{equation}
{\color{blue}ok}

We observe that $\forall i,j=1,\ldots,N-1$
\begin{equation}
	\sum_{a\neq b}e_{ij}^{[a]}e_{ji}^{[b]}=e_{ij}^{tot}e_{ji}^{tot}-e_{ii}^{tot}
\end{equation}
then 
\begin{equation}
	D_{N,N}^{2L-1}=e_{NN}^{tot}\left(2e_{NN}^{tot}+I^{tot}\right)-\sum_{j=1}^{N}e_{jj}^{tot}%=2e_{NN}^{tot}\left(e_{NN}^{tot}+I^{tot}\right)-L
\end{equation}
Then
\begin{equation}
	\boxed{D_{N,N}^{2L-1}	=2e_{NN}^{tot}\left(e_{NN}^{tot}+I^{tot}\right)-L}
\end{equation}
{\color{blue}ok?}where we used $
\sum_{i=1}^Ne_{ii}^{tot}=L$. 

While, $\forall i=1,\ldots,N-1$
...
\begin{equation}\label{a2lm1}
	\begin{split}
		D_{ii}^{2L-1}&=2e_{ii}^{tot}+\left(e_{NN}^{tot}-\sum_{j=1}^{N-1}e_{jj}^{tot}\right)-2\sum_{a\neq b}e_{ii}^{[a]}e_{ii}^{[b]}-2\sum_{a\neq b}\sum_{j=1\,:\,j\neq i}^{N-1}e_{ij}^{[a]}e_{ji}^{[b]}\\
		&=e_{11}^{tot}-\sum_{j=1}^{N-1}e_{jj}^{tot}+2e_{ii}^{tot}-2\sum_{a\neq b}\sum_{j=1}^{N-1}e_{ij}^{[a]}e_{ji}^{[b]}\\
		%&=e_{11}^{tot}-\sum_{j=2}^{N}e_{jj}^{tot}+2e_{ii}^{tot}\\
		%&=2e_{11}^{tot}+2e_{ii}^{tot}-C_1
		&=e_{NN}^{tot}-\sum_{j=1}^{N-1}e_{jj}^{tot}+2e_{ii}^{tot}-2\sum_{a\neq b}\sum_{j=1}^{N-1}e_{ij}^{[a]}e_{ji}^{[b]}\\ 
	\end{split}
\end{equation}
...
\begin{equation}
	\begin{split}
		D_{ii}^{2L-1}&=2e_{ii}^{tot}+\left(e_{NN}^{tot}-\sum_{j=1}^{N-1}e_{jj}^{tot}\right)-2\sum_{j=1}^{N-1}e_{ij}^{tot}e_{ji}^{tot}+2(N-1)e_{ii}^{tot}
	\end{split}
\end{equation}
then
\begin{equation}
	\begin{split}
		D_{ii}^{2L-1}=	2Ne_{ii}^{tot}+\left(e_{NN}^{tot}-\sum_{j=1}^{N-1}e_{jj}^{tot}\right)-2\sum_{j=2}^{N}e_{ij}^{tot}e_{ji}^{tot}%=2Ne_{ii}^{tot}+ 2e_{NN}^{tot}-L -2\sum_{j=2}^{N}e_{ij}^{tot}e_{ji}^{tot}
	\end{split}
\end{equation}
Then 
\begin{equation}
	\boxed{D_{ii}^{2L-1}=2Ne_{ii}^{tot}+ 2e_{NN}^{tot}-L -2\sum_{j=2}^{N}e_{ij}^{tot}e_{ji}^{tot}}
\end{equation}
{\color{blue}ok}
%blue} In case of a chain $L=2$ and $N=3$ and choosing $i=2$ this reads as 
%\begin{equation*}
%	D_{22}^{2L-1}=6e_{22}^{tot}+e_{11}^{tot}-e_{22}^{tot}-e_{33}^{tot}-2e_{22}^{tot}e_{22}^{tot}-2e_{23}^{tot}e_{32}^{tot}
%\end{equation*}
%where
%\begin{equation*}
%	e_{ab}^{tot}=e_{ab}\otimes \mathbbm{1}+\mathbbm{1}\otimes e_{ab}\qquad \forall a,b\in \{1,2,3\}
%\end{equation*}
%This gives exactly
%\begin{equation*}
%	D_{22}^{2L-1}=\left(
...
\begin{equation}
\begin{split}
B_{j}^{2L-1}=2e_{j1}^{tot}+
2\sum_{a,b=1\,:\, a<b}^{L}\left(e_{j1}^{[a]}e_{11}^{[b]}\right)-2\sum_{a,b=1\,:\, a>b}^{L}\,\sum_{k=2}^{N}\left( e_{jk}^{[a]}e_{k1}^{[b]}\right)
\end{split}
\end{equation}
\begin{equation}
\begin{split}
B_{j}^{2L-1}&=
2\sum_{a,b=1}^{L}\left(e_{j1}^{[a]}e_{11}^{[b]}\right)-2\sum_{a,b=1\,:\, a>b}^{L}\,\sum_{k=1}^{N}\left( e_{jk}^{[a]}e_{k1}^{[b]}\right)\\
%	&=
%	2e_{j1}^{tot}e_{11}^{tot}-2\sum_{a,b=1\,:\, a>b}^{L}\,\sum_{k=1}^{N} e_{jk}^{[a]}e_{k1}^{[b]}
\end{split}
\end{equation}
Thus
\begin{equation}
\boxed{	B_{j}^{2L-1}=	2e_{j1}^{tot}e_{11}^{tot}-2\sum_{a,b=1\,:\, a>b}^{L}\,\sum_{k=1}^{N} e_{jk}^{[a]}e_{k1}^{[b]}}
\end{equation}
Finally, for every $i,j=2,\ldots,N$ such that $i\neq j$, we have that 
\begin{equation}
\boxed{D_{i,j}^{2L}=-2e_{ji}^{tot}}
\end{equation}
...
\begin{equation}
\begin{split}
Q_{2L}^{0}&=N(I^{tot}+e_{NN}^{tot})+e_{NN}^{tot}(2e_{NN}^{tot}+I^{tot})-\sum_{j=1}^{N-1}e_{jj}^{tot}-2N\sum_{j=1}^{N-1}e_{jj}^{tot}+\sum_{k=1}^{N-1}\left\{e_{NN}^{tot}-\sum_{j=1}^{N-1}e_{jj}^{tot}+\sum_{j=1}^{N-1}e_{kj}^{tot}e_{jk}^{tot}\right\}
\end{split}
\end{equation}
\begin{equation}
	\begin{split}
\boxed{Q_{2L}^{0}=(N-NL-2L)I^{tot}+2(N+2)e_{NN}^{tot}+2e_{NN}^{tot}e_{NN}^{tot}+2\sum_{j,k=1}^{N-1}e_{kj}^{tot}e_{jk}^{tot}}
\end{split}
\end{equation}
%\begin{equation}	
%\begin{split}
%Q_{2L}^{0}&=NI^{tot}+2e_{11}^{tot}(e_{11}^{tot}+I^{tot})-(N+2)\sum_{j=2}^{N}e_{jj}^{tot}+2\sum_{i,j=2}^{N}e_{ij}^{tot}e_{ji}^{tot}\\
%&=N+2e_{11}^{tot}(e_{11}^{tot}+1)+(N+2)(e_{11}^{tot}-L)+2\sum_{i,j=2}^{N}e_{ij}^{tot}e_{ji}^{tot}\\
%\end{split}
%\end{equation} 
we also introduce 
\begin{equation}
\mathcal{Q}_{2L}=\sum_{i=1}^{N-1}2(\alpha_{i}-\beta_{i})B_{i}^{2L-1}
\end{equation}
...
All terms are diagonal except the last one which can  be  identified with the second Casimir of the $gl(N-1)$ subalgebra
\begin{equation}
\hat C_2^{N-1}= \sum_{i,j=1}^{N-1}e_{ij}^{tot}e_{ji}^{tot}
\end{equation} 
On the Bethe states (and on the irrep decomposition of the spin chain) it acts diagonally with eigenvalues given in terms of shifted weights 
\begin{equation}
C_2^{N-1}=\sum_{j=1}^{N-1}\ell_j(\ell_j -N+2)+\frac{1}{6}(N-1)(N-2)(N-3)
\end{equation} 
{\color{blue}to be checked...} {\color{blue} It works!!}, see e.g. [Das,Okubo].
...
\end{equation} 
{\color{blue}ok}
\subsubsection{Exchange relations between charges}
...
\subsection{The non local transformation}
...
We have that 
\begin{equation}
	\left[H_{0},Q^{0}_{2L}\right]=0\qquad \left[H^{''},Q_{2L}\right]=0
\end{equation}
thus it is enough to look for a matrix $W$ such that 
\begin{equation}\label{GoalWQ}
	Q^{2L}W=WQ_{2L}^{0}
\end{equation}
...
\begin{equation}
	WH_{0}=HW
\end{equation}
Power expanding the transfer matrix by charges and considering the limit for $x\to\infty$ we have that 
\begin{equation}
	\begin{split}
		...
		\end{split}
\end{equation}
and also satisfies \eqref{GoalWQ}. We now write the power expansion with respect to $B_{i}^{2L-1}$ of $W$. We obtain that, using the multi-variate version of \eqref{veryUsefulSeries},
\begin{equation}
	\begin{split}
	W=&\lim_{\epsilon\to 0}\epsilon\sum_{\bm{m}\,:\,|\bm{m}|=L}\sum_{k_{1}=0}^{L}\ldots\sum_{k_{N-1}=0}^{L}\mathbbm{1}_{\{k_{1}+\cdots+k_{N-1}\leq L\}}2^{k_{1}+\ldots+k_{N-1}}\Delta_{1}^{k_{1}}\cdots\Delta_{N-1}^{k_{N-1}}\\&SYM\left(\left\{\left(\Lambda_{\bm{m}}^{Q}+\epsilon-Q_{2L}^{0}\right)^{-1}B_{1}^{2L-1}\right\}^{k_{1}},\ldots,\left\{\left(\Lambda_{\bm{m}}^{Q}+\epsilon-Q_{2L}^{0}\right)^{-1}B_{N-1}^{2L-1}\right\}^{k_{N-1}}\right)
	\\&
...
\begin{equation}
	...
	\end{split}
\end{equation}
Considering the empty state as 
\begin{equation}
...
\end{pmatrix}
\end{equation}
then, we can map via $W$ the above vector to the steady state $|\Psi^{''}\rangle$ of $H^{''}$ defined in \eqref{steadyS-SECOND-def}, namely
\begin{equation}
...
\begin{equation}
	\begin{split}
	B_{j}(x)D_{jj}(y)=&\frac{(x+y)(1+x-y)}{(x-y)(1+x+y)}D_{jj}(y)B_{j}(x)-\frac{1}{(1+x+y)}\sum_{i=1\,:\,i\neq j}^{N-1}B_{i}(x)D_{i1}(y)
	\\-&\frac{(x+y)}{(x-y)(1+x+y)}D_{jj}(x)B_{j}(y)-\frac{1}{(1+x+y)}D_{NN}(x)B_{j}(y)
	\end{split}
\end{equation}
\item for $i,j\neq N$
\begin{equation}
	\begin{split}
		B_{i}(x)D_{jj}(y)=D_{jj}(y)B_{j}(x)+\frac{1}{(x-y)}\left(D_{ji}(y)B_{j}(x)-D_{ji}(x)B_{j}(y)\right)
	\end{split}
\end{equation}
\item for $j\neq N$
\begin{equation}
	\begin{split}
		B_{j}(x)D_{NN}(y)&=\frac{(x-y)(1+x+y)}{(x+y)(1+x-y)}D_{NN}(y)B_{j}(x)+\frac{(x-y)}{(x+y)(1+x-y)}\sum_{i=1}^{N-1}B_{i}(y)D_{ij}(x)
		\\&+
		\frac{1}{(1+x-y)}B_{j}(y)D_{NN}(x)
	\end{split}
\end{equation}
\end{itemize}
\end{comment}
...
% \end{itemize}
\subsubsection{Explicit solution for $G_{m_{2},m_{3},\epsilon}^{k_{2},k_{3}}(x)$ using symmetric group}

 For the sake of notation we denote by 
\begin{equation}
	\begin{split}
		A_{2}= \left\{(2x+N)\left((\Lambda_{m_{2},m_{3}}(x)+\epsilon)I-T_{0}(x)\right)^{-1}B_{2}(x)\right\}\\
		A_{3}=\left\{(2x+N)\left((\Lambda_{m_{2},m_{3}}(x)+\epsilon)I-T_{0}(x)\right)^{-1}B_{3}(x)\right\}
	\end{split}
\end{equation}
%{\color{red}Are we missing the prefactor $(N+2x)$?}
where the dependence of $A_{2},A_{3}$ on $x$ and $\epsilon$ is understood. Condition \eqref{cond4} becomes

\begin{equation}\label{cond4A}
	\begin{split}
...
\begin{equation}
	i_{1}=\ldots=i_{k_{2}}=2\qquad i_{k_{2}+1}=\ldots =i_{k_{2}+k_{3}}=3
\end{equation}
Then, we introduce the permutation 
\begin{equation}
	\sigma(i_{j})=i_{k}\qquad \forall j,k=1,\ldots,k_{2}+k_{3}
\end{equation}
We write
\begin{equation}
	G_{m_{2},m_{3},\epsilon}^{k_{2},k_{3}}(x)=\frac{1}{k_{2}!k_{3}!}\sum_{\sigma \in \text{SYM}}A_{\sigma(i_{1})}\cdots A_{\sigma(i_{k_{2}+k_{3}})}
\end{equation}
where we denoted by $\text{SYM}$ the symmetric group on $\{i_{1},\ldots,i_{k_{2}+k_{3}}\}$.\\
It solves \eqref{cond4A}, indeed
\begin{align*}
	&A_{2}G_{m_{2},m_{3},\epsilon}^{k_{2}-1,k_{3}}(x)+A_{3}G_{m_{2},m_{3},\epsilon}^{k_{2},k_{3}-1}(x)
	\\=&A_{2}\frac{k_{2}}{k_{2}}G_{m_{2},m_{3},\epsilon}^{k_{2}-1,k_{3}}(x)+A_{3}\frac{k_{3}}{k_{3}}G_{m_{2},m_{3},\epsilon}^{k_{2},k_{3}-1}(x)
	\\=&
	A_{2}\frac{k_{2}}{k_{2}}\frac{1}{(k_{2}-1)!k_{3}!}\sum_{\sigma \in \text{SYM}}A_{\sigma(i_{2})}\cdots A_{\sigma(i_{k_{2}+k_{3}})}\\+&A_{3}\frac{k_{3}}{k_{3}}\frac{1}{k_{2}!(k_{3}-1)!}\sum_{\sigma \in \text{SYM}}A_{\sigma(i_{1})}\cdots A_{\sigma(i_{k_{2}})}A_{\sigma(k_{2}+2)}\cdots A_{\sigma(i_{k_{2}+k_{3}})}
	\\=&
	\frac{1}{k_{2}!k_{3}!}\left(\sum_{\sigma \in \text{SYM}}k_{2}A_{2}A_{\sigma(i_{2})}\cdots A_{\sigma(i_{k_{2}+k_{3}})}+ \sum_{\sigma \in \text{SYM}}k_{3}A_{3}A_{\sigma(i_{1})}\cdots A_{\sigma(i_{k_{2}})}A_{\sigma(k_{2}+2)}\cdots A_{\sigma(i_{k_{2}+k_{3}})}\right)
	\\=&
	\frac{1}{k_{2}!k_{3}!}\sum_{\sigma \in \text{SYM}}A_{\sigma(i_{1})}\cdots A_{\sigma(i_{k_{2}+k_{3}})}
	\\=&
		G_{m_{2},m_{3},\epsilon}^{k_{2},k_{3}}(x)
\end{align*}
where in the up to last equality we used the fact that the quantity first summation is symmetric with respect to the exchange of $A_{2}$ and thus we have $k_{2}$ equal terms. Similarly, for the second summation using the symmetry with respect to $A_{3}$. \\
This argument can be generalized for $N>2$ species introducing
\begin{equation}
	G_{m_{2},\ldots,m_{N},\epsilon}^{k_{2},\ldots,k_{N}}(x)=\frac{1}{k_{2}!\cdots k_{N}!}\sum_{\sigma\in \text{SYM}}A_{i_{1}}\cdots A_{i_{k_{2}+\cdots+k_{N}}}
\end{equation}
where we assumed 
\begin{equation}
	i_{1}=\cdots=i_{k_{2}}=2,\qquad \ldots \qquad i_{k_{2}+\cdots+k_{N-1}}=\cdots=i_{k_{2}+\ldots+k_{N}}=N
\end{equation}
\begin{comment}
To write its solution we define
\begin{equation}
	\chi^{k_{2},k_{3}}:=\left\{\left(i_{1},\ldots,i_{(k_{2}+k_{3})}\right)\in \{2,3\}^{k_{2}+k_{3}}\;:\;\sum_{s=1}^{k_{2}+k_{3}}\delta_{i_{s},2}=k_{2}\right\}
\end{equation}
We introduce the following notation: for any fixed configuration $(i_{1},\ldots,i_{(k_{2}+k_{3})})\in \chi^{k_{2},k_{3}}$ 
\begin{equation}
	\mathcal{A}(i_{1},\ldots,i_{(k_{2}+k_{3})})=A_{i_{1}}A_{i_{2}}\cdots A_{i_{(k_{2}+k_{3})}}
\end{equation}
We introduce $\text{SYM}\left(\chi^{k_{2},k_{3}}\right)$ the \textit{symmetric group} on $\chi^{k_{2},k_{3}}$ and we denote by $\left(\sigma(i_{1}),\ldots,\sigma(i_{(k_{2}+k_{3})})\right)$ the permutation of an element of the group. \\
The following expression solves \eqref{cond4A}
\begin{equation}
	G_{m_{2},m_{3},\epsilon}^{k_{2},k_{3}}(x)=\sum_{\sigma\in \text{SYM}(\chi^{k_{2},k_{3}})}\mathcal{A}\left(\sigma(i_{1}),\ldots,\sigma(i_{(k_{2}+k_{3})})\right).
\end{equation}
Indeed
\begin{align}
A_{2}G_{m_{2},m_{3},\epsilon}^{k_{2}-1,k_{3}}(x)+A_{3}G_{m_{2},m_{3},\epsilon}^{k_{2},k_{3}-1}(x)&= A_{2}\sum_{\sigma\in \text{SYM}(\chi^{k_{2}-1,k_{3}})}\mathcal{A}\left(\sigma(i_{2}),\ldots,\sigma(i_{(k_{2}+k_{3})})\right)
\\&+
 A_{3}\sum_{\sigma\in \text{SYM}(\chi^{k_{2},k_{3}-1})}\mathcal{A}\left(\sigma(i_{2}),\ldots,\sigma(i_{(k_{2}+k_{3})})\right)
 \\&=
 \sum_{\sigma\in \text{SYM}(\chi^{k_{2}-1,k_{3}})}\mathcal{A}\left(2,\sigma(i_{2}),\ldots,\sigma(i_{(k_{2}+k_{3})})\right)
 \\&+
 \sum_{\sigma\in \text{SYM}(\chi^{k_{2},k_{3}-1})}\mathcal{A}\left(3,\sigma(i_{2}),\ldots,\sigma(i_{(k_{2}+k_{3})})\right)
 \\&=
 \sum_{\sigma\in \text{SYM}(\chi^{k_{2},k_{3}})}\mathcal{A}\left(\sigma(i_{1}),\ldots,\sigma(i_{(k_{2}+k_{3})})\right)
 \\&=
 	G_{m_{2},m_{3},\epsilon}^{k_{2},k_{3}}(x)
\end{align}
where in the up to last equality we used the fact that $i_{1}\in \{2,3\}$. 
\paragraph{Generalization to arbitrary $N$}
For arbitrary $N\geq2$ we have 
\begin{equation}
	\chi^{k_{2}\ldots,k_{N}}:=\left\{\left(i_{1},\ldots,i_{(k_{2}+\cdots+k_{N})}\right)\in \{2,\ldots,N\}^{k_{2}+\cdots+k_{N}}\;:\;\sum_{s=1}^{k_{2}+\cdots+k_{N}}\delta_{i_{s},q}=k_{q},\;\forall \in \{2,\ldots,N\}\right\}
\end{equation}

Thus the explicit expression becomes
\begin{equation}
G_{m_{2},\ldots,m_{N},\epsilon}^{k_{2},\ldots,k_{N}}(x)=\sum_{\sigma\in \text{SYM}(\chi^{k_{2},\ldots,k_{N}})}\mathcal{A}\left(\sigma(i_{1}),\ldots,\sigma(i_{(k_{2}+\cdots+k_{3})})\right)
\end{equation}
with the same notation as in $N=3$ case. 
\end{comment}
\subsubsection{Explicit solution for $G_{m_{2},m_{3},\epsilon}^{k_{2},k_{3}}(x)$ with sums and products}
\begin{equation}
	\begin{split}
...
\begin{equation}
	\begin{split}
	A_{2}= \left\{(2x+N)\left((\Lambda_{m_{2},m_{3}}(x)+\epsilon)I-T_{0}(x)\right)^{-1}B_{2}(x)\right\}\\
	A_{3}=\left\{\left(2x+N)((\Lambda_{m_{2},m_{3}}(x)+\epsilon)I-T_{0}(x)\right)^{-1}B_{3}(x)\right\}
	\end{split}
\end{equation}
...
\begin{equation}
	\begin{split}
A_{2}G_{m_{2},m_{3},\epsilon}^{k_{2}-1,k_{3}}(x)&=A_{2}\sum_{i_{2}=0}^{1}\ldots\sum_{i_{(k_{2}+k_{3})}=0}^{1}\mathbbm{1}_{\{i_{2}+i_{k_{2}}+\cdots+i_{(k_{2}+k_{3})}=k_{2}-1\}}\prod_{s=2}^{k_{2}+k_{3}}A_{2}^{i_{s}}A_{3}^{1-i_{s}}
\\&=
...
Denoting by
\begin{equation}
A_{j}=	\left\{\left((2x+N)(\Lambda_{m_{2}\ldots,m_{N}}(x)+\epsilon)I-T_{0}(x)\right)^{-1}B_{j}(x)\right\}\qquad \forall j\in \{2,\ldots,N\}
\end{equation}
