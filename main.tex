...
\begin{align}
	...
	\sum_{a=1}^{N-1}\left(\delta_{cN}\sum_{b=1}^{N-1}\left(E_{ad}\delta_{bN}-\delta_{ad}E_{bN}\right)-\delta_{d<N}\left(E_{aN}\delta_{cN}-E_{cN}\delta_{aN}\right)\right)\nonumber\\=&
...
From the third commutator on we always obtain zero. All in all we have that 
 \begin{align}\label{HT-BA}
 	\exp{\left(-\sum_{a=1}^{N-1}E_{aN}\right)}E_{cd}\exp{\left(\sum_{a=1}^{N-1}E_{aN}\right)}=E_{cd}-\left(\delta_{cN}\sum_{a=1}^{N-1}E_{ad}-\delta_{d<N}E_{cN}\right)-\delta_{cN}\delta_{d<N}\sum_{b=1}^{N-1}E_{bN}
 \end{align}
\begin{comment}
\begin{enumerate}
...
By using \eqref{HT-BA} and the fact that the Casimir operator \eqref{secondCasimir} is central for the algebra we have 
\begin{comment}
