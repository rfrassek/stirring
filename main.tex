...
{\color{blue}
\begin{remark}
Alternatively, by a direct computation we immediately verify 
\begin{equation}
\begin{split}
\langle N,\ldots,N,a_1,N,\ldots,N,a_m,N,\ldots,N|\Psi^{'}\rangle
 =\sum_{\bm{\tau}\in\Omega^{'}}\prod_{k=1}^{m}\delta_{{{\tau}_{x_k}},a_{k}}\langle \bm{\tau}|\Psi\rangle=\mathbb{E}\left[\prod_{k=1}^{m}\mathbbm{1}_{\{Y_{x_{k}}=a_{k}\}}\right]
 \end{split}
\end{equation} 
where the entries $a_i\neq N$ are at position $x_i$. Here we only  used  that $|\Psi^{'}\rangle=S_1|\Psi\rangle$ and
\begin{equation}
