...
\newcommand{\dd}{\mathcal{D}_{\hat{x}}}
...
\end{equation}
Moreover, we denote a particle configuration as $\mathbf{n}\in \Omega$, where $\mathbf{n}=(n_{k}^{x})_{x\in V,\,k\in\{1,\ldots,N\}}$  and we denote the site occupation vector as $n^{x}=(n_{1}^{x},\ldots,n_{N}^{x})$, for all $x\in V$.\\
...
\mathcal{L}_{x,y}f(\bm{n})=\sum_{k,l=1}^{N}n_{k}^{x}n_{l}^{y}\left[f(\bm{n}-\delta_{x}^{k}+\delta_{l}^{x}+\delta_{k}^{y}-\delta_{l}^{y})-f(\bm{n})\right]
\end{equation}
\begin{equation}\label{siteGenerator}
    \mathcal{L}_{x}f(\bm{n})=\sum_{k,l=1}^{N}\alpha_{k}^{x}n_{l}^{x}\left[f(\bm{n}+\delta_{k}^{x}-\delta_{l}^{x})-f(\bm{n})\right]
\end{equation}
where 
\begin{equation}
...
The process $(\mathbf{n}(t))_{t\geq 0}$, defined by the generator \eqref{Generator}, is dual to the absorbing boundary process $(\bm{\xi}(t))_{t\geq 0}$ living in the enlarged state space
\begin{equation}\label{dualStateSpace}
    \widetilde{\Omega}=\Omega_{x}^{V}\times \mathbb{N}_{0}^{V}
\end{equation}
with $\forall x\in V$ and $\forall t\geq 0$ we define
\begin{equation}
    \xi^{x}(t)=\left(r_{1}^{x}(t),\ldots,r_{N}^{x}(t),m_{2}^{\hat{x}}(t),\ldots,m_{N}^{\hat{x}}(t)\right)
\end{equation}
 where we denote $r_{k}^{x}(t)$ the occupation of dual particle of type $k\in \{1,\ldots,N\}$ at site $x$ at time $t\geq 0$ and where $m_{k}^{\hat{x}}(t)$ the occupation of dual particle of type $k\in \{1,\ldots,N\}$ at extra-site $\hat{x}$ connected to $x\in V$ at time $t\geq 0$.\\
...
\begin{equation}\label{siteDualGenerator}
    \widetilde{\mathcal{L}}_{x}f(\bm{\xi})=\sum_{i=1}^{N}\alpha_{i}^{x}\sum_{k=2}^{N}\xi_{k}^{x}\left(f(\bm{\xi}-\delta_{k}^{x}+\delta_{1}^{x}+\delta_{k}^{\hat{x}})-f(\bm{\xi})\right)
\end{equation}
and the duality function is 
\begin{equation}\label{dualityElements}
	D(\bm{n},\bm{\xi})=\prod_{x\in V}\left(\frac{(2j-\sum_{k=2}^{N}r_{k}^{x})!}{\nu!}\prod_{k=2}^{N}\frac{n_{k}^{x}!}{(n_{k}^{x}-r_{k}^{x})!}\left(\frac{\alpha_{k}^{x}}{\sum_{i=1}^{N}\alpha_{i}^{x}}\right)^{m_{k}^{\hat{x}}}\right)
\end{equation}
\newline
On one hand, the edge part of the dual generator \eqref{DualGenerator} is a copy of \eqref{edgeGenerator}, therefore it performs the stirring dynamics on the graph. On the other hand, the site dual generator \eqref{siteDualGenerator} replaces a particle of any type $k\in\{2,\ldots,N\}$ at site $x$ with a particle of type $1$ and creates a particle of the same type $k$ at the extra-site $\hat{x}$. This last transition is performed with rate $\sum_{i=1}^{N}\alpha_{i}^{x}r_{k}^{x}$. This means that eventually the dual process empties the graph, putting all the dual particles of species $\{2,\ldots,N\}$ in the extra-sites, that play the role of absorbing boundaries. 
\newline \newline
\textbf{Remark}: in the reversible situation, i.e. when $\forall x\in V$ $\alpha_{k}^{x}=\alpha_{k}$, the average of the element of the duality matrix with respect to the reversible measure \eqref{reversibleMeasure}, is
\begin{equation}
\mathbb{E}_{\mu^{rev}}\left[D(\bm{n},\bm{\xi})\right]=\prod_{k=2}^{N}\left(\frac{\alpha_{k}}{\sum_{i=1}^{N}\alpha_{i}}\right)^{\sum_{x\in V}r_{k}^{x}+\sum_{x\in V}m_{k}^{\hat{x}}}\qquad \forall \bm{n}\in \Omega,\quad\forall \bm{\xi}\in \widetilde{\Omega}
\end{equation}

\subsection{Proof of duality}
To prove duality between $(\bm{n}(t))_{t\geq 0}$ and $(\bm{\xi}(t))_{t\geq 0}$ we shall show that \eqref{dualityRelationGenerator} is fulfilled. Here the state space is discrete, therefore the proof consists in finding an intertwining matrix $D$ that satisfies \eqref{dualityIntertwines}. To show this, we will use the Hamiltonians (linked with the generators by \eqref{Hamiltonian-Generato}) and their Lie algebraic description. Indeed, in this formalism the proof reduces in finding symmetries and exponential transformation of generators of the Lie algebra, that are the building blocks of the original and dual Hamiltonians \eqref{OriginalHamiltonian} and \eqref{DualHamiltonian} respectively.  \\
The duality statement of subsection \ref{statementDualitySubsection} is rewritten in terms of Hamiltonian operators. The dual state space \eqref{dualStateSpace} is interpreted as a vector spaces with elements
\begin{equation}
    |\bm{\xi}\rangle=\bigotimes_{x\in V}\left(|r_{1}^{x},\ldots,r_{N}^{x}\rangle\otimes |m_{2}^{\hat{x}},\ldots,m_{N}^{\hat{x}}\rangle\right)
\end{equation}
...
\begin{equation}\label{siteDualHamiltonian}
    \widetilde{H}_{x}=\sum_{i=1}^{N}\alpha_{i}^{x}\sum_{k=2}^{N}\left((a^{\dagger})_{k}^{\hat{x}}\,E_{1k}^{x}-E_{kk}^{x}\right)
\end{equation}
...
$(a^{\dagger})_{k}^{\hat{x}}$ as a copy of $a^{\dagger}$ acting on the extra-site $\hat{x}$ and on the species $k\in\{2,\ldots,N\}$. \\
...
and where 
\begin{equation}\label{dualityMatrix}
\dd=\sum_{m_{2}^{\hat{x}},\ldots,m_{N}^{\hat{x}}=0}^{\infty}\prod_{k=2}^{N}\left(\frac{\alpha_{k}^{x}}{\sum_{i=1}^{N}\alpha_{i}^{x}}\right)\langle m_{2}^{\hat{x}},\ldots,m_{N}^{\hat{x}}|
\end{equation}
The matrix $R_{x}$ is diagonal. Their elements are related to the inverse of the weights of the reversible measure \eqref{reversibleMeasure}. In particular, to obtain these elements, we have considered the weights of \eqref{reversibleMeasure} when all the parameters $p_{i}=\frac{1}{N}$. Then, the constant $\left(\frac{1}{N}\right)^{\nu}$ has not been considered, since it does not change the duality relation. This $R_{x}$ is called the 'cheap' duality matrix (see \cite{giardina2009duality}). 
Since \eqref{dualityMatrix} is product over sites, \eqref{DualityRelation} is equivalent of proving that 
...
\end{equation}
We perform the prove in three steps: first we will show that matrix \eqref{dualityMatrix} has indeed elements \eqref{dualityElements}; second we will prove \eqref{edgeDualRealtion}; finally we will show\eqref{siteDualRelation}. 
\paragraph{Proof of the elements of the duality matrix}We aim to show that 
...
\begin{align*}
	 &\langle n^{x}|d_{x}\otimes \dd|\xi^{x}\rangle\\&=\langle n_{1}^{x},\ldots,n_{N}^{x}|\sum_{m_{2}^{\hat{x}},\ldots,m_{N}^{\hat{x}}=0}^{\infty}\prod_{k=2}^{N}\left(\frac{\alpha_{k}^{x}}{\sum_{i=1}^{N}\alpha_{i}^{x}}\right)^{m_{k}^{\hat{x}}}\langle m_{2}^{\hat{x}},\ldots,m_{N}^{\hat{x}}|
	 \otimes(\exp{(E_{12}^{x}+\ldots+E_{1N}^{x}}))^{T}R_{x}\\&|r_{1}^{x},\ldots,r_{N}^{x}\rangle_{x} \otimes |q_{2}^{\hat{x}},\ldots,q_{N}^{\hat{x}}\rangle
\end{align*}
On one hand, on the extra-site $\hat{x}$ we have 
\begin{align*}
\sum_{m_{2}^{\hat{x}},\ldots,m_{N}^{\hat{x}}=0}^{\infty}\prod_{k=2}^{N}\left(\frac{\alpha_{k}^{x}}{\sum_{i=1}^{N}\alpha_{i}^{x}}\right)^{m_{k}^{\hat{x}}}\langle m_{2}^{\hat{x}},\ldots,m_{N}^{\hat{x}}||q_{2}^{\hat{x}},\ldots,q_{N}^{\hat{x}}|=\prod_{k=2}^{N}\left(\frac{\alpha_{k}^{x}}{\sum_{i=1}^{N}\alpha_{i}^{x}}\right)^{m_{k}^{\hat{x}}}
\end{align*}
...
\begin{flushright}
    $\square$
\end{flushright}
\paragraph{Proof of \eqref{edgeDualRealtion}}To show \eqref{edgeDualRealtion} we need two ingredients. First is the existence of a similarity transformation between the Hamiltonian $H_{x,y}$ defined in \eqref{edgeHamiltonian} and its transposed. As we will show, this similarity transformation is $R_{x}$ defined in \eqref{Rmatrix}. Second, the possibility of finding a symmetry for the Hamiltonian \eqref{edgeHamiltonian}, exploiting the fact that $H_{x,y}$ is (up to a constant) the coproduct of the second Casimir of $gl(N)$, as shown in \eqref{hamiltonianCasimir}. As we will see, this symmetry is $exp{(E^{x})}\exp{(E^{y})}$ with $E^{x}$ defined in \eqref{equationEx}. \\
We show that 
...
\end{equation}
For fixed $(x,y)\in \mathcal{E}$ and write
\begin{equation}
...
\end{equation}
i.e. $S_{x,y}$ is a symmetry of $H_{x,y}$. \\ Finally, we have that 
\begin{equation}
    \begin{split}
        H_{x,y}^{T}D&=(R_{x}R_{y})H_{x,y}(R_{x}R_{y})^{-1}\left(d_{x}\otimes\dd\right)\left(d_{y}\otimes\mathcal{D}_{\hat{y}}\right)\prod_{z\in V\,:\, z\neq x,y}\left(d_{z}\otimes \mathcal{D}_{\hat{z}}\right)
        \\&=\left(R_{x}\exp{(E^{x})}\otimes \dd\right)\left(R_{y}\exp{(E^{y})}\otimes \mathcal{D}_{\hat{y}}\right)H_{x,y}\prod_{z\in V\,:\, z\neq x,y}\left(d_{z}\otimes \mathcal{D}_{\hat{z}}\right)
        \\&=
        DH_{x,y}
    \end{split}
\end{equation}
 ...
 \end{equation}
For the following of the proof we will need the application of the Hadamard formula with $B=E$ defined in \eqref{equationE} and with $A$ equal to some of the generator $E_{ab}$ of the Lie algebra. Therefore we compute 
\begin{enumerate}
...
\end{split}
\end{equation*}
Inserting the above commutators in \eqref{HadamardFormula}, we obtain \eqref{HT_E1l}.
...
\end{equation*}
Inserting the above commutator in \eqref{HadamardFormula}, we obtain \eqref{HT_Ekl}.
...
\begin{align*}
[E,E_{11}]=\sum_{a=2}^{N}\left(E_{ak}\delta_{11}-E_{\ell 1}\delta_{a1}\right)=\sum_{a=2}^{N}E_{a1};
\end{align*}
  Inserting the above commutator in \eqref{HadamardFormula}, we obtain \eqref{HT_E11}.
\end{enumerate}
...
\sum_{m_{2}^{\widetilde{x}},\ldots,m_{N}^{\widetilde{x}}=0}^{\infty}\prod_{k=2}^{N}\left(\frac{\alpha_{k}^{x}}{\sum_{i=1}^{N}\alpha_{i}^{x}}\right)^{m_{k}^{\widetilde{x}}}\langle m_{2}^{\widetilde{x}},\ldots,m_{k}^{\widetilde{x}},\ldots,m_{N}^{\widetilde{x}}|(a^{\dagger})_{k}^{\widetilde{x}}E_{1k}^{x}
    \end{split}
\end{equation}
where, in the up to last equality, we performed a change of summation variable. Therefore, inserting this last equation in \eqref{siteHadamardII} we obtain 
\begin{equation}
    \begin{split}
     &H_{x,y}^{T}d_{x}\sum_{m_{2}^{\widetilde{x}},\ldots,m_{N}^{\widetilde{x}}=0}^{\infty}\prod_{k=2}^{N}\left(\frac{\alpha_{k}^{x}}{\sum_{i=1}^{N}\alpha_{i}^{x}}\right)^{m_{k}^{\widetilde{x}}}\langle m_{2}^{\widetilde{x}},\ldots,m_{N}^{\widetilde{x}}|
\\=&
