...
Currents will arise when at least two reservoirs have different densities. 
	{\color{blue}
\begin{remark}[Should we add this?]
		In the most general case we have reversibility if 
		\begin{equation*}
			\alpha_{k}^{x}\left(|\alpha^{y}|-\alpha_{k}^{y}\right)=\alpha_{k}^{y}\left(|\alpha^{x}|-\alpha_{k}^{x}\right)\qquad \forall x,y\in V,\quad \forall k\in \{1,\ldots,N\}
		\end{equation*}
		where
		\begin{equation*}
			|\alpha^{x}|=\sum_{a=1}^{N}\alpha_{a}^{x}
		\end{equation*}
		this is the multi-species counterpart of $\alpha\delta=\beta\gamma$ of the SEP case (see Section 3.2 of \cite{carinci2013duality}) \\
		Should we say that or can we restrict to the case where $\alpha_{a}^{x}=\alpha_{a}$?
		 Maybe we put a remark. It does not affect duality, since the matrix $R$ that we define in Section \ref{sectionDuality} does not depend on the density of the multinomials.
		\end{remark}
	}
...
\end{corollary}
\textbf{Proof of \ref{Corollary-NESS-ABS}:}
...
\begin{equation}			\mathbbm{1}_{\{\sigma_{x}=N\}}=1-\sum_{a_{x}=1}^{N-1}\mathbbm{1}_{\{\sigma_{x}=a_{x}\}}\dt
\end{equation}
...
+&\left(\prod_{x=1\,:\,\tau_{x}\neq N}^{L}\mathbbm{1}_{\{\sigma_{x}=\tau_{x}\}}\right)(-1)^{L-\ell}\sum_{1\leq q_{1}<\ldots<q_{L-\ell}\leq L}\left(\prod_{k=1}^{L-\ell}\delta_{\tau_{q_{k}},N}\right)\sum_{a_{q_{1}},\ldots,a_{q_{L-\ell}}=1}^{N-1}\prod_{k=1}^{L-\ell}\mathbbm{1}_{\{\sigma_{q_{k}}=a_{q_{k}}\}}\dt
\end{align} 
...
\paragraph{Proof of \eqref{ResulsBasis}} 
...
\begin{align}
	...
	=&\delta_{\tau_{x},N}\delta_{\sigma_{x},N}+(1-\delta_{\tau_{x},N})(1-\delta_{\sigma_{x},N})+\beta_{\tau_{x}}\delta_{\sigma_{x},N}(1-\delta_{\tau_{x},N})\dt
\end{align}
%\begin{equation}
...
\begin{equation}
	...
	=&\begin{cases}
		...
		1\qquad &\text{if}\quad \tau_{x}= N\dt
	\end{cases}
\end{split}
\end{equation}
...
=&\frac{1}{(L+1)!}\sum_{\sigma_{1}=1}^{N}\ldots\sum_{\sigma_{L}=1}^{N}\prod_{x=1}^{L}\left(\alpha_{\tau_{x}}-\beta_{\tau_{x}}\right)^{1-\delta_{\sigma_{x},N}}\left(1+\sum_{j=x}^{L}\delta_{\sigma_{j},N}\right)\\&\left[\delta_{\tau_{x},N}\delta_{\sigma_{x},N}+(1-\delta_{\tau_{x},N})(1-\delta_{\sigma_{x},N})+\beta_{\tau_{x}}\delta_{\sigma_{x},N}(1-\delta_{\tau_{x},N})\right]\dt
...
and
\begin{align}
	\sum_{\sigma_{1}=1}^{N}\ldots\sum_{\sigma_{L}=1}^{N}\prod_{x=1}^{L}\left(1+\sum_{j=x}^{L}\delta_{\sigma_{j},N}\right)=\sum_{c_{1}=\delta_{\tau_{1},N}}^{1}\ldots\sum_{c_{L}=\delta_{\tau_{L},N}}^{1}\prod_{x=1}^{L}\left(1+\sum_{j=x}^{L}c_{j}\right)\dt
\end{align}
...
\begin{equation}
	\mathbb{E}_{\mu}\left[\prod_{k=1}^{m}\mathbbm{1}_{\{\tau_{x_{k}}=a_{k}\}}\right]=\sum_{\bm{\tau}\in \Omega^{'}}\left(\prod_{k=1}^{m}\mathbbm{1}_{\{\tau_{x_{k}}=a_{k}\}}\right)\langle\bm{\tau}|\Psi\rangle\dt%=\sum_{\bm{\tau}\in \Omega^{'}}\left(\prod_{i=1}^{m}\mathbbm{1}_{\{\tau_{y_{i}}=s_{i}\}}\right)\Psi(\bm{\tau})
\end{equation}
...
\begin{align}
	\mathbb{E}_{\mu}\left[\prod_{k=1}^{m}\mathbbm{1}_{\{\tau_{x_{k}}=a_{k}\}}\right]=&\sum_{c_{1}=0}^{1}\ldots\sum_{c_{m}=0}^{1}\frac{(L+1-\sum_{a=1}^{m}c_{a})!}{(L+1)!}\prod_{k=1}^{m}(\alpha_{a_{k}}-\beta_{a_{k}})^{c_{k}}\beta_{a_{k}}^{1-c_{k}}\left(2+L-x_{k}-\sum_{j=k}^{m}c_{j}\right)^{c_{k}}\nonumber\\
...
\end{equation}
 Therefore, we obtain 
 	...
 	\Psi^{'}(\bm{\tau})=\mathbb{E}_{\mu}\left[\prod_{x=1}^{L}\left(\mathbbm{1}_{\{\sigma_{x}=\tau_{x}\}} \right)^{(1-\delta_{\tau_{x},N})}\right]\dt
 \end{equation} 
...
we have that 
\begin{align}
	\Psi^{'}(\bm{\tau})=&\mathbb{E}_{\mu}\left[\prod_{x=1}^{L}\left(\mathbbm{1}_{\{\sigma_{x}=\tau_{x}\}} \right)^{(1-\delta_{\tau_{x},N})}\right]=\mathbb{E}_{\mu}\left[\prod_{k=1}^{\ell}\mathbbm{1}_{\{\sigma_{x_{k}}=\tau_{x_{k}}\}}\right]=\sum_{\bm{\sigma}\in \Omega_{\ell}^{'}(\bm{\tau})}\mu(\bm{\sigma})=\mu_{x_{1},\ldots,x_{\ell}}(\bm{\tau})\dt
\end{align} 
...
\begin{remark}%[Correlation as the steady state of a $m$-chain ]
	...
	\begin{equation}
		\Psi(\bm{\tau})=\Psi^{'}(\bm{\tau})=\mathbb{E}_{\mu}\left[\prod_{x=1}^{L}\mathbbm{1}_{\{\sigma_{x}=\tau_{x}\}}\right]
	\end{equation}
...
\begin{comment}
	...
	\begin{equation}
		\Psi(\bm{\tau})=\langle \bm{\tau}|\Psi\rangle= \mu(\bm{\tau})\dt
	\end{equation}
...
%\end{equation}
\textbf{Proof of \eqref{NESS-SSEP}:} the proof follows from \eqref{implicit-PSI-PSIp} and \eqref{PsiI-N2}. 
\begin{flushright}
	$\square$
\end{flushright}
\begin{comment}
...
\begin{equation}
	\frac{d \rho_{a}(x,t)}{dt}=\mathbb{E}_{\mu}\left[\mathcal{L}^{rd}n_{a}^{x}(t)\right]\dt
\end{equation} 
...
The duality function is given by
\begin{equation}
	D(\bm{n},\bm{\xi})=\prod_{a=1}^{N-1}\left(\frac{\alpha_{a}}{|\alpha|}\right)^{\xi_{a}^{0}}\left(\prod_{x=1}^{L}\frac{(\nu -\sum_{a=1}^{N-1}\xi_{a}^{x})!}{\nu!}\prod_{a=1}^{N-1}\frac{n_{a}^{x}!}{(n_{a}^{x}-\xi_{a}^{x})!}\right)\prod_{a=1}^{N-1}\left(\frac{\beta_{a}}{|\beta|}\right)^{\xi_{a}^{L+1}}\dt
\end{equation}
