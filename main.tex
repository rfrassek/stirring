...
at the boundaries and with maximum one particle per site the process is integrable. Based on the results of \cite{vanicat2017exact},
...
Boundary-driven particle systems are paradigmatic models for non-equilibrium statistical mechanics. In a boundary-driven system, the model is put in contact with reservoirs at its ``boundaries'' imposing different particle densities, and a current is generated through the system. In the long time limit, a non-equilibrium steady state sets in, with a stationary value of the current. {\color{red} The aim of this paper is to study the mathematical aspects of the stationary measure of boundary-driven {\em multispecies} particle systems \cite{schutz2017kardar, vanicat2017exact, crampe2016integrable, finn2018matrix, floreani2022switching, casini2022uphill, casini2023density, redig2022ergodic, redig2023equilibrium, chatterjee2023multi, zhou2021orthogonal, franceschini2022orthogonal,floreani2023non}.}
...
For the single species case,  the solution of the exclusion process with open boundaries \cite{derrida1993exact,1993JSP....72..277S} has been of crucial importance in our understanding of the structural properties of non-equilibrium state, such as long-range correlations \cite{spohn1983long} and non-local density large deviation functions \cite{derrida2007non,derrida1998exact} and current large deviations \cite{mallick2022exact,bodineau2005current}. The results available for the exclusion process are rooted in the algebraic symmetries of the corresponding integrable quantum spin chain, i.e. the XXX Heisenberg Hamiltonian with ${su}(2)$ spins of value $s=1/2$.
In this paper, we investigate the multispecies analog of the exclusion process in the boundary-driven context \cite{vanicat2017exact}, i.e. the {\em  multispecies stirring process with open boundaries}. See also \cite{crampe2016integrable, finn2018matrix} for the asymmetric setup.  The bulk part of the model with $N$ species is associated with the integrable ${gl}(N)$ 
Heisenberg XXX Hamiltonian with integrable non-diagonal open boundaries.
...
In general, this is a difficult task. However,  for the case of integrable systems they can be computed exactly in closed-form. 
For recent examples in the framework of singlespecies systems, see \cite{frassek2020eigenstates, frassek2022exact}.  
{\color{red}
...
Here the computation of the absorption probabilities relies on the extension of the  Matrix Product Ansatz \cite{derrida1993exact} to multiple species \cite{vanicat2017exact},{\color{red}
...
\todo{Exchange hole with void? Also can the remark be just text?}
\end{remark}
...
each reservoir at site $x$ {\color{red}wants to fix}\todo{?} its own set of densities $\rho^{x}=(\rho_{1}^{x},\ldots,\rho_{N}^{x})$ with
\begin{equation}
\label{rhox}
...
In this section we specialize the multispecies stirring process to the geometry of the one-dimensional chain with sites $\{1,\ldots,L\}$ where two reservoirs act at the boundary sites $1$ and $L$.  The reservoirs, exchanging particle with the external environment, put the chain out of equilibrium. This process was studied previously in \cite{vanicat2017exact}.

The Hamiltonian that we consider here is obtained from \eqref{OriginalHamiltonian} assuming that the conductances are
\begin{equation}
