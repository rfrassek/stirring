...
\begin{equation}
	WH_{0}=HW
\end{equation}
Formula \eqref{W-extended} comes from standard perturbation theory. However, since the process is integrable it is possible to express $W$ in function of the conserved charges of the system. This expression, as it will be show in the following sections, allows to find explicit expression of the eigenvector by the Bethe ansatz and also allows to retrieve again the steady state of $H^{''}$ by applying $W$ to the ground state. To find it, we use the Quantum Inverse Scattering Method (QISM). We first present the simpler case of $gl(2)$, then we try to adapt the proof the general $gl(N)$. 
...
%\end{equation}
\item For $B_{j}(x)$  $\forall j=2,\ldots,L$ 
\begin{equation}
		B_{j}^{2L-1}=	2e_{j1}^{tot}e_{11}^{tot}-2\sum_{a,b=1\,:\, a>b}^{L}\,\sum_{k=1}^{N} e_{jk}^{[a]}e_{k1}^{[b]}
\end{equation}
\begin{comment}
\begin{equation}
...
\end{equation}
Power expanding the matrix $W$ with respect to $\Delta=(\alpha-\beta)$ and making it act on 
...
\begin{equation}\label{exchangeQk}
	...
	\end{split}
\end{equation}
We use the following power expansion
\begin{equation}\label{veryUsefulSeries}
	(A-B)^{-1}=\sum_{n=0}^{\infty}(A^{-1}B)^{n}A^{-1}
\end{equation}
We obtain
...
Following the result in the simpler $gl(2)$ case, we aim to generalize it to higher rank chains. Repeating the computations we find that 
\begin{equation}\label{W-extended}
	W= \lim_{\epsilon\to 0}\epsilon\sum_{\bm{m}\,:\,|\bm{m}|=L}  \left(\Lambda_{\bm{m}}+\epsilon-H\right)^{-1}|\Psi_{\bm{m}}^{0}\rangle \langle \Psi_{\bm{m}}^{0}|
\end{equation}
...
\begin{equation}
	...
	\end{split}
\end{equation}
 Recalling the expression of $\mathcal{Q}^{2L}$ given in \eqref{Q-nonDiag} we write the multi-variable power expansion with respect to $\Delta_{i} B_{i}^{2L-1}$ for all $i\in\{1,\ldots,N-1\}$. We obtain that, using the multi-variate version of \eqref{veryUsefulSeries},
\begin{equation}
	\begin{split}
	...
	\end{split}
\end{equation} 
Here $SYM$ is the following transformation. Introduce, the following $i_{j}$ with $j=1,\ldots,k_{1}+\ldots+k_{N}$ indices, such that 
\begin{equation}
	\begin{split}
...
\begin{equation}
	\begin{split}
&SYM_{k_{1},\ldots,k_{N-1}}\left(\left\{\left(\Lambda_{\bm{m}}^{Q}+\epsilon-Q_{2L}^{0}\right)^{-1}B_{1}^{2L-1}\right\},\ldots,\left\{\left(\Lambda_{\bm{m}}^{Q}+\epsilon-Q_{2L}^{0}\right)^{-1}B_{N-1}^{2L-1}\right\}\right)\\&:=\frac{1}{k_{1}!\cdots k_{N-1}!}\sum_{\sigma\in Sym}A_{\sigma(i_{1})}\cdots A_{\sigma(i_{k_{1}+\ldots+k_{N-1}})}
	\end{split}
\end{equation}
