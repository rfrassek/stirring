...
with open boundaries. 
In words, the process is described as follows. Each site
...
{\color{blue} \paragraph{Example for an $L=2$ chain.} To clarify equation \eqref{implicit-mass-function} we report the example of a chain with $L=2$, where we compute the probability mass functions of three configurations. Without loss of generality we assume that the particles are always of species $1$. We have that 
 \begin{itemize}
 	\item probability of two occupied site:
 	\begin{equation}
 		\mu(1,1)=\mathbb{E}\left[\mathbbm{1}_{\{Y_{1}=1\}}\mathbbm{1}_{\{Y_{2}=1\}}\right]\dt
 	\end{equation}
 	Here the right-hand-side corresponds to $b=0$ and average can be computed by Corollary~\ref{Corolollary-ABS_Corr-abstract} with $m=2$, $x_{1}=1,\;x_{2}=2$ and $a_{1}=a_{2}=1$.
 	...
 	\end{align}
 	Here the first term on the right-hand-side corresponds to $b=0$ and since $\bm{\tau}=(N,N)$ we obtain $1$. The second and the third term on the right-hand-side correspond to $b=1$. They can be computed by Corollary~\ref{Corolollary-ABS_Corr-abstract} with $m=1$ and by fixing $x_{1}=1$, $a_{1}=1,\ldots,N-1$ and $x_{2}=2$, $a_{2}=1,\ldots,N-1$ respectively. The fourth term on the right-hand-side corresponds to $b=2$ and it can be computed by Corollary~\ref{Corolollary-ABS_Corr-abstract} with $m=2$, $x_{1}=1,\,x_{2}=2$ and $a_{1},a_{2}=1,\ldots,N-1$. 
 \end{itemize}}
...
Therefore, using the linearity of the expectation and introducing the index $b\in \{0,\ldots,L\}$ {we rewrite the sum in equation \eqref{sum} as} \eqref{implicit-mass-function}.  {Indeed, when $b>L-\ell$ we have vanishing terms in the sum \eqref{implicit-mass-function}.}
\begin{flushright}
	$\square$
\end{flushright}
...
\end{equation}
		...
		\begin{align}\label{implicit-PSI-PSIp}
			\Psi(\bm{\tau})=&\langle \bm{\tau}|S_{1}^{-1}|\Psi^{'}\rangle=\sum_{\bm{\sigma}\in \Omega^{'}}\Psi^{'}(\bm{\sigma})\prod_{x=1}^{L}\left(\delta_{\tau_{x},\sigma_{x}}+(-1)\delta_{\tau_{x},N}\sum_{a=1}^{N-1}\delta_{\sigma_x,a}\right)\nonumber\\
			...
			=&\sum_{b=0}^{L}(-1)^{b}\sum_{1\leq q_{1}<\ldots<q_{b}\leq L}\left(\prod_{k=1}^{b}\delta_{\tau_{q_{k}},N}\right)\sum_{a_{1},\ldots a_{b}=1}^{N-1}\Psi^{'}(\tau_{1},\ldots,\underset{\uparrow}{a_{1}}_{q_{1}},\ldots,\underset{\uparrow}{a_{b}}_{q_{b}},\ldots,\tau_{L})
		\end{align} 
...
\begin{equation}
	...
	\end{cases}\dt
\end{equation}
{\color{blue}As already pointed out,} the duality function is \eqref{dualityElements}, adapted to the chain. Therefore, we have that
\begin{equation}
	D(\bm{n},\bm{\xi})= \frac{\prod_{k=1}^{m}n_{a_{k}}^{x_{k}}}{C(\nu,m)}\dt
\end{equation}
where $C(\nu,m)$ is a constant that depends on $m$ and {\color{blue}on the maximal occupancy} per site $\nu$. 
...
{\color{blue} We recall a general fact. Considering a generic $N-1$ species Markov process $(\bm{n}(t))_{t\geq 0}$ (where in the spirit of Section~\ref{sec1}, $n_{A}^{x}$ denotes the number of particles or the number of holes at site $x$), we define the average density occupation of species $a\in\{1,\ldots,N-1\}$ at site $x$ and at time $t$ as
\begin{equation}
...
\end{equation} }
In \cite{casini2022uphill} {\color{blue}it has been defined a process with two species $a=1,2$ and the holes (denoted by $0$ in that context)} whose evolution equation for the average occupation $\rho_{a}(x,t)$ of species $a\in \{1,2\}$ is given by a difference-differential equation with two discrete Laplacians (one for each species, multiplied by a proper diffusivity constant) and a linear reaction term (mutation), i.e.
...
\begin{equation}
	...
	\end{cases}\dt
\end{equation} Now we consider the elements $\langle\bm{\sigma}|D|\xi(\bm{\tau})\rangle$ of the duality matrix $D$ and, {\color{blue} since no dual particles are present in the extra-sites $0$ and $L+1$}, we have
...
Let $\ell$ be the number of sites {\color{blue}occupied by a particle} in the configuration $\bm{\tau}$, i.e.  $\ell=\sum_{x=1}^{L}(1-\delta_{\tau_{x},N})$. We denote by $x_{1},\ldots,x_{\ell}$, such that $x_{k}<x_{k+1}$ for all $k\in \{1,\ldots,\ell-1\}$, the coordinates where a particle of any species is present. Finally, we call 
\begin{equation}
