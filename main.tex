...
\end{equation}
In the long-time limit the dual process voids the chain, i.e. all particles of types $\{1,\ldots,N-1\}$ are eventually absorbed at the extra-sites $0$ and $L+1$ and replaced by types $N$. We also notice that, up the the bosonic creation operators $(\mathbf{a}^{+})_{a}$, the dual boundary Hamiltonians \eqref{boundary-H-dual-chain} are triangular. This simplification will be crucial in Sections \ref{sectionIntegrabiliy}.\\
...
%\end{equation}
%\\
Using the above duality elements, we can compute the  $m$-point correlations between non-empty particles using the absorption probabilities. Therefore, to determine the non-equilibrium steady state correlations of the integrable multi-species stirring process, it is enough to compute these absorption probabilities.  %For arbitrary $m\in \mathbb{N}$ such that $m<L$, we fix $a_{k}\in\{1,\ldots,N-1\}$ and $x_{k}\in\{1,\ldots,L\}$ such that $x_{k}<x_{k+1}$ for every $k\in\{1,\ldots,m\}$. 
...
\begin{equation}
\mathbb{E}_{\mu^{\text{NESS}}}\left[\prod_{k=1}^{m}\mathbbm{1}_{\{\tau_{x_{k}}=a_{k}\}}\right]=\sum_{t_{1}=0}^{1}\ldots \sum_{t_{m}=0}^{1}\prod_{k=1}^{m}\alpha_{a_{k}}^{t_{k}}\beta_{a_{k}}^{1-t_{k}}\mathcal{P}(t_{1},\ldots,t_{m},\bm{\xi})
\end{equation}
where
\begin{equation}\label{absProbabilitiesIntegrable}
	\mathcal{P}(t_{1},\ldots,t_{m},\bm{\xi}):=\mathbb{P}\left(\bm{\xi}(\infty)=\sum_{k=1}^{m}\left(t_{k}\delta^{0}_{a_k}+(1-t_{k})\delta^{L+1}_{a_k}\right)\lvert \bm{\xi}(0)=\bm{\xi}\right)
\end{equation}
is the  probability that  $t_{k}$ particles of species $a_{k}$ (initially positioned at site $x_{k}$) are absorbed at the extra site $\{0\}$. The dual configuration is $\bm{\xi}=\sum_{k=1}^{m}\delta_{a_{k}}^{x_{k}}$, meaning that 
\begin{equation}
	\begin{cases}
...
\end{proposition}
\textbf{Proof}:
For any $\bm{\tau}\in \Omega$ the elements of the duality matrix defined in \eqref{ElementsDualityMatrixChain_1} read
\begin{equation}
...
\mathbb{E}_{\mu^{\text{NESS}}}\left[\prod_{k=1}^{m}\mathbbm{1}_{\{\tau_{x_{k}}=a_{k}\}}\right]&=\lim_{t\to \infty}\mathbb{E}_{\bm{\tau}}\left[D(\bm{\tau}(t),\bm{{\xi}})\right]=\lim_{t\to \infty}\mathbb{E}_{\bm{\xi}}\left[D(\bm{\tau},\bm{{\xi}}(t))\right]
	\\&=\sum_{t_{1}=0}^{1}\ldots \sum_{t_{m}=0}^{1}\prod_{k=1}^{m}\alpha_{a_{k}}^{t_{k}}\beta_{a_{k}}^{1-t_{k}}\mathcal{P}(t_{1},\ldots,t_{m},\bm{\xi})
\end{split}
\end{equation}
\begin{flushright}
	$\square$
...
\begin{equation}\label{bulk}
	\left[X_{a},X_{b}\right]=(\alpha_{a}-\beta_{a})X_{b}-\lambda_{b}X_{a}\qquad\forall a,b=1,\ldots,N
\end{equation}
...
\begin{remark} Only $N-1$ of the $N$ equations \eqref{leftBoundary}  are independent. This can be seen by summing them over the species $a\in \{1,\ldots,N\}$, i.e.
	\begin{equation}
		\sum_{a=1}^{N}	\langle V^{*}|\left(\alpha_{a}(X_{1}+\ldots+X_{N})-X_{a}\right)=0,%\sum_{a=1}^{N}(\alpha_{a}-\beta_{a})\langle V^{*}|=0
	\end{equation}
...
The MPA gives an abstract form of the non-equilibrium steady state in terms of an algebra of operators. However, the computation of exact correlations are in general involving. 
...
Consider $m\in \{1,\ldots,L\}$ points with coordinates given by $(x_{1},\ldots,x_{m})$, such that $x_{k}<x_{k+1}$ $\forall k\in \{1,\ldots,m-1\}$. We assume that in each of these points there is a particle of species $a_{k}\in \{1,\ldots,N-1\}$. Then the $m$-point correlations with respect to the non-equilibrium steady state measure $\mu^{NESS}$ are given by 
\begin{equation}\label{exactCorrelations}
	\begin{split}
		&\mathbb{E}_{\mu^{\text{NESS}}}\left[\prod_{k=1}^{m}\mathbbm{1}_{\{\tau_{x_{k}}=a_{k}\}}\right]\\=&\sum_{t_{1},\ldots,t_{m}=0}^{1}\left(\prod_{k=1}^{m}\alpha_{a_{k}}^{t_{k}}\beta_{a_{k}}^{1-t_{k}}\right)\sum_{c_{1}=t_{1}}^{1}\ldots\sum_{c_{m}=t_{m}}^{1}f(L,c_{1},\ldots,c_{m})\left(\prod_{k=1}^{m}(-1)^{c_{k}-t_{k}}g_{k}(L,x_{k},c_{k},\ldots,c_{m})\right)
		\end{split}
\end{equation}
where
...
\end{equation}
Moreover, the absorption probabilities defined in \eqref{absProbabilitiesIntegrable} are
...
We consider the average with respect to $\mu^{NESS}$ of the occupation variable of the species $a_{1}\in \{1,\ldots,N-1\}$ at coordinate $x_{1}\in \{1,\ldots,L\}$. %This reads
%\begin{equation}
%\mathbb{E}_{\mu^{NESS}}\left[\mathbbm{1}_{\{\tau_{x_{1}}=a_{1}\}}\right]=\sum_{t_{1}=0}^{1}\alpha_{a_{1}}^{t_{1}}\beta_{a_{1}}^{1-t_{1}}\sum_{c_{1}=t_{1}}^{1}f(L,c_{1})g_{a_{1}}(L,x_{1},c_{1})\dt
%\end{equation}
Using \eqref{abs-probabilities} we obtain the absorption probabilities 
\begin{equation}
	\mathcal{P}(0,\bm{\xi})=1-\frac{(L+1-x_{1})}{(1+L)}\qquad \mathcal{P}(1,\bm{\xi})=\frac{(L+1-x_{1})}{(1+L)}
\end{equation} 
where $\bm{\xi}=\delta_{a_{1}}^{x_{1}}$. Then, using \eqref{exactCorrelations}, we have
\begin{equation}\label{one-pts-corr}
	 \mathbb{E}_{\mu^{NESS}}\left[\mathbbm{1}_{\{\tau_{x_{1}}=a_{1}\}}\right]=\frac{(L+1-x_{1})}{L+1}(\alpha_{a_{1}}-\beta_{a_{1}})+\beta_{a_{1}}\dt
\end{equation}
\paragraph{Two-point correlations}
We consider the average with respect to $\mu^{NESS}$ of the occupation variable of the species  $a_{1},a_{2}\in \{1,\ldots,N-1\}$ at coordinates $x_{1},x_{2}\in \{1,\ldots,L\}$ such that $x_{1}< x_{2}$. 
%\begin{equation}
%	\begin{split}
%		 \mathbb{E}_{\mu^{NESS}}\left[\mathbbm{1}_{\{\tau_{x_{1}}=a_{1}\}}\mathbbm{1}_{\{\tau_{x_{2}}=a_{2}\}}\right]= \sum_{t_{1}=0}^{1}\sum_{t_{2}=0}^{1}\left(\prod_{k=1}^{2}\alpha_{a_{k}}^{t_{k}}\beta_{a_{k}}^{1-t_{k}}\right)\sum_{c_{1}=t_{1}}^{1}\sum_{c_{2}=t_{2}}^{1}f(L,c_{1},c_{2})\prod_{j=1}^{2}g_{k}(L,x_{j},c_{j},c_{2})\dt
%	\end{split}
%\end{equation}
Using \eqref{abs-probabilities} we obtain the absorption probabilities
\begin{equation}
...
\begin{equation}
	\begin{split}
	 \mathbb{E}_{\mu^{NESS}}\left[\mathbbm{1}_{\{\tau_{x_{1}}=a_{1}\}}\mathbbm{1}_{\{\tau_{x_{2}}=a_{2}\}}\right]&=\beta_{a_{1}}\beta_{a_{2}}+\frac{(L+1-y_{1})}{L+1}(\alpha_{a_{1}}-\beta_{a_{1}})\beta_{a_{2}}+\frac{(L+1-x_{2})}{L+1}(\alpha_{a_{2}}-\beta_{a_{2}})\beta_{a_{1}}\\&+\frac{(L-x_{1})(L+1-x_{2})}{L(L+1)}(\alpha_{a_{1}}-\beta_{a_{1}})(\alpha_{a_{2}}-\beta_{a_{2}})\dt
	\end{split}
\end{equation}
Therefore, we compute the second cumulant, i.e. the two-point connected correlation
\begin{equation}\label{two-pts-corr}
	\begin{split}
		 \mathbb{E}_{\mu^{NESS}}\left[\mathbbm{1}_{\{\tau_{x_{1}}=a_{1}\}}\mathbbm{1}_{\{\tau_{x_{2}}=a_{2}\}}\right]-\prod_{k=1}^{2}\mathbb{E}_{\mu^{NESS}}\left[\mathbbm{1}_{\{\tau_{x_{k}}=a_{k}\}}\right]=-\frac{x_{1}(L-x_{2}+1)}{L(L+1)^{2}}\left(\alpha_{a_{1}}-\beta_{a_{1}}\right)\left(\alpha_{a_{2}}-\beta_{a_{2}}\right)\dt
	\end{split}
\end{equation}
\paragraph{Three-point correlations}
We consider the average with respect to $\mu^{NESS}$ of the occupation variable of the species  $a_{1},a_{2},a_{3}\in \{1,\ldots,N-1\}$ at coordinates $x_{1},x_{2},x_{3}\in \{1,\ldots,L\}$ such that $x_{1}<x_{2}<x_{3}$. Using \eqref{abs-probabilities} we compute the absorption probabilities 
\begin{equation}
	\begin{split}
...
\begin{align}\label{three-pts-corr}
 \mathbb{E}_{\mu^{NESS}}\left[\prod_{k=1}^{3}\mathbbm{1}_{\{\tau_{x_{k}}=a_{k}\}}\right]&=\beta_{a_{1}}\beta_{a_{2}}\beta_{a_{3}}+\frac{(L+1-x_{1})}{(L+1)}(\alpha_{a_{1}}-\beta_{a_{1}})\beta_{a_{2}}\beta_{a_{3}}
	\\&+\nonumber
...
\begin{align}
	&\mathbb{E}_{\mu^{NESS}}\left[\prod_{k=1}^{3}\mathbbm{1}_{\{\tau_{x_{k}}=a_{k}\}}\right]+2\prod_{k=1}^{3}\mathbb{E}_{\mu^{NESS}}\left[\mathbbm{1}_{\{\tau_{x_{k}}=a_{k}\}}\right]\nonumber\\-&\mathbb{E}_{\mu^{NESS}}\left[\mathbbm{1}_{\{\tau_{x_{1}}=a_{1}\}}\mathbbm{1}_{\{\tau_{x_{2}}=a_{2}\}}\right]\mathbb{E}_{\mu^{NESS}}\left[\mathbbm{1}_{\{\tau_{x_{3}}=a_{3}\}}\right]\nonumber\\
	-&\mathbb{E}_{\mu^{NESS}}\left[\mathbbm{1}_{\{\tau_{x_{1}}=a_{1}\}}\mathbbm{1}_{\{\tau_{x_{3}}=a_{3}\}}\right]\mathbb{E}_{\mu^{NESS}}\left[\mathbbm{1}_{\{\tau_{x_{2}}=a_{2}\}}\right]\nonumber\\
	-&\mathbb{E}_{\mu^{NESS}}\left[\mathbbm{1}_{\{\tau_{x_{2}}=a_{2}\}}\mathbbm{1}_{\{\tau_{x_{3}}=a_{3}\}}\right]\mathbb{E}_{\mu^{NESS}}\left[\mathbbm{1}_{\{\tau_{x_{1}}=a_{1}\}}\right]\nonumber
	\\
...
Therefore, we obtain 
\begin{align*}
	\langle V^{*}|\widetilde{X}_{N}^{n}|V\rangle&=%\frac{\Gamma(2+n_{N})}{\Gamma(2+n_{N}-1)}\langle V^{*}|\widetilde{X}_{N}^{n_{N}-1}|V\rangle=\langle V^{*}|\widetilde{X}_{N}^{n_{N}}|V\rangle=\frac{\Gamma(2+n_{N})}{\Gamma(2+n_{N}-1)}\frac{\Gamma(2+n_{N}-1)}{\Gamma(2+n_{N}-2)}\langle V^{*}|\widetilde{X}_{N}^{n_{N}-2}|V\rangle\\&=
...
\begin{equation}
	\mathbb{E}_{\mu^{\text{NESS}}}\left[\prod_{k=1}^{m}\mathbbm{1}_{\{\tau_{x_{k}}=a_{k}\}}\right]=\sum_{\bm{\tau}\in \Omega}\left(\prod_{k=1}^{m}\mathbbm{1}_{\{\tau_{x_{k}}=a_{k}\}}\right)\langle\bm{\tau}|\Psi\rangle%=\sum_{\bm{\tau}\in \Omega}\left(\prod_{i=1}^{m}\mathbbm{1}_{\{\tau_{y_{i}}=s_{i}\}}\right)\Psi(\bm{\tau})
\end{equation}
We observe that 
\begin{equation}
...
We call $\mu^{NESS}$ the stationary measure of the the process $(\bm{n}(t))_{t\geq 0}$. For every $m\in\{1,\ldots,L\}$ we consider the dual configuration  $\bm{\xi}=\sum_{k=1}\delta_{a_{k}}^{x_{k}}$ with $x_{k}\in\{1,\ldots,L\}$ and $a_{k}\in \{1,\ldots,N-1\}$ for every $k\in \{1,\ldots,m\}$. The following  equalities holds using duality relation \eqref{duality-expectation}:
\begin{equation}\label{ExptationSS}
	\begin{split}
		\mathbb{E}_{\mu^{NESS}}\left[D(\bm{n}(t),\bm{\xi})\right]&=\lim_{t\to\infty}\mathbb{E}_{\bm{n}}\left[D(\bm{n}(t),\bm{\xi})\right]=\lim_{t\to\infty}\mathbb{E}_{\bm{\xi}}\left[D(\bm{n},\bm{\xi}(t))\right]
		\\&=
	...
	\end{split}
\end{equation}
where  $\rho_{\alpha}^{L}=\frac{\alpha_{a}}{|\alpha|}$, $\rho_{\alpha}^{R}=\frac{\beta_{a}}{|\beta|}$ and where 
\begin{equation}\label{Pass} 
	\mathcal{P}(i_{1},\ldots,i_{N},\bm{\xi})	=\mathbb{P}\left(\bm{\xi}(\infty)=\sum_{a=1}^{N-1}\left(t_{a}\delta_{0}+(|\xi_{a}|-t_{a})\delta_{L+1}\right)| \bm{\xi}(0)=\bm{\xi}\right)
\end{equation}
are the absorption probabilities for the non-integrable chain, i.e. the probabilities that $t_{a}$ particles of species $a$ are absorbed at $\{0\}$. Here we denoted $|\xi_{a}|=\sum_{x=1}^{L}\xi_{a}^{x_{k}}$.
The duality function \eqref{dualityElements} computed in the dual configurations $\bm{\xi}$ is given by 
\begin{equation}
	D(\bm{n},\bm{\xi})= \frac{\prod_{k=1}^{m}n_{a_{k}}^{x_{k}}}{(\nu-m+1)}\dt
\end{equation}
Therefore, up to a constant, $\mathbb{E}_{\mu^{\text{stat}}}\left[D(\bm{n},\bm{\xi})\right]$ gives the non-equilibrium steady state $m-$points correlations. This expectation can be computed by \eqref{ExptationSS} once the absorption probabilities are known. However, these absorption probabilities fulfil some difference equations that are not easy to solve. For example, we report in Appendix \ref{appendix-1pts-general} the solution for the simplest case of one point correlations that is exactly solvable, since the single dual particle behaves as an independent random walker. 
...
We only report the final result. The details are written in Appendix \ref{appendix-RD}. For arbitrary site $x\in \{2,\ldots,L-1\}$, the action on the occupation variable $f(\bm{n})=n_{d}^{x}$ of particle of type $a\in \{1,\ldots,N-1\}$ reads
...
\end{equation}
where $\Delta_{L}n_{a}^{x}= n_{a}^{x+1}+n_{x}^{x-1}-2n_{a}^{x}$. \\For $x=1$ and for all $a\in \{1,\ldots,N-1\}$ we have 
...
\end{equation}
On the occupation variable $n_{a}^{L}$ the action is similar, but with the upper case $1$ replaced by $L$.\\ In the spirit of \cite{casini2022uphill} we can define the average density occupation of species $d$ at site $x$ as
\begin{equation}
...
\end{equation}
where 
$\mathcal{L}_{x,x+1}^{rd}$ is defined in \eqref{edgeGeneratorRD} and, $\widetilde{\mathcal{L}}_{\text{left}}^{rd}$ and $\widetilde{\mathcal{L}}_{\text{right}}^{rd}$ are again the absorbing dual generators $\widetilde{\mathcal{L}}_{x}$ defined in \eqref{siteDualGenerator}, with the choice of the reservoirs parameters \eqref{boundaryParamRD}.
...
\begin{equation}
	H^{rd}=H_{\text{left}}+\sum_{x=1}^{L-1}\left(\sigma_{11}\mathcal{H}_{x,x+1}+\sigma_{12}\sum_{c=1}^{N-2}\mathcal{H}_{x,x+1}^{c}+(\Upsilon-2\sigma_{12})\mathcal{H}_{x,x+1}^{m}\right)+H_{\text{right}}
\end{equation}
where $\mathcal{H}_{x,x+1}$ and $H_{\text{left}},H_{\text{right}}$ are \eqref{edgeHamiltonian}  and \eqref{siteHamiltonian} (with the choice of the boundary parameters done in \eqref{boundaryParamRD}) respectively, while
\begin{equation}
...
%\paragraph{Duality for $H_{x,x+1}^{c}$}
 We compute the commutator between $\mathcal{H}_{x,x+1}^{c}$ and $\exp{(E^{x})}\exp{(E^{x+1})}$ with $E$ defined in \eqref{EquationEx}. Using \eqref{eq:comgl}, the bilinearity and associativity of the Kronecker product and the bilinearity of the brackets we obtain 
\begin{align*}
...
\end{comment}
 Finally, considering all possible $\tau_{1},\ldots,\tau_{L}\in \{1,\ldots,N\}$, we obtain \eqref{SteadyState-coeff-implicit}. 
	...
	\begin{flushright}
		$\square$
	\end{flushright}
\section{One point correlations for the $\nu\geq 1$ case}\label{appendix-1pts-general}
