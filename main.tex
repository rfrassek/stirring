...
{\color{blue}$m$ colours denoted by $a_{k}\in\{1,\ldots,N-1\}$  with $k=1,2,\ldots, m$, chosen among the $N-1$ available species.} Then, the $m$-point correlations in the non-equilibrium steady state read\todo{this I find still confusing [WHAT ABOUT THE BLUE SENTENCE?]}
...
To each of these variables we associate coordinates $x_{\ell_{1}},\ldots,x_{\ell_{q}}\in\{x_{1},\dots,x_{m}\}$ that are such that $c_{x_{\ell_{k}}}=1$ for all $k\in \{1,\ldots,q\}$ and $c_{x}=0$ for $x\notin \{x_{\ell_{1}},\ldots,x_{\ell_{q}}\}$. Therefore, for any fixed $c_{x_{1}},\ldots,c_{x_{m}}$, we find $q$ {\color{blue} (by the summation $\textstyle{\sum_{k=1}^{m}}c_{x_{k}}=q$)} \todo{you mean the sum is q?[FIXED]} and $\ell_{1},\ldots,\ell_{q}$ from which we fix $x_{\ell_{1}},\ldots,x_{\ell_{q}}$ {\color{blue} as explained.} Then, we obtain  
	\begin{equation}
...
\label{sec5}
{\color{blue}In Section \ref{sectionIntegrabiliy} we have shown that, combining duality and integrability of the multispcies stirring process with $\nu=1$ and on the geometry of chain with two boundaries, the non-equilibrium steady state can be found in an explicit form. As soon as $\nu>1$ the integrability property is lost, but duality still hold. Therefore, in this section we first address the following question: \textit{can we use duality to characterize the non-equilibrium steady state of the boundary driven multispecies stirring process on a chain with $\nu>1$ via absorption probabilities}?  This question can be extend to the geometry of the graph $G$ introduced in Section \ref{sec1}, however, for the sake of simplicity and  for the sake of clarity, we keep the geometry of a chain with two reservoirs attached to the end sites $1$ and $L$ of the chain. \\
...
\begin{enumerate}
	...
	in Appendix~\ref{appendix-1pts-general}, the expression for the one-point correlation. \todo{is this on a chain?[ DONE]}
...
Indeed, 
\begin{enumerate}
	\item \textit{\underline{Stirring generator}}: {\color{blue}this is the edge generator $\mathcal{L}_{x,x+1}$ of the multispecies stirring process introduced in Section \ref{sec1} (with $y=x+1$). As already pointed out, in this nearest neighbour chain, its action consists in exchanging two particles or an hole and a particle on two adjacent sites. Here we show that its action on the occupation variable of any species gives a discrete gradient in the occupation variable itself. Indeed, we have that }
\todo{shorter? }
\begin{equation}
