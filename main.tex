...
The situation simplifies for a chain with two reservoirs on the boundary sites. 
In this context, the integrable structure of the process can be combined with duality to compute non-equilibrium steady state. 
...
Furthermore, reservoirs are attached at each site of the graph, creating and removing particle. When the graph is a finite chain with two reservoirs attached to the first and the last sites respectively and when $\nu=1$, the process becomes integrable (see \cite{vanicat2017exact}). {\color{blue}The choice of the boundary reservoirs introduced in \cite{vanicat2017exact} is motivated from the fact that they allow to determine an absorbing dual process, regardless of the geometry on which the process is defined and on the maximal number of particle hosted at each site}.
...
{\color{blue}where we defined $\langle\rho_{a_{1}}^{x_{1}}\rangle$.}
...
\end{equation}
\todo{these do not commute. Introduced ordered product. Please check if ok}
where
...
\end{equation} 
\todo{double check and ref to $d_x$}
we have
\begin{equation}
...
\langle N,\ldots,N,a_1,N,\ldots,N,a_m,N,\ldots,N|\Psi^{'}\rangle\dt
 \end{split}
\end{equation} 
Here the entries $a_i\neq N$ in \eqref{eq:EE} are at position $x_i$. 
\end{remark}
