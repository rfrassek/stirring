...
\begin{equation}\label{absProbabilitiesIntegrable}
	\mathcal{P}(t_{1},\ldots,t_{m},\hat{\bm{\xi}}):=\mathbb{P}\left(\bm{\xi}(\infty)=\sum_{k=1}^{m}\left(t_{k}\delta^{0}_{a_k}+(1-t_{k})\delta^{L+1}_{a_k}\right)\lvert \bm{\xi}(0)=\bm{\xi}\right)
\end{equation}
...
\begin{equation}\label{abs-probabilities}
	\mathcal{P}(t_{1},\ldots,t_{m},\bm{\xi})=\sum_{c_{1}=t_{1}}^{1}\ldots\sum_{c_{m}=t_{m}}^{1}f(L,c_{1},\ldots,c_{m})\prod_{k=1}^{m}(-1)^{c_{k}-t_{k}}g_{k}(L,x_{k},c_{k},\ldots,c_{m})
\end{equation}
...
\begin{equation}
	\begin{split}
		...
		=&\sum_{c_{1},\ldots,c_{m}=0}^{1}
		f(L,c_{1},\ldots,c_{m})\sum_{t_{1}=0}^{c_{1}}\ldots\sum_{t_{m}=0}^{c_{m}}\prod_{k=1}^{m}(-1)^{c_{k}-t_{k}}\alpha_{a_{k}}^{t_{k}}\beta_{a_{k}}^{1-t_{k}}g_{q}(L,a_{k},c_{k},\ldots,c_{m})\dt
	\end{split}
\end{equation}
By exchanging the summations we rewrite as 
\begin{equation}
	\begin{split}
		\mathbb{E}_{\mu^{\text{NESS}}}\left[\prod_{k=1}^{m}\mathbbm{1}_{\{\tau_{x_{k}}=a_{k}\}}\right]=&\sum_{t_{1},\ldots,t_{m}=0}^{1}
		\sum_{c_{1}=t_{1}}^{1}\ldots\sum_{c_{m}=t_{m}}^{1}f(L,c_{1},\ldots,c_{m})\prod_{k=1}^{m}(-1)^{c_{k}-t_{k}}g_{k}(L,x_{k},c_{k},\ldots,c_{m})\alpha_{s_{k}}^{t_{k}}\beta_{s_{k}}^{1-t_{k}}\\
		=&\sum_{t_{1},\ldots,t_{m}=0}^{1}\left(\prod_{k=1}^{m}\alpha_{a_{k}}^{t_{k}}\beta_{a_{k}}^{1-t_{k}}\right)\sum_{c_{1}=t_{1}}^{1}\ldots\sum_{c_{m}=t_{m}}^{1}f(L,c_{1},\ldots,c_{m})\left(\prod_{k=1}^{m}(-1)^{c_{k}-t_{k}}g_{k}(L,y_{k},c_{k},\ldots,c_{m})\right)\dt
	\end{split}
\end{equation}
...
\end{equation}
This formula will be useful in the following. 
\end{remark}
