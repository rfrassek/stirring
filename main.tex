...
In this section we write an explicit formula for the non-equilibrium steady state of $H$ defined in \eqref{definition-SteadyStateH}. This formula is follows from the ground state $|\Psi^{'}\rangle$found in \eqref{ABS_1}.
	...
	\begin{comment}
		...
		In the last equality we have used equation \eqref{ABS_1}. 
			...
			A formula for $|\Psi\rangle$ can be also obtain using the conservation of probability in the non-equilibrium steady state. Consider a generic $\bm{\tau}\in \Omega$, then we have 
				...
				\end{equation}
			where the expectation can be computed using \eqref{exactCorrelations}. 
				\begin{comment}
			...
			\begin{equation}
				\begin{cases}
					\tau_{x}=i_{x}\qquad &\text{if}\quad x\in\{q_{1},\ldots,w_{b}\}\\
