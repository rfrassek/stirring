...
{\color{blue}In this context, the process posses an integrable structure \cite{schutzManyBody,SSEPReviewRagoucy}. This integrability property can be combined with duality to compute absorption probabilities}. As shown in \cite{frassek2020eigenstates,frassek2020duality}, this approach leads to closed-form expressions for the correlations and the non-equilibrium steady state. 





\todo{which process do we mean here? SEP?}
...
{\color{magenta} Cristian I think it is good to keep some of the following citations, especailly the one to Schutz...
 \cite{schutz2017kardar, vanicat2017exact, crampe2016integrable, finn2018matrix, floreani2022switching, casini2022uphill, casini2023density, redig2022ergodic, redig2023equilibrium, chatterjee2023multi,  franceschini2022orthogonal,floreani2023non,carinci2023open}
...
\label{rem-rev}
		...
		\end{equation}
		reversibility holds. 
		%This is slightly more general than \eqref{reversibilityCondition}.
...
\end{equation} 
with $\tau_{x}\in \{1,\ldots,N\}$, $\forall x\in \{1,\ldots,L\}$. Since the maximal occupancy at each site is $\nu=1$,\todo{but is it at max 1? I do not understand what we call the stirring process....}  the configuration $\bm{n}$ introduced in Section~\ref{subsectionGeneratorStr} and $\bm{\tau}$ are related by 
...
{\color{blue} \begin{remark}
	In the following, with a slightly abuse of notation, we denote the configuration vectors on the extra-site $u(x)$ by $|\xi_{1}^{u(x)},\ldots,\xi_{N-1}^{u(x)}\rangle$. This allows to recall that these vectors belongs to the extra-space $N_{0}^{N-1}$ "attached" to original configuration space $\Omega$. Moreover, this notation allows to directly connect the ket-vector $|\xi_{1}^{u(x)},\ldots,\xi_{N-1}^{u(x)}\rangle$ with the row vector $(\xi_{1}^{u(x)},\ldots,\xi_{N-1}^{u(x)})$ defined in \eqref{row-dual-vectors}.
\end{remark}}
...
\end{equation} 
where $\mathcal{P}_{x_1}(1)$ is the probability that a single random walk started at $x_1$ is absorbed in $0$.\todo{site 0?} Then, using \eqref{exactCorrelations}, we have
...
\end{align}
\todo{should this all be $Y$ in the identities?}
...
of polynomials in the left and right boundary densities with coefficients given by the absorption probabilities, as claimed in Corollary~\ref{Corolollary-ABS_Corr-abstract}.
	...
	\todo{added ref... What about Section 4.4.5?}
	\end{itemize}
...
\end{equation} 
\todo{double check and ref to $d_x$ and say that R is id}
...
\end{equation}
		...
		\end{align}
		\todo{can we remove q index in sum?}
...
\end{pmatrix}\dt
\end{equation}
{\color{blue}As a further check we report here the steady state correlations and the vectors $|\Psi^{''}\rangle$, $|\Psi^{'}\rangle$ and $|\Psi\rangle$ for the SSEP, i.e. the situation where the process has only one species of particles and holes. }
...
\end{equation}
\todo{is there $c_j$ and $c_k$ missing?}
