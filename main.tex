...
At each site $x\in V$, we describe the occupation with an $N-$dimensional vector in which the value of the $k$-th component $n_{k}^{x}$ denotes the number of particles of species $k\in \{1,\ldots,N\}$. On each vertex we allow maximal occupancy $\nu\in \mathbb{N}$. Then, the configuration space is
...
\end{equation}
{\color{red}The finite-dimensional representations are labelled by partitions $\lambda=(\lambda_1,\lambda_2,\ldots,\lambda_N)$ of $\nu$ with $\lambda_i\in \mathbb{N}$ and $\sum_{i=1}^N \lambda_i = \nu$. We assume $\lambda_i\geq \lambda_{i+1}$ without loss of generality.}
...
\end{equation}
where  $ \langle m_{1},\ldots,m_{N}|$ is the row vector obtained by transposing $|m\rangle$ and $\delta_{m_{k},n_{k}}$ is the Kronhecker delta. 
...
Describe eigenvalues of $T_0(x)$ following.
\cite{belliard2}
...
\cite{Belliard2}
>>>>>>> 51c47b05b035b5a7d79d0539de1628d37ff12941
\section{Conclusion}
