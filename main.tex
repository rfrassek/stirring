...
\begin{equation}\label{SteadyState-W-Good}
	...
	\end{split}
\end{equation}
\textbf{Remark}: relation \eqref{SteadyState-W-Good} holds only for the action of $W$ on the vacuum state $|\Psi_{0}^{0}\rangle$. Acting on different equilibrium eigenstates, exchange relations between $B_{i}^{2L-1}$ and $Q_{0}^{2L}$ are more difficult and we cannot rewrite \eqref{W-charges-Bad} in such a simpler form. 
...
\begin{flushright}
	$\square$
\end{flushright}
As a consequence of \eqref{veryUsefulSeries} and of \eqref{Two-Relation}, it is easy to prove that for all  $k_{1},\ldots,k_{N-1}\in\mathbb{N}$
...
\sum_{t=0}^{\infty}\left\{2j(D_{1,1}^{2L}+D_{N,N}^{2L})-4j(j-1)\right\}^{-t}\left(\Lambda_{0}+\epsilon-Q_{0}^{2L}\right)^{-t-1}|\Psi_{0}^{0}\rangle
\end{align}
We observe that
\begin{equation}
	\lim_{\epsilon\to 0}\left(\Lambda_{0}+\epsilon-Q_{0}^{2L}\right)^{-1}\Psi_{0}^{0}=|\Psi_{0}^{0}\rangle
\end{equation}
...
\begin{align}
	...
	\\=&
	\lim_{\epsilon\to 0}\sum_{t=0}^{\infty}\left\{Q_{0}^{2L}-2j(D_{1,1}^{2L}+D_{N,N}^{2L})+4j(j-1)\right\}^{-t}\left(\Lambda_{0}+\epsilon\right)^{-t-1}\Psi_{0}^{0}
	\\=&
