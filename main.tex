...
{\color{blue}
	...
	Using \eqref{Tensor-S1} we obtain 
		...
		%	\Psi^{'}(\tau_{1},\ldots,\underset{\uparrow}{a_{1}}_{q_{1}},\ldots,\underset{\uparrow}{a_{b}}_{q_{b}},\ldots,\tau_{L})
			\Psi^{'}(N,\ldots,N,\underset{\uparrow}{b_{1}}_{x_{1}},N,\ldots,N,\underset{\uparrow}{b_{m}}_{x_{m}},N,\ldots,N)
		\end{equation}
		indicates that in positions $x_{1},\ldots,x_{m}$ there are particles of species $b_{1},\ldots,b_{m}$, while there are holes $N$ in all other positions.} %{given $\bm{\tau}$ from the bra-vector $\langle \bm{\tau}|$}☺ introduced before equation \eqref{Tensor-S1},} for all $b\in \{0,\ldots,L\}$ and for all $1\leq q_{1}<\ldots<q_{b}\leq L$ {at sites $q_{k}$ (for $k=1,\ldots,b$) we have a particle of type $a_{k}$, while at the other sites the occupation $\tau_{x}$ is left unchanged  ($x\notin\{q_{1},\ldots,q_{b}\}$). }  One can check that equation \eqref{implicit-PSI-PSIp} is indeed formula \eqref{implicit-mass-function} of Corollary~\ref{Corollary-NESS-ABS}. 
