...
\begin{equation}
	D(\bm{n},\bm{\hat{\xi}})= \frac{\prod_{x=1}^{m}n_{s_{a}}^{y_{x}}}{(\nu-m+1)}
\end{equation}
...
\begin{equation}\label{boundaryThermalized}
	\begin{split}
		...
		\binom{\nu}{r_{1},\ldots, r_{N}}\prod_{a=1}^{N-1}\left(\frac{\alpha_{k}}{\alpha_{1}}\right)^{r_{k}}\left(\frac{\alpha_{1}}{|\alpha|}\right)^{\nu}
		\end{split}
	\end{equation}
...
where 
\begin{equation}
	\rho=\rho_{a}=\frac{\alpha}{\alpha_{1}+(N-1)\alpha}\quad \forall a\in \{1,\ldots,N-1\}\qquad \text{and}\qquad \rho_{N}=\frac{\alpha_{1}}{\alpha_{1}+(N-1)\alpha}
\end{equation}
Namely,
\begin{equation}
...
\begin{equation}
	\bm{\xi}=\left(\xi_{2}^{0},\ldots,\xi_{N}^{0}\right)\otimes\left(\xi_{1}^{x},\ldots,\xi_{N}^{x}\right)^{\otimes L}\otimes \left(\xi_{2}^{L+1},\ldots,\xi_{N}^{L+1}\right)
\end{equation}
where the component $\xi_{a}^{x}$ is interpreted as the number of dual particles of type $a\in \{1,\ldots,N\}$ at site $x$, 
...
The duality function is given by
\begin{equation}
	D(\bm{n},\bm{\xi})=\prod_{a=1}^{N-1}\left(\rho_{a}^{L}\right)^{\xi_{a}^{u(x)}}\left(\prod_{x=1}^{L}\frac{(\nu -\sum_{a=2}^{N}\xi_{a}^{x})!}{\nu!}\prod_{a=2}^{N}\frac{n_{a}^{x}!}{(n_{a}^{x}-\xi_{a}^{x})!}\right)\prod_{a=1}^{N-1}\left(\rho_{a}^{R}\right)^{\xi_{a}^{u(x)}}
\end{equation}
%where we denote the \textit{density} of the species $a\in \{2,\ldots,N\}$ imposed by the reservoir at site $x\in V$  by 
%\begin{equation}
%	\rho_{a}^{x}:=\frac{\alpha_{a}^{x}}{|\alpha^{x}|}
...
\end{align}
Since the matrix $R_{x}$ is the same, we shall show that $\mathcal{H}_{x,x+1}^{c}$ and $\mathcal{H}_{x,x+1}^{m}$ have $\exp{(E^{x})}\exp{(E^{x})}$ as a symmetry. The first result will follow from the commutator between $\mathcal{H}_{x,x+1}^{c}$ and $\sum_{d=2}^{N}(E_{d1}^{x})\otimes \sum_{k=2}^{N}E_{b,1}^{x}$. Using \eqref{eq:comgl}, the bilinearity and associativity of the Kronecker product and the bilinearity of the brackets we obtain 
\begin{align*}
	&\left[\left(\sum_{d=2}^{N}E_{d1}^{x}\otimes\sum_{k=2}^{N}E_{k1}^{x+1}\right), \sum_{a,b=1}^{N}\left(E_{g_{c}(b)a}^{x}\otimes E_{g_{c}(a)b}^{x+1} -E_{aa}^{x}\otimes E_{bb}^{x+1}\right)\right]
	\\=&
	\sum_{d,k=2}^{N}\sum_{a,b=1}^{N}\left\{\left((E_{da}^{x}\delta_{g_{c}(b)1}-E_{g_{c}(b)1}^{x}\delta_{ad})-(E_{da}^{x+1}\delta_{a1}-E_{a1}^{x+1}\delta_{da})\right)\right. \\ \otimes&\left. \left((E_{kb}^{y}\delta_{g_{c}(a)1}-E_{g_{c}1}^{x+1}\delta_{kb})-(E_{kb}^{x+1}\delta_{b1}-E_{b1}^{x+1}\delta_{kb})\right)\right\}
	\\=&
	\sum_{d,k=2}^{N}\sum_{a,b=1}^{N}\left\{\left(E_{da}^{x}-E_{g_{c}(b)1}^{x}-E_{da}^{x}+E_{a1}^{x}\right)\otimes\left(E_{kb}^{x+1}-E_{g_{c}(a)1}^{x+1}-E_{kb}^{y}+E_{b1}^{x+1}\right)\right\}
	\\=&
	\sum_{d,k=2}^{N}\sum_{a,b=1}^{N}\left\{\left(E_{a1}^{x}-E_{g_{c}(b)1}^{x}\right)\otimes \left(E_{b1}^{x+1}-E_{g_{c}(a)1}^{x+1}\right)\right\}
	\\=&0
\end{align*}
where in the last equality we used the invertibility of the map $g_{c}(\cdot)$. \\
The second result will follow from the commutator between $\mathcal{H}_{x,x+1}^{m}$ and $\sum_{d=2}^{N}(E_{d1}^{x})\otimes \sum_{k=2}^{N}E_{b,1}^{x}$. The prof is similar and gives
\begin{equation}
	\left[\sum_{a=2}^{N}E_{a1}^{x}\otimes \sum_{b=2}^{N}E_{b1}^{x},\;\sum_{i=1}^{N}\sum_{a,l=1}^{N}\left(E_{la}^{x}\otimes \mathbbm{1}^{x+1}-E_{aa}^{x}\otimes \mathbbm{1}^{x+1}\right)\right]=0
\end{equation}
\begin{flushright}
	$\square$
\end{flushright}
