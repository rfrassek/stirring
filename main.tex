...
reduces to the identity which implies $e_{ab}^T= e_{ba}$. Therefore, the duality matrix reads
\begin{equation}
...
\end{equation} 
where $H^{'}= \mathcal{S}_{1} H \mathcal{S}_{1}^{-1} $ has both boundary terms in a triangular form, and $H^{''} = \mathcal{S}_{2}H' \mathcal{S}_{2}^{-1}$ has the left boundary in a triangular form and the right boundary is diagonal. Thanks to these transformations the commutators \eqref{bulk} and the action on the boundary vectors \eqref{leftBoundary}, \eqref{rightBoundary} of the operators $(X_{a})_{a\in\{1,\ldots,N\}}$ of the MPA become simpler, therefore, the steady state $|\Psi^{''}\rangle$ of $H^{''}$ will be written explicitly. Then, reversing the transformations we will retrieve $|\Psi\rangle$.

The first transformation $\mathcal{S}_{1}$ is closely related to the dual matrix. Moreover, we will see that $H^{'}$ is $\widetilde{H}$, up to the extra-site term described by the bosonic creation operator in \eqref{siteDualHamiltonian}. Therefore, $H^{'}$ is not stochastic. The second transformation $\mathcal{S}_{2}$ diagonalize the right boundary. The composition  $\mathcal{S}_{2} \mathcal{S}_{1}$ allows to simplify the commutators of the MPA algebra, paying the price of having left boundary that depends on the difference between the densities of the two reservoirs. 
...
\begin{equation}
\mathcal{S}_{1}=\prod_{x=1}^{L}d_{x}^{T}
\end{equation}
...
\end{equation}
The boundary Hamiltonians $B_{1}^{'}$ and $B_{L}^{'}$ relates to the transposed of  $\widetilde{H}_{0}$ and $\widetilde{H}_{L+1}$, the boundary part of the dual Hamiltonian  defined in \eqref{siteDualHamiltonian},
...
\begin{equation}
	B^{'}_{1}=\sum_{a=2}^{N}\left(\alpha_{a}e_{a1}^{1}-e_{aa}^{1}\right)\qquad B^{'}_{L}=\sum_{a=2}^{N}\left(\beta_{a}e_{a1}^{L}-e_{aa}^{L}\right)
\end{equation}
Obviously
