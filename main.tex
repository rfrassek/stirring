...
\begin{equation}\label{ElementsDualityMatrixChain_1}
	D(\bm{n},\bm{\xi})=\prod_{a=1}^{N-1}\alpha_{a}^{\xi_{a}^{0}}\left(\prod_{x=1}^{L}\prod_{a=1}^{N-1}\mathbbm{1}_{\{\delta_{\tau_{x},a}\geq \xi_{a}^{x}\}}\right)\prod_{a=1}^{N-1}\beta_{a}^{\xi_{a}^{L+1}}\dt
\end{equation}
\\
...
\begin{equation}
	D(\bm{n},\bm{\hat{\xi}})=\prod_{k=1}^{m}\mathbbm{1}_{\{\delta_{\tau_{y_{k}},s_{k}}=1\}}=\prod_{k=1}^{m}\mathbbm{1}_{\{\tau_{y_{k}}=s_{k}\}}\dt
\end{equation}
Denoting the $m$-point correlations by $\mathbb{E}_{\mu^{\text{NESS}}}\left[\prod_{k=1}^{m}\mathbbm{1}_{\{\tau_{y_{k}}=s_{k}\}}\right]$ we have 
...
\begin{equation}\label{CorrelationByABS-prob}
	\begin{split}
\mathbb{E}_{\mu^{\text{NESS}}}\left[\prod_{k=1}^{m}\mathbbm{1}_{\{\tau_{y_{k}}=s_{k}\}}\right]&=\lim_{t\to \infty}\mathbb{E}_{\bm{n}}\left[D(\bm{n}(t),\bm{\hat{\xi}})\right]=\lim_{t\to \infty}\mathbb{E}_{\bm{\xi}}\left[D(\bm{n},\bm{\hat{\xi}}(t))\right]
...
\begin{equation}\label{ResulsBasis}
	|\Psi^{''} \rangle=\frac{1}{\left(1+L\right)!}\prod_{x=1}^{L} \lambda_{\tau_{x}}^{(1-\delta_{\tau_{x},N})}\left(1+\sum_{j=x}^{L}\delta_{\tau_{j},N}\right)
\end{equation}
%\begin{equation}\label{ResulsBasis}
...
In the first part of the proof we will use the notation with $\bm{n}$, then we will change to the notation with $\bm{\tau}$ at the end of the proof. We recall that these two notations are related by \eqref{notation-change-relation}.  We consider the vector with elements given by the operators of the MPA $(X_{1},\ldots,X_{N})$ and we act on it with the similarity transformation $S=S_{2}S_{1}$ obtaining new operators $(\Xt_{1},\ldots,\Xt_{N})$ that will satisfy simpler commutation relations. 
...
\begin{equation}\label{ABS_1}
	\Psi^{'}(\bm{\tau})=\sum_{c_{1}=0}^{1-\delta_{\tau_{1},N}}\ldots\sum_{c_{L}=0}^{1-\delta_{\tau_{L},N}}\frac{\Gamma(2+L-\sum_{z=1}^{L}c_{z})}{\Gamma(L+2)}\prod_{x=1}^{L}\left(\lambda_{\tau_{x}}\left(2+L-x-\sum_{j=x}^{L}c_{j}\right)\right)^{c_{x}}\beta_{\tau_{x}}^{(1-c_{x}-\delta_{\tau_{x},N})}\dt
\end{equation}
...
\end{comment}
For a fixed configuration $\bm{\mu}=(\mu_{1},\ldots,\mu_{L})$, by using equation \eqref{ResulsBasis} we have
\begin{equation}
	\Psi^{''}(\bm{\mu})=\frac{1}{\left(1+L\right)!}\prod_{x=1}^{L} \lambda_{\tau_{x}}^{(1-\delta_{\tau_{x},N})}\left(1+\sum_{j=x}^{L}\delta_{\tau_{j},N}\right)
\end{equation}
\begin{comment}
\begin{equation}
...
\begin{equation}
	\begin{split}
\mathbb{E}_{\mu^{\text{NESS}}}\left[\prod_{k=1}^{m}\mathbbm{1}_{\{\tau_{y_{k}}=s_{k}\}}\right]=&\frac{1}{(1+L)!}\sum_{c_{y_{1}},\ldots,c_{y_{m}}=0}^{1}\prod_{k=1}^{m}(\alpha_{s_{k}}-\beta_{s_{k}})^{c_{y_{k}}}\beta_{s_{k}}^{1-c_{y_{k}}}\left(2+L-y_{k}-\sum_{j=k}^{m}c_{y_{j}}\right)\\&\prod_{x=1\,:\,x\notin \{y_{1},\ldots,y_{m}\}}^{L}\left(2+L-x-\sum_{k=1}^{m}\mathbbm{1}_{\{x<y_{k}\}}c_{y_{k}}\right)
\end{split}
\end{equation}
EQUATION TO CHECK...It seems to me that it is more complicated than \eqref{exactCorrelations}...\\
\underline{Explanation of the notation issue}:  Assume that we aim to find the 1-point correlation for $s$ at site $y$. If we compute it starting from \eqref{ABS_1} we have that all the $c_{x}$ with $x\neq y$ just disappear and we obtain 
\begin{equation}
	\mathbb{E}_{\mu^{NESS}}\left[\mathbbm{1}_{\{\tau_{y}=s\}}\right]=\sum_{c_{y}=0}^{1}\frac{\Gamma(2+L-c_{y})}{\Gamma(2+L)}\lambda_{s}^{c_{y}}\beta_{s}^{c_{y}}\left(2+L-y-c_{y}\right)^{c_{y}}
\end{equation}
We do not have to keep the product over all $x$ and only the "correlated coordinate" $y$ does appear. On the other hand, if we compute 1-point correlation for $s$ at site $y$ using \eqref{ABS} we obtain 
\begin{equation}
\mathbb{E}_{\mu^{NESS}}\left[\mathbbm{1}_{\{\tau_{y}=s\}}\right]=\frac{1}{(1+L)!} \sum_{c_{y}=0}^{1}\lambda_{s}^{1-c_{y}}\beta^{c_{y}}
\end{equation}
Here we have to keep the product over all $x$ and the "correlated coordinate" hidden in the product $y$.
}\\
