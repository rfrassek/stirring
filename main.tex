...
\mathbb{E}_{\mu^{\text{NESS}}}\left[\prod_{k=1}^{m}\mathbbm{1}_{\{\tau_{y_{k}}=s_{k}\}}\right]&=\lim_{t\to \infty}\mathbb{E}_{\bm{n}}\left[D(\bm{n}(t),\bm{\hat{\xi}})\right]=\lim_{t\to \infty}\mathbb{E}_{\bm{\xi}}\left[D(\bm{n},\bm{\hat{\xi}}(t))\right]
	\\&=\sum_{c_{1}=0}^{1}\ldots \sum_{c_{m}=0}^{1}\prod_{k=1}^{m}\alpha_{s_{k}}^{c_{k}}\beta_{s_{k}}^{1-c_{k}}\mathcal{P}(c_{1},\ldots,c_{m},\hat{\bm{\xi}})
\end{split}
\end{equation}
where
\begin{equation}\label{absProbabilitiesIntegrable}
	\mathcal{P}(c_{1},\ldots,c_{m},\hat{\bm{\xi}}):=\mathbb{P}\left(\bm{\xi}(\infty)=\sum_{k=1}^{m}\left(c_{k}\delta_{0}^{s_k}+(1-c_{k})\delta_{L+1}^{s_k}\right)\lvert \bm{\xi}(0)=\hat{\bm{\xi}}\right)
\end{equation}
...
\begin{equation}\label{ResulsBasis}
	|\Psi^{''} \rangle= \sum_{\tau}\frac{1}{\left(1+L\right)!}\prod_{x=1}^{L} \lambda_{\tau_{x}}^{(1-\delta_{\tau_{x},N})}\left(1+\sum_{j=x}^{L}\delta_{\tau_{j},N}\right) |\tau\rangle
\end{equation}
%\begin{equation}\label{ResulsBasis}
