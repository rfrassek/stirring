...
\end{equation}
Using \eqref{BActionVoid} and \eqref{GainCommutationQ} we have that for all $i,j\in\{1,\ldots,N-1\}$
...
\end{equation}
Indeed, consider \eqref{ExQ0B-charges} then we can write 
\begin{align}
...
\begin{align}
	&Q_{0}^{2L}B_{j}^{2L-1}Q_{0}^{2L}B_{i}^{2L-1}|\Psi_{0}^{0}\rangle=\left\{B_{j}^{2L-1}\left(Q_{0}^{2L}-2(D_{j,j}^{2L}+D_{N,N}^{2L})\right)+4B_{j}^{2L-1}\right\}\nonumber\\&B_{i}^{2L-1}\left(Q_{0}^{2L}-2(D_{1,1}^{2L}+D_{N,N}^{2L})\right)|\Psi_{0}^{0}\rangle
\end{align} 
thus we have 
\begin{align}
...
For fixed $k_{1},\ldots,k_{N-1}\in \mathbb{N}_{0}$, \eqref{Two-Relation} and \eqref{Commutativity-Gained} allow to write 
\begin{equation}\label{k-times-QB}
	...
	\end{split}
\end{equation}
Therefore, using \eqref{veryUsefulSeries}, we obtain
