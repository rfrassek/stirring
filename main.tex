...
\newcommand{\twoj}{\nu}
\newcommand{\fra}[1]{\textcolor[rgb]{1,0,0}{#1}}
\newcommand{\fraT}[1]{\textcolor[rgb]{0,0,0}{#1}}
\allowdisplaybreaks
...
\par\endgroup
 \vspace{3.5em}
 \begingroup\large \bf
 {Francesco Casini$^{(a,b)}$, Rouven Frassek$^{(a)}$, Cristian Giardin\`a$^{(a)}${\footnote{Author to whom any correspondence should be addressed: cristian.giardina@unimore.it }}}
 \par\endgroup
\vspace{2em}
...
\fraT{that does not permit  color-changing until absorption at the boundary}, 
...
We start in Section~\ref{sec1} by \fraT{defining} the open multispecies stirring
...
additionally a \fraT{particle} of any type is replaced with a
...
with $N$ conservation laws, \fraT{where $N$ is the number of species plus the holes. This means that the sum of the particle number of each species and the number of holes is conserved by the dynamics.}
...
Namely, on each site $x\in V$  a particle {or a hole} indicated by $B$ is replaced with a particle {or a hole} indicated $A$ at rate $\Gamma_{x}\alpha_{A}^{x}n_{B}^{x}$. {The transitions where \fraT{a} particle is replaced by a hole/a hole is replaced by a particle is interpreted as a removal/injection of particles.}
...
\end{equation} 
Under these assumption the Hamiltonian~\ref{hamiltonian} is integrable. This has been \fraT{shown} in \cite{vanicat2017exact} within  the Quantum Inverse Scattering Method.  We will consider this process more extensively in Section~\ref{sectionIntegrabiliy}.
...
\begin{theorem}[Absorbing duality]\label{thm-duality}
	...
	\end{equation}
where 
$\mathcal{L}_{x,y}$ \fraT{are} defined in \eqref{edgeGenerator} and, for any function $f:\widetilde{\Omega}\to \mathbb{R}$ 
...
\begin{equation}\label{ortho-extraSite}
	\langle q_{1}^{u(x)},\ldots,q^{u(x)}_{N-1}|\xi_{1}^{u(x)},\ldots,\xi_{N-1}^{u(x)}\rangle=\prod_{a=1}^{N-1}\delta_{\xi_{a}^{u(x)},\fraT{q_{a}^{u(x)}}}\,.
\end{equation} }
...
\begin{align}
&\langle n_{1}^{x},\ldots,n_{N}^{x}|(\exp{(E_{N1}^{x}+\ldots+E_{N\,N-1}^{x})})^{T}R_{x}|\xi_{1}^{x},\ldots,\xi_{N}^{x}\rangle\nonumber\\&= \langle  n_{1}^{x},\ldots,n_{N}^{x}|\left(\sum_{k_{1}=0}^{\infty}\frac{\left\{\left(E_{N1}^{x}\right)^{T}\right\}^{k_{1}}}{k_{1}!}\ldots\sum_{k_{N-1}=0}^{\infty}\frac{\left\{\left(E_{N\,N-1}^{x}\right)^{T}\right\}^{k_{N-1}}}{k_{N-1}!}\fraT{\sum_{r\in\Omega_{x}}\frac{r_{1}^{x}!\cdots r_{N}^{x}!}{\nu!}|r_{1}^{x},\ldots,r_{N}^{x}\rangle\langle r_{1}^{x},\ldots,r_{N}^{x}|}\right)\nonumber
\\&
...
\end{equation}
 ...
 \begin{flushright}
     $\square$
 \end{flushright}
 \paragraph{Proof of boundary duality \eqref{siteDualRelation}.} To prove \eqref{siteDualRelation} we transform via the Hadamard formula \footnote{\fraT{See \cite{khuan} for an application of the Hadamard formula in the multispecies asymmetric context.}} the transposed of the site Hamiltonian \eqref{siteDualHamiltonian} and then we  introduce a creation operator acting on an extra-site $u(x)$.
...
From the third commutator on we always obtain zero, i.e. $[\mathcal{B},[\mathcal{B},[\mathcal{B},\mathcal{A}]]]=0$. All in all we have that 
 \begin{align}\label{HT-BA}
 	\fraT{\exp{\left(%-\sum_{a=1}^{N-1}E_{aN}
 ...
 \end{align}
\fraT{It follows that}
\begin{align}\label{uno}
	\fraT{\exp{(-E)}\left(\sum_{a,b=1\,b\neq a}^{N-1}\alpha_{a}\left(E_{ba}-E_{bb}\right)\right)\exp{(E)}=\sum_{a,b=1\,b\neq a}^{N-1}\alpha_{a}\left(E_{ba}-E_{bb}\right)}\co
\end{align}
\begin{align}\label{due}
	\fraT{\exp{(-E)}\left(\sum_{a=1}^{N-1}\alpha_{a}\left(E_{Na}-E_{NN}\right)\right)\exp{(E)}=\sum_{a=1}^{N-1}\left(E_{Na}-E_{aa}-\sum_{b=1\,b\neq a}^{N-1}E_{ba}\right)}
\end{align}
\fraT{and}
\begin{align}\label{tre}
	\fraT{\exp{(-E)}\left(\alpha_{N}\sum_{b=1}^{N-1}\alpha_{a}\left(E_{bN}-E_{bb}\right)\right)\exp{(E)}=-\alpha_{N}\sum_{b=1}^{N-1}E_{bb}}
\end{align}
...
\end{equation}
By using \fraT{\eqref{uno},\eqref{due} and \eqref{tre})} and the fact that 
\fraT{$\sum_{A,B=1}^{N}E_{AB}$} is central for the algebra we have 

\begin{align}\label{HadTransfBoundary}
&\exp{(-E^{x})}\sum_{A=1}^{N}\sum_{B=1}^{N}\alpha_{A}^{x}\left(E_{B A}^{x}-E_{BB}^{x}\right)\exp{(E^{x})}\nonumber
\\=&
\fraT{\sum_{a=1}^{N-1}\alpha_{a}\left(E_{Na}^{x}-E_{aa}^{x}\right)-\sum_{a,b=1\, a\neq b}^{N-1}\alpha_{a}^{x}E_{bb}^{x}-\alpha_{N}\sum_{a=1}^{N-1}E_{aa}^{x}\nonumber}
\\=&
\fraT{	|\alpha^{x}|\sum_{a=1}^{N-1}\left(\rho_{a}^{x}E_{Na}^{x}-E_{aa}^{x}\right)}\dt
\end{align}
...
\right)
    \end{split}
\end{equation}}
{With a slight abuse of notation and in the spirit of Remark \ref{reamrk-extraSite}, we here denote the occupation vector at extra-site $u(x)$ by $\langle \xi_{1}^{u(x)},\ldots,\xi_{N-1}^{u(x)}|$\fraT{.}
} \\
\fraT{Recalling} the action of the bosonic creation operator acting at site $u(x)$ and on the species $a\in \{1,\ldots,N-1\}$ we have that 
...
\end{equation}
Using the above equation \fraT{and considering the addends multiplied by the Lie generator $E_{Na}^{x}$ (for all $a\in \{1,\ldots,N-1\}$) on the right hand side term of \eqref{siteHadamardII}} we have that 
...
\end{equation}
In the long-time limit the dual process voids the chain, i.e. all particles of types $\{1,\ldots,N-1\}$ are eventually absorbed at the extra-sites $0$ and $L+1$ and replaced by types $N$. We also notice that, \fraT{up to the the bosonic} creation operators $(\mathbf{a}^{+})_{a}$, the dual boundary Hamiltonians \eqref{boundary-H-dual-chain} are triangular. This simplification will be crucial in Section~\ref{sectionIntegrabiliy}.\\
...
In this section we combine the matrix product ansatz (MPA) \cite{vanicat2017exact} and  the $gl(N)$ invariance of the process in the bulk, cf.~\eqref{symmetryH}, to exactly compute the correlations in the non-equilibrium steady state
 for the integrable version of the multispecies stirring process \fraT{defined on a line segment}, see Section~\ref{integrableChain-duality}. 
...
\end{equation}
and their action on the boundary vectors  \fraT{$\langle \langle W|$ and $|V\rangle \rangle$} are
...
\end{equation}
\fraT{From \cite{vanicat2017exact} we recall that the matrices $(X_{A})_{A\in \{1,\ldots,N\}}$ act on a \textit{supplementary space} and that the boundary vectors are bra-ket vectors living in this space.}
...
\mathbb{E}\left[\prod_{k=1}^{m}\mathbbm{1}_{\{Y_{x_{k}}=a_{k}\}}\right]\\=\fraT{\sum_{s_{1},\ldots,s_{m}=0}^{1}\left(\prod_{k=1}^{m}\alpha_{a_{k}}^{s_{k}}\beta_{a_{k}}^{1-s_{k}}\right)\mathcal{P}_{x_{1},\ldots,x_{m}}(s_{1},\ldots,s_{m})}
\end{equation}
where
\begin{equation}\label{abs-probabilities}
	\mathcal{P}_{x_{1},\ldots,x_{m}}\fraT{(s_{1},\ldots,s_{m})=\sum_{c_{1}=s_{1}}^{1}\ldots\sum_{c_{m}=s_{m}}^{1}f(c_{1},\ldots,c_{m})\prod_{j=1}^{m}(-1)^{c_{j}-s_{j}}g_{j}(x_{j},c_{j},\ldots,c_{m})}
\end{equation}
with
...
\end{align}
The left one is lower triangular and depends on the differences between the boundary parameters, while the right one is diagonal. Equations \eqref{tri-diag-boundaryHamiltonians} are proved using the Hadamard formula \eqref{HadamardFormula} with $\mathcal{B}=\sum_{c=1}^{N-1}\beta_{c}e_{cN}$\fraT{, and either $\mathcal{A}=e_{aN},$ or $\mathcal{A}=e_{aa}$}. \fraT{Namely, by using \eqref{HT-BA} we obtain}
\begin{align}
	\fraT{S_{2}\left(\sum_{a=1}^{N-1}\alpha_{a}e_{aN}^{1}\right)S_{2}^{-1}=\sum_{a=1}^{N-1}\alpha_{a}e_{aN}^{1}}
\end{align}
\fraT{and}
\begin{align}
\fraT{-S_{2}\sum_{a=1}^{N-1}e_{aa}^{1}S_{2}^{-1}=-\sum_{a=1}^{N-1}\left(e_{aa}^{1}+\beta_{a}e_{aN}^{1}\right)}
\end{align}
\fraT{Therefore,  for the left boundary} 
\begin{equation}
	\begin{split}
...
\begin{equation}\label{similarity}
	S=S_{2}S_{1}=\prod_{x=1}^{L}\exp{\left(-\sum_{a=1}^{N-1}\beta_{a}e_{aN}^{x}\right)}\fraT{\exp{\left(\sum_{a=1}^{N-1}e_{Na}^{x}\right)}}\dt
\end{equation}
...
\begin{equation}\label{Xtildes2b}
 \tilde X_a=X_{a}-\beta_{a}(\fraT{X_{1}}+\ldots+X_{N})\,,\qquad \tilde X_N= 
				X_{1}+\ldots +X_{N}
\end{equation} 
...
\end{equation}  
cf.~\eqref{commLEFT2}.
Using the transformed operators \fraT{$(\widetilde{X}_{A})_{A\in\{1,\ldots,N\}}$}, the vector $|\Psi^{''}\rangle$ of the matrix product ansatz is written as
...
\end{align}
where in the second equality \fraT{we used \eqref{commLEFT} and in the third equality we used  \eqref{usefulRelaton-many}}. 
...
=&\frac{(1+\sum_{x=1}^{L}\delta_{\tau_{x},N})!}{(L+1)!}\prod_{x=1}^{L}\fraT{\left(\alpha_{\tau_{x}}-\beta_{\tau_{x}}\right)^{1-\delta_{\tau_{x},N}}}\frac{(1+\sum_{j=x+1}^{L}\delta_{\tau_{j},N})!}{(\sum_{j=x}^{L}\delta_{\tau_{j},N})!}\nonumber\\
=&\frac{1}{(L+1)!}\prod_{x=1}^{L}\fraT{\left(\alpha_{\tau_{x}}-\beta_{\tau_{x}}\right)^{1-\delta_{\tau_{x},N}}}\frac{(1+\sum_{j=x}^{L}\delta_{\tau_{j},N})!}{(\sum_{j=x}^{L}\delta_{\tau_{j},N})!}\nonumber\\
=&\frac{1}{(L+1)!}\prod_{x=1}^{L}\fraT{\left(\alpha_{\tau_{x}}-\beta_{\tau_{x}}\right)^{1-\delta_{\tau_{x},N}}}\left(1+\sum_{j=x}^{L}\delta_{\tau_{j},N}\right)\dt
\end{align}
...
\end{equation}
\fraT{whose action} on the occupation variable $|\sigma\rangle$ of a site is 
\begin{equation}
...
\begin{flushright}
    $\square$
\end{flushright}
\begin{remark} By changing the summation indices from $c_x\in \{\delta_{\tau_{x},N},1\}$ to $c_{x}^{'}\in \{0,1-\delta_{\tau_{x},N}\}$ \fraT{(implying that $c_{x}=1-c_{x}^{'}$)} and by  the fact that occupation of each site is bounded by $1$ we rewrite \eqref{ABS} as 
...
\end{equation}
\fraT{and where we exploited equation \eqref{remove-exponent}.}
...
\begin{align}
	...
	=&\fraT{\sum_{s_{1},\ldots,s_{m}=0}^{1}\left(\prod_{k=1}^{m}\alpha_{a_{k}}^{s_{k}}\beta_{a_{k}}^{1-s_{k}}\right)\sum_{c_{1}=s_{1}}^{1}\ldots\sum_{c_{m}=s_{m}}^{1}f(c_{1},\ldots,c_{m})\left(\prod_{k=1}^{m}(-1)^{c_{k}-s_{k}}g_{k}(x_{k},c_{k},\ldots,c_{m})\right)\dt}
\end{align}
...
\end{equation}
We recall that the dual processes are defined on an enlarged \fraT{chain} with \fraT{two extra-sites} $0$ and $L+1$ attached to $1$ and $L$ respectively.}
...
\fraT{	\hat{\xi}_{A}^{x}=}	\begin{cases}
		\fraT{1\qquad} &\fraT{\text{if}\quad x=x_{k}\;\text{and} \; A=a_{k}}\\
		\fraT{0\qquad} &\fraT{\text{otherwise}}
	\end{cases}
\end{equation}
...
\begin{equation}\label{ExptationSS}
	\begin{split}
		...
		\\&=
		\sum_{s_{1}=0}^{|\xi_{1}|}\ldots\fraT{\sum_{s_{N-1}=0}^{|\xi_{N-1}|}}\prod_{a=1}^{N-1}\left(\rho_{a}^{\text{left}}\right)^{s_{a}}\left(\rho_{a}^{\text{right}}\right)^{|\xi_{a}|-s_{a}}\fraT{\mathcal{P}_{x_{1},\ldots,x_{m}}(s_{1},\ldots,s_{N-1})}
	\end{split}
\end{equation}
here we recall that, generalizing Corollary~\ref{Corolollary-ABS_Corr-abstract},
\begin{equation}\label{Pass} 
\fraT{\mathcal{P}_{x_{1},\ldots,x_{m}}(s_{1},\ldots,s_{N-1})}	=\mathbb{P}\left(\bm{\xi}(\infty)=\sum_{a=1}^{N-1}\left(s_{a}\delta_{a}^{0}+(|\xi_{a}|-s_{a})\delta_{a}^{L+1}\right)\Big|  \bm{\xi}(0)=\bm{\xi}\right)
\end{equation}
are the absorption probabilities for the non-integrable chain, i.e. the probabilities that $s_{a}$ particles of species $a$ are absorbed at $0$. Here we have denoted $|\xi_{a}|=\sum_{x=1}^{L}\xi_{a}^{x}$.
 ...
 The absorption probabilities for higher point correlations fulfil  difference equations.
  \begin{remark}
	\fraT{Considering arbitrary coordinates $(x_{k})_{k\in\{1,\ldots,m\}}$ (not necessary different) and considering proper dual configurations, the technique explained above can be used to compute, in function of the absorption probabilities, the mixed factorial moments under the non-equilibrium stationary distribution.}
\end{remark}
...
\end{align}
\fraT{Here we introduce the vectors $r=(r_{1},\ldots,r_{N})$ and  $\epsilon=(\epsilon_{1},\ldots,\epsilon_{N})$ are such that $\forall A\in\{1,\ldots,N\}$, for each of their components $(r_{A})_{A\in\{1,\ldots,N\}},(\epsilon_{A})_{A\in\{1,\ldots,N\}}$ we have $r_{A}\in \{0,\ldots,\nu\}$ and $\epsilon_{A}\in \{0,\ldots,2\nu\}$, under the constraints 
\begin{equation}
...
\begin{equation}\label{bulkThermalized}
	...
	\\&\times
	\left\{f(n^{1},\ldots,n^{x-1},r,n^{x}+n^{x+1}-r,n^{x+2},\ldots,n^{L})-f(\bm{n})\right\}\fraT{\mu_{TH}({r}|{\epsilon})}
	\end{split}
\end{equation}
while
...
where $\mathcal{L}^{TH}_{x,x+1}$ is \eqref{bulkThermalized} and $\widetilde{\mathcal{L}}_{\text{left}}^{TH}$ and $\widetilde{\mathcal{L}}_{\text{right}}^{TH}$ are absorbing with rate $1$:
	\begin{align}
	\fraT{\widetilde{\mathcal{L}}_{\text{left}}^{TH}f(\bm{\xi})}&\fraT{=\left\{f\left(\xi^{0}+\sum_{a=1}^{N-1}\bm{\delta}_{a}^{0}\xi_{a}^{1},\;\xi^{1}-\sum_{a=1}^{N-1}\bm{\delta}_{a}^{1}\xi_{a}^{1}+\bm{\delta}_{N}^{1}\sum_{a=1}^{N-1}\xi_{a}^{1},\;\xi^{2},\ldots,\xi^{L}\right)-f\left(\bm{\xi}\right)\right\}}\nonumber\\ \fraT{\widetilde{\mathcal{L}}_{\text{right}}^{TH}f(\bm{\xi})}&\fraT{=\left\{f\left(\xi^{0},\ldots,\xi^{L-1},\;\xi^{L}-\sum_{a=1}^{N-1}\bm{\delta}_{a}^{L}\xi_{a}^{L}+\bm{\delta}_{N}^{L}\sum_{a=1}^{N-1}\xi_{a}^{L},\;\xi^{L+1}+\sum_{a=1}^{N-1}\bm{\delta}_{a}^{L+1}\xi_{a}^{L}\right)-f\left(\bm{\xi}\right)\right\}}\dt
\end{align}
%\begin{align}
...
\begin{equation}\label{DualGeneratorRD}
	\fraT{\widetilde{\mathcal{L}}^{rd}=\widetilde{\mathcal{L}}_{\text{left}}^{rd}+\sum_{x=1}^{L-1}\mathcal{L}_{x,x+1}^{rd}+\widetilde{\mathcal{L}}_{\text{right}}^{rd}}
\end{equation}
where 
...
\fraT{To explicitly find the result, we aim to determine $\mathcal{P}_{x_{a}}(s_{a})$.}%=\mathbb{P}_{\delta_{a}^{x}}\left(\xi_{\infty}=s_{a}\delta_{a}^{0}+(1-s_{a})\delta_{a}^{L+1}\right)$.
...
\begin{equation}\label{gamblers-ruin}
	\begin{cases}
		\fraT{\Delta_{L}\mathcal{P}_{x_{a}}(1)=0}\\
		\fraT{\mathcal{P}_{1}(1)=\frac{|\alpha|}{1+|\alpha|}+\mathcal{P}_{2}(1)\frac{1}{1+|\alpha|}}\\
		\fraT{\mathcal{P}_{L}(1)=\mathcal{P}_{L-1}(1)\frac{1}{1+|\beta|}}
	\end{cases}
\end{equation}
where $\Delta_{L}$ is the discrete Laplace operator on the chain of length $L$. The equations \eqref{gamblers-ruin} can be solved and using \fraT{$\mathcal{P}_{x_{a}}(1)=1-\mathcal{P}_{x_{a}}(0)$} we obtain 
\begin{equation}
...
\end{equation}
First, we act with $\mathcal{L}_{\text{left}}^{TH}$ on $D(\bm{n},\bm{\xi})$. By writing explicitly $\fraT{\rho_{a}^{\text{left}}}=\frac{\alpha_{a}}{|\alpha|}$, this action gives
...
\end{align}
We consider the first addend in the curly bracket of the \eqref{ProvaBordoTH}. We add and remove $\xi_{a}^{1}$ at the exponent of $\left(\frac{\alpha_{a}}{|\alpha|}\right)$ \fraT{and then we multiply} this first addend by $\left(\frac{\alpha_{N}}{|\alpha|}\right)^{-(r_{1}^{1}+\ldots+r_{N-1})+(r_{1}^{1}+\ldots+r_{N-1})}$ to get
...
We \fraT{multiply} it by $\prod_{a=1}^{N-1}\left(\frac{\alpha_{N}}{|\alpha|}\right)^{-r_{a}+r_{a}}$ to get
...
\begin{align}
	\left(\mathcal{L}_{\text{left}}^{TH}D(\cdot,\bm{\xi})\right)(\bm{n})&=\left\{\prod_{a=1}^{N-1}\left(\frac{\alpha_{k}}{|\alpha|}\right)^{\xi_{a}^{0}+\xi_{a}^{1}}\prod_{x=2}^{N}d_{x}(n^{x},\xi^{x})\prod_{d=1}^{N-1}\fraT{(\rho_{a}^{\text{right}})}^{\xi_{d}^{L+1}}-D(\bm{n},\bm{\xi})\right\}
	\\&=
...
Indeed, 
\begin{enumerate}
	\item \textit{\underline{Stirring generator}} : {using edge generator $\mathcal{L}_{x,x+1}$ of the multispecies stirring process introduced in Section~\ref{sec1} (with $y=x+1$) \fraT{and acting on the function $f(\bm{n})=n_{d}^{x}$} we have that} 
\begin{equation}
...
\item \textit{\underline{Stirring-mutation generator}} : \fraT{considering the generator $\mathcal{L}_{x,x+1}^{c}$, defined in \eqref{stirring-mutation-generatorDensity} for every $c\in \{1,\ldots,N-2\}$} \fraT{and acting on the function $f(\bm{n})=n_{d}^{x}$} we have 
\begin{equation}
	\begin{split}
...
\item \textit{\underline{Pure-mutation generator}} : \fraT{considering the generator $\mathcal{L}_{x,x+1}^{m}$, defined in equation \eqref{pure-mutation-gneratorDensity}} \fraT{and acting on the function $f(\bm{n})=n_{d}^{x}$} we have that 
\begin{equation}
...
\fraT{Acting on the function $f(\bm{n})=n_{d}^{1}$}  with $\mathcal{L}_{\text{left}}^{rd}$ we obtain
