...
%As a consequence,
%we will show that in this case it is possible to compute correlation functions 
%and the stationary state in a closed-form. 
...
more involved (for recent results on the single species case see \cite{schuetz2022reverse}).
%, see also  \cite{redig2022ergodic,floreani2022switching}.
%Asymmetric multispecies  models have been considered in \cite{schutz2017kardar, chatterjee2023multi, franceschini2022orthogonal}. 
%Integrability results have been proved both with \cite{crampe2016integrable, finn2018matrix} and without boundary driving \cite{mallick1,mallick2}. 


%Finally, the presence of different types of particles have also been modelled via a "multi-layer" dynamics (see \cite{redig2022ergodic,floreani2022switching}). As far as we know, in this last context integrability does not hold.
...
%, including instantaneous thermalization limits and reaction-diffusions. 
...
%  \cite{schutz2017kardar, crampe2016integrable, finn2018matrix, chatterjee2023multi,redig2022ergodic, floreani2022switching} and


% \cite{casini2023density,  franceschini2022orthogonal,redig2023equilibrium,floreani2023non,carinci2023open}
...
\paragraph{Acknowledgments.} We acknowledge support from Istituto Nazionale di Alta Matematica (GNFM),  the INFN grant Gauge and String Theory (GAST),  the PRIN project CUP-E53D23002220006,  the FAR UNIMORE project CUP-E93C23002040005 and  the INdAM–GNFM project CUP-E53C22001930001.  
...
with open boundaries. 
%In words, the process is described as follows. 
Each site
...
% {\color{blue} The transitions where a hole is exchanged with a particle, can be interpreted as \textit{purely diffusive}.}
...
The duality result yields {to a dual process that} %several simplifications,  the main one being that the
%dual process
...
\end{equation}}
%\begin{equation}\label{implicit-mass-function}
%	\mu(\bm{\tau})=\sum_{b=0}^{L}(-1)^{b}\sum_{1\leq q_{1}<\ldots<q_{b}\leq L}\left(\prod_{k=1}^{b}\delta_{\tau_{q_{k}},N}\right)\sum_{a_{{1}},\ldots,a_{{b}}=1}^{N-1}{\color{black}
%	\mathbb{E}\left[\Bigg( \prod_{x=1\,:\,\tau_{x}\neq N}^{L}\mathbbm{1}_{\{Y_{x}=\tau_{x}\}}\Bigg) \Bigg(\prod_{k=1}^{b}\mathbbm{1}_{\{Y_{q_{k}}=a_{{k}}\}}\Bigg)\right]}\,.
%\end{equation} 
\end{corollary}
%{\color{blue} Here $\mu(\bm{\tau})$ is the  non-equilibrium steady probability of having a configuration $\bm{\tau}$, that can be composed by particles and holes as well. However, from Corollary~\ref{Corolollary-ABS_Corr-abstract}, we only have an explicit formula for the expectations of the indicator of particle configurations (indeed $a_{1},\ldots,a_{m}$ are different from $N$). Therefore, to write the probability mass function $\mu$ for occupations containing holes we have to use the conservation of probability to get rid of the indicators of these holes. Then, equation \eqref{implicit-mass-function} is derived (for details see below the proof of Corollary~\ref{Corollary-NESS-ABS}).\\
...
\paragraph{Example for an $L=2$ chain.} To clarify equation \eqref{implicit-mass-function} we report the example of a chain with $L=2$, where we compute the probability mass functions of three configurations. Without loss of generality we write only the formulae involving particles of species $1$. We have: 
 \begin{itemize}
 	...
 	\end{equation}
 	Here we have only one term corresponding to $m=2$ in formula  \eqref{implicit-mass-function}.
	%and average can be computed by Corollary~\ref{Corolollary-ABS_Corr-abstract} with $m=2$, $x_{1}=1,\;x_{2}=2$ and $a_{1}=a_{2}=1$.
 	...
 	\end{align}
	{\color{blue}Here in the last equality we have two terms: the first corresponding to $m=1$ (with $x_{1}=1$) and the second corresponding to $m=2$ in formula \eqref{implicit-mass-function}.}
% 	Here the first term on the right-hand-side corresponds to $b=0$ in \eqref{implicit-mass-function} and can be computed by Corollary~\ref{Corolollary-ABS_Corr-abstract} with $m=1$, $x_{1}=1$ and $a_{1}=1$. The second term on the right-hand-side corresponds to $b=1$ in \eqref{implicit-mass-function} and can be computed by Corollary~\ref{Corolollary-ABS_Corr-abstract} with $m=2$, $x_{1}=1,\;x_{2}=2$ and $a_{1}=1,\;a_{2}=a$ for all $a=1,\ldots N-1$. 
 	...
 	\end{align}
 	{\color{blue}Here in the last equality we have four terms: the first term corresponds to $m=0$, the second and the third term correspond to $m=1$ (with $x_{1}=1$ and $x_{1}=2$ respectively) and the fourth term corresponds to $m=2$ in formula \eqref{implicit-mass-function}. }
	%They can be computed by Corollary~\ref{Corolollary-ABS_Corr-abstract} with $m=1$ and by fixing $x_{1}=1$, $a_{1}=1,\ldots,N-1$ and $x_{2}=2$, $a_{2}=1,\ldots,N-1$ respectively. 
	
	%and it can be computed by Corollary~\ref{Corolollary-ABS_Corr-abstract} with $m=2$, $x_{1}=1,\,x_{2}=2$ and $a_{1},a_{2}=1,\ldots,N-1$. 
 \end{itemize}
...
{\color{blue}In the right-hand-side of the equation above the first addend is the product of the indicator of $\ell$ sites, since none of the empty sites of the configuration $\bm{\tau}$ has been considered. Similarly, the second addend is the product of the indicator of $\ell+1$ sites. Indeed, in addition to the $\ell$ occupied sites of $\bm{\tau}$ one empty site is in turn chosen and its hole is filled with all possible particles. In the next addends the idea is repeated with $2,3,...L-\ell$ empty sites in turn filled with all possible particles. We notice that the exponent of the factors (-1) is given by the number of holes that have been turned into particles. Then, we introduce an index $m\in \{\ell,\ell+1,\ldots,L\}$ that counts the number of correlated sites. For each $m$ we introduce the coordinates $1\leq x_{1}<x_{2}<\ldots<x_{m}\leq L$. Then, we associate to to each of these $m$ one of the addend in \eqref{sum}. In particular, for $m=\ell$ we associate the first addend and we rewrite it as
	\begin{align}
		\prod_{x=1\,:\,\tau_{x}\neq N}^{L}\mathbbm{1}_{\{Y_{x}=\tau_{x}\}}=&\sum_{1\leq x_{1}<\ldots<x_{\ell}\leq L}\;\left(\prod_{k=1}^{\ell}(1-\delta_{\tau_{x_{k}},N})\right)\sum_{b_{1},\ldots,b_{\ell}=1}^{N-1}\left(\prod_{k=1}^{\ell}\delta_{\tau_{x_{k}},b_{k}}\right)\left(\prod_{k=1}^{\ell}\mathbbm{1}_{\{Y_{x_{k}}=b_{k}\}}\right)\nonumber
	\\	=&
	...
	\end{align}
	Similarly, the second addend in \eqref{sum} is associate to $m=\ell +1$ and it is rewritten as 
	\begin{align}
&\left(\prod_{x=1\,:\,\tau_{x}\neq N}^{L}\mathbbm{1}_{\{Y_x=\tau_{x}\}}\right)(-1)\left\{\sum_{x_{1}=1}^{L}\delta_{\tau_{x_{1}},N}\sum_{a_{1}=1}^{N-1}\mathbbm{1}_{\{Y_{x_{1}}
	=
	a_{1}\}}\right\}
			\\=&
		\sum_{1\leq x_{1}<\ldots<x_{\ell+1}\leq L}\;\left(\prod_{x\notin \{x_{1},\ldots,x_{\ell+1}\}}\delta_{\tau_{x},N}\right)\sum_{b_{1},\ldots,b_{\ell+1}=1}^{N-1}\left(\prod_{k=1}^{\ell}\delta_{\tau_{x_{k}},b_{k}}-\delta_{\tau_{x_{k},N}}\right)\left(\prod_{k=1}^{\ell+1}\mathbbm{1}_{\{Y_{x_{k}}=b_{k}\}}\right)
	\end{align}
	The idea goes on for $m=\ell+2,\ldots L$ and we obtain that 
\begin{align}
&\left(\prod_{x=1\,:\,\tau_{x}\neq N}^{L}\mathbbm{1}_{\{Y_x=\tau_{x}\}}\right)\left(\prod_{x=1\,:\,\tau_{x}=N}^{L}\left(1-\sum_{a=1}^{N-1}\mathbbm{1}_{\{Y_x=a\}}\right)\right)\nonumber\\
=&
\sum_{m=\ell}^{L}\;\sum_{1\leq x_{1}<x_{2}<\ldots<x_{m}\leq L}\left(\prod_{x\notin\{x_{1},\ldots,x_{m}\}}\delta_{\tau_{x},N}\right)\sum_{b_{1},\ldots,b_{m}=1}^{N-1}\left(\prod_{k=1}^{m}\delta_{\tau_{x_{k}},b_{k}}-\delta_{\tau_{x_{k}},N}\right)\prod_{k=1}^{m}\mathbbm{1}_{\{Y_{x_{k}}=b_{k}\}}
\end{align}
The above sum can be extended from $m=0$ since all the terms before $m=0,\ldots,\ell-1$ are vanishing. 
Finally, by taking the expectation with respect to the steady state distribution we obtain \eqref{implicit-mass-function}.}  %{Indeed, when $b>L-\ell$ we have vanishing terms in the sum \eqref{implicit-mass-function}.} }
\begin{flushright}
	$\square$
\end{flushright}
...
{\color{blue}
	\begin{align}
		...
		%=\sum_{\bm{\sigma}\in \Omega^{'}}\Psi^{'}(\bm{\sigma})\prod_{x=1}^{L}\left(\delta_{\tau_{x},\sigma_{x}}+(-1)\delta_{\tau_{x},N}\sum_{a=1}^{N-1}\delta_{\sigma_x,a}\right)\nonumber\\
	\end{align}
	%Considering 
	%\begin{equation}
	...
	Using \eqref{Tensor-S1} we obtain 
			...
			\Psi(\bm{\tau})=&\sum_{m=0}^{L}\sum_{1\leq x_{1}<x_{2}<\ldots<x_{m}\leq L}\left(\prod_{x\notin\{x_{1},\ldots,x_{m}\}}\delta_{\tau_{x_{k}},N}\right)\sum_{b_{1},\ldots,b_{m}=1}^{N-1}\left(\prod_{k=1}^{m}\delta_{\tau_{x_{k}},b_{k}}-\delta_{\tau_{x_{k}},N}\right)\nonumber\\
			\times &			
		...
		%	\Psi^{'}(\tau_{1},\ldots,\underset{\uparrow}{a_{1}}_{q_{1}},\ldots,\underset{\uparrow}{a_{b}}_{q_{b}},\ldots,\tau_{L})
			\Psi^{'}(N,\ldots,N,\underset{\uparrow}{b_{1}}_{x_{1}},N,\ldots,N,\underset{\uparrow}{b_{m}}_{x_{m}},N,\ldots,N)
		\end{equation}
		indicates that in positions $x_{1},\ldots,x_{m}$ we have particles of species $b_{1},\ldots,b_{m}$ and we have holes $N$ in all other positions.} %{given $\bm{\tau}$ from the bra-vector $\langle \bm{\tau}|$}☺ introduced before equation \eqref{Tensor-S1},} for all $b\in \{0,\ldots,L\}$ and for all $1\leq q_{1}<\ldots<q_{b}\leq L$ {at sites $q_{k}$ (for $k=1,\ldots,b$) we have a particle of type $a_{k}$, while at the other sites the occupation $\tau_{x}$ is left unchanged  ($x\notin\{q_{1},\ldots,q_{b}\}$). }  One can check that equation \eqref{implicit-PSI-PSIp} is indeed formula \eqref{implicit-mass-function} of Corollary~\ref{Corollary-NESS-ABS}. 
...
%In the rest of this section we consider a chain of length $L$, where two reservoirs are attached to sites $1$ and $L$. Each site can {\color{blue} host at most $\nu\geq 1$ particles per site, whose type is chosen among the $N-1$ available ones}. 
%The graph of the dual process is still a chain, but with two extra-sites $0$ (connected to $1$) and $L+1$ (connected to $L$). \todo{this is said before} 
%The processes considered in this section will be denoted with same {\color{blue} random variable} \todo{variables?[DONE]} $\left(\bm{n}(t)\right)_{t\geq0}$. {\color{blue} We will specify the generators case by case}. %their corresponding generator will be specified in each subsection. 
%We use the notation of Section~\ref{sectionDuality} specialized to the one dimensional chain with $\nu >1$.
%{\color{blue}The state space of the original process} is denoted by 
%\begin{equation}%\label{stateSpace-Chain}
%	\Omega_{L}=\bigotimes_{x=1}^{L} \Omega%_{x}\,.
%\end{equation}
% The dual process will always be denoted by $(\bm{\xi}(t))_{t\geq 0}$ with the dual generator specified case by case \todo{?}. The dual configuration space is the one defined in \eqref{dualStateSpace} and adapted to the chain of size $L$ and with two extra-site $0$ and $L+1$ attached to $1$ and $L$ respectively. 
