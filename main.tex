...
\end{cases}
\end{equation}
This means that on the edge $(x,y)\in \mathcal{E}$ at rate $\omega_{x,y}n_{a}^{x}n_{b}^{y}$ a particle of type $a$ at site $x$ is exchanged with a particle of type $b$ at site $y$; furthermore, on each site $x\in V$ at rate $\Gamma_{x}\alpha_{a}^{x}n_{b}^{x}$ a particle of type $b$ is replaced with a particle of type $a$. 
...
\end{equation}
Here $E_{ab}^{x}$ is a copy of $E_{ab}$ defined in \eqref{actionE} acting on site $x$. 
...
where the edge Hamiltonian is
\begin{equation}\label{edgeHamiltonian}
H_{x,y}=\sum_{a,l=1}^{N}E_{al}^{x}\otimes E_{la}^{y}-E_{ll}^{x}\otimes E_{aa}^{y}
 \end{equation}
 and where the site Hamiltonian is
 \begin{equation}\label{siteHamiltonian}
...
$(a^{\dagger})_{a}^{\widehat{x}}$ denotes a copy of $a^{\dagger}$ acting on the extra-site $\widehat{x}$ and on the species $a\in\{2,\ldots,N\}$. \\
...
\begin{align*}
&\langle n_{1}^{x},\ldots,n_{N}^{x}|(\exp{(E_{12}^{x}+\ldots+E_{1N}^{x})})^{T}R_{x}|r_{1}^{x},\ldots,r_{N}^{x}\rangle\\&= \langle  n_{1}^{x},\ldots,n_{N}^{x}|\left(\sum_{k_{2}=0}^{\infty}\frac{\left\{\left(E_{12}^{x}\right)^{T}\right\}^{k_{2}}}{k_{2}!}\ldots\sum_{k_{N}=0}^{\infty}\frac{\left\{\left(E_{1N}^{x}\right)^{T}\right\}^{k_{N}}}{k_{N}!}\sum_{s\in\Omega_{x}}\frac{s_{1}^{x}!\ldots s_{N}^{x}!}{\nu!}|s_{1}^{x},\ldots,s_{N}^{x}\rangle\langle s_{1}^{x},\ldots,s_{N}^{x}|\right)
\\&
...
\end{equation}
i.e. it is a symmetry of $H_{x,y}$. \\ Exploiting these considerations we write


\begin{comment}
As already pointed out, $H_{x,y}$ is a linear function of $\Delta(C_{2})$. This implies that symmetries of  $\Delta(C_{2})$ are also symmetries of  $H_{x,y}$. Moreover, the coproduct is a Lie algebra homomorphism, i.e. $\forall A,B\in gl(N)$
...
\begin{equation}\label{symmetryH}
    \left[S_{x,y},H_{x,y}\right]=0
\end{equation}
i.e. $S_{x,y}$ is a symmetry of $H_{x,y}$. \\ Exploiting these considerations we write
\end{comment}
\begin{equation}
    \begin{split}
...
\begin{align*}
    ...
    \\&=
    \sum_{a=2}^{N}\alpha_{a}^{x}\left(E_{1a}^{x}+E_{11}^{x}-\sum_{c=2}^{N}E_{c1}^{x}+E_{ca}^{x}-E_{11}^{x}+
    \sum_{c=2}^{N}E_{c1}^{x}\right)\\&+\alpha_{1}^{x}\sum_{b=2}^{N}\left(E_{b1}^{x}-E_{bb}^{x}-E_{b1}^{x}\right)+\sum_{a=2}^{N}\sum_{b=2}^{N}\alpha_{a}^{x}\left(E_{ba}^{x}+E_{b1}^{x}-E_{bb}^{x}-E_{b1}^{x}\right)
    \\&=\sum_{a=2}^{N}\alpha_{a}^{x}E_{1a}^{x}-\sum_{i=1}^{N}\alpha_{i}^{x}\sum_{b=2}^{N}E_{bb}^{x}=
...
d_{x}\otimes \sum_{m_{2}^{\widehat{x}},\ldots,m_{N}^{\widehat{x}}=0}^{\infty}\prod_{a=2}^{N}\left(\rho_{a}^{x}\right)^{m_{a}^{\widehat{x}}}\langle m_{2}^{\widehat{x}},\ldots,m_{N}^{\widehat{x}}|\sum_{i=1}^{N}\alpha_{i}^{x}\sum_{a=1}^{N}\left(\rho_{a}^{x}E_{1a}^{x}-E_{aa}\right)
    \end{split}
\end{equation}
Recalling the action of the bosonic creation operator acting at site $\widehat{x}$ and on the species $a\in \{2,\ldots,N\}$ we have that 
...
\begin{equation}
    \begin{split}
&\sum_{m_{2}^{\widehat{x}},\ldots,m_{N}^{\widehat{x}}=0}^{\infty}\prod_{a=2}^{N}\left(\rho_{a}^{x}\right)^{m_{a}^{\widehat{x}}}\langle m_{2}^{\widehat{x}},\ldots,m_{a}^{\widehat{x}},\ldots,m_{N}^{\widehat{x}}|\rho_{a}^{x}E_{1a}^{x}
...
\end{equation}
Under these assumption the Hamiltonian is integrable. \\
For what concerns duality, in this setting the extra-sites are denoted by $0$ and $L+1$, since connected respectively with sites $1$ and $L$ of the chain. Moreover, the matrix $R_{x}$ reduces to the identity. Therefore, the duality matrix reads as
\begin{equation}
	D=\mathcal{D}_{0}\otimes\prod_{x=1}^{L}\exp{(E^{x})}\otimes \mathcal{D}_{L+1}
\end{equation}
and the dual Hamiltonian 
\begin{equation}
	\widetilde{H}=\widetilde{H}_{0}+\sum_{x=1}^{L-1}H_{x,x+1}+\widetilde{H}_{L+1}
\end{equation}
In the long-time horizon, the dual process voids the chain, and all particles are eventually absorbed at extra-site $0$ or $L+1$.
\paragraph{Matrix Product Ansatz (MPA)} Matrix product ansatz for the non-equilibrium steady state has been formulated in \cite{vanicat2017exact}, here we recall its main points.\\
 Calling $|\Psi(t)\rangle$ a vector that contains the probability of every configuration of the chain with Hamiltonian \eqref{hamiltonian} at time $t\geq 0$, its evolution equation is given by 
 the master equation
\begin{equation}
    \frac{d|{\Psi}(t)\rangle}{dt}=H|{\Psi}(t)\rangle
\end{equation}
This Markov chain is positive recurrent, therefore there exist a unique stationary measure, that will be reached when time goes to infinity, regardless of the initial distribution. 
We denote by $|\Psi\rangle$ the column vector that contains the probabilities of every configuration of the chain with respect to this unique stationary distribution. This vector is the right eigenvector with vanishing eigenvalue of $H$, i.e. it solves 
\begin{equation}
	H|\Psi\rangle =0
\end{equation}
...
The MPA gives an abstract form of the steady state in terms of an algebra of operators. However, to write explicitly the steady state one should know a representation of this algebra. Another approach is to directly use the commutators and figure out correlations from them. This path has been followed in \cite{vanicat2017exact} to compute one and two points correlations. However, it is interesting to put effort in this direction and look for explicit form of arbitrary correlations. We will carry out this aim by the use of duality, proved in Section \ref{sectionDuality}. The idea is summarized in the following. We define a sequence of similarity transformation such that 
\begin{equation}
	H\;\xrightarrow{\mathcal{S}_{1}}\;H^{'}\;\xrightarrow{\mathcal{S}_{2}}\;H^{''}
\end{equation} 
where $\mathcal{S}_{1}$ makes $H^{'}$ with both diagonal boundaries and $\mathcal{S}_{2}$ makes $H^{''}$ with left triangular boundary and right diagonal one. Thanks to these transformations the commutators \eqref{bulk} and the action on the boundary vectors \eqref{rightBoundary}, \eqref{leftBoundary} of the operators $(X_{a})_{a\in\{1,\ldots,N\}}$ of the MPA become simple, therefore, the steady state $|\Psi\rangle$ of $H^{''}$ will be written explicitly. Then, reversing the transformations we will retrieve $|\Psi^{''}\rangle$.\\
The first transformation $\mathcal{S}_{1}$ is closely related to the dual matrix. Moreover, we will see that $H^{'}$ is $\widetilde{H}$, up to the extra-site term described by the bosonic creation operator in \eqref{siteDualHamiltonian}. Therefore, $H^{'}$ is not stochastic. This informally explains the role of duality. The second transformation $\mathcal{S}_{2}$ diagonalize the right boundary. This allows to simplify a lot the commutators of the MPA algebra, paying the price of having left boundary that depends on the difference between the densities of the two reservoirs. \\
The computation are organized as follows: we first define the similarity transformations $\mathcal{S}_{1},\mathcal{S}_{2}$ and the Hamiltonians $H^{'},H^{''}$. Second, we compute the steady state of $H^{''}$ thanks to the simpler commutators. Finally, we invert the similarities and retrieve the steady state of $H^{'}$ and $H$ respectively. 
...
\end{comment}
\subsubsection{The similarity transformation}\label{subsectionSTransf}
We introduce the first transformation as
\begin{equation}\label{transformationV}
 S_{1}:=\exp{\left(\sum_{a=2}^{N}e_{a1}\right)}=\mathbbm{1}+\sum_{a=2}^{N}e_{1a}
\end{equation}
Observe that $S_{1}$ is the transposed of the bulk part of the duality matrix $d_{x}$  at site $x\in \{1,\ldots,L\}$ (see \eqref{bulkElementDualityMatrix}). By taking the tensor product over the chain we define
\begin{equation}
    \mathcal{S}_{1}=S_{1}^{\otimes L}
\end{equation}
that is related with the duality matrix by 
\begin{equation}
\mathcal{S}_{1}=\mathbbm{1}_{0}\otimes \prod_{x=1}^{L}d_{x}^{T}\otimes \mathbbm{1}_{L+1}
\end{equation}
 This $\mathcal{S}_{1}$ is invertible, then we introduce 
\begin{equation}\label{hatHamiltonian}
   H^{'}:=B^{'}_{1}+H_{bulk}+B^{'}_{L}
\end{equation}
that is similar to $H$ via $\mathcal{S}_{1}$, i.e. 
\begin{equation}\label{similarV}
   H=\mathcal{S}_{1}^{-1}H^{'}\mathcal{S}_{1}
\end{equation}
The boundary Hamiltonians$B_{1}^{'}$ and $B_{L}^{'}$ are $\widetilde{H}_{0}$ and $\widetilde{H}_{L+1}$ without the extra-site bosonic creation operators $(a^{\dagger})_{a}^{0}$ and $(a^{\dagger})_{a}^{L+1}$, i.e.
\begin{equation}
	B^{'}_{1}=\sum_{a=2}^{N}\left(\alpha_{a}E_{1a}^{1}-E_{aa}^{1}\right)\qquad B^{'}_{1}=\sum_{a=2}^{N}\left(\beta_{a}E_{1a}^{L}-E_{aa}^{L}\right)
\end{equation}
Therefore, we can conclude that $H^{'}$ is $\widetilde{H}^{T}$ in which the extra-site parts have been removed and replaced by identities. \\
The steady state $|\Psi^{'}\rangle$ of $H^{'}$ fulfils the equations
\begin{equation}
	H^{'}|\Psi^{'}\rangle=0
\end{equation}
then, it is easy to see that 
\begin{equation}\label{S1-Inverse}
	|\Psi\rangle =\mathcal{S}_{1}^{-1}|\Psi^{'}\rangle
\end{equation}
We introduce the second transformation as 
 \begin{equation}
 	S_{2}:=\exp{\left(-\sum_{a=2}^{N}\beta_{a}e_{a1}\right)}=\mathbbm{1}-\sum_{a=2}^{N}\beta_{a}e_{a1}
 \end{equation}
By taking the tensor product over the chain we also define
\begin{equation}
	\mathcal{S}_{2}=S_{2}^{\otimes L}
\end{equation}
Using $\mathcal{S}$ we introduce 
\begin{equation}
	H^{''}=B_{1}^{''}+H_{bulk}+B_{L}^{''}
\end{equation}
that is similar to $H^{'}$ by $S_{2}$, i.e.
\begin{equation}
	H^{'}=\mathcal{S}_{2}^{-1}H^{''}\mathcal{S}_{2}
\end{equation}
The boundary Hamiltonians of $H^{''}$ reads
\begin{equation}
	B_{1}^{''}=e_{11}-\mathbbm{1}+\sum_{a=2}^{N}(\alpha_{a}-\beta_{a})e_{a1}\qquad B_{L}^{''}=e_{11}-\mathbbm{1}
\end{equation}
i.e. the left one is lower triangular and depends on the differences between boundary parameters, while the right one is diagonal. 
The steady state of $|\Psi^{''}\rangle$ of $H^{''}$ solves 
\begin{equation}
	H^{''}|\Psi^{''}\rangle=0
\end{equation}
and it is related to $|\Psi^{'}\rangle$ by 
\begin{equation}\label{S2-Inverse}
	|\Psi^{'}\rangle = \mathcal{S}_{2}^{-1}|\Psi^{''}\rangle
\end{equation}
It is convenient to introduce also the similarity between $H$ and $H^{''}$, such that
\begin{equation}
H=\mathcal{S}^{-1}H^{''}\mathcal{S}
\end{equation}
where 
\begin{equation}\label{similarity}
	S=S_{2}S_{1}=\exp{\left(-\sum_{a=2}^{N}\beta_{a}e_{a1}\right)}\exp{\left(\sum_{a=2}^{N}e_{a1}\right)}
\end{equation}
and 
\begin{equation}
	\mathcal{S}=S^{\otimes L}
\end{equation}
This transformation connect the steady states of $H$ and $H^{''}$ by 
\begin{equation}
	|\Psi\rangle=\mathcal{S}^{-1}|\Psi^{''}\rangle
\end{equation}
\begin{comment}
...
\end{comment}
\subsubsection{The steady state of $H^{''}$}\label{subsectionSSHsec}
...
\begin{align*}
	...
	\\&=
	\lambda_{a}\widetilde{X}_{a}^{n-2}\left(\lambda_{a}\widetilde{X}_{1}+\widetilde{X}_{1}\widetilde{X}_{a}\right)\widetilde{X}_{1}^{\ell-1}+\widetilde{X}_{a}^{n-2}\left(\lambda_{a}\widetilde{X}_{1}+\widetilde{X}_{1}\widetilde{X}_{a}\right)\left(\lambda_{a}\widetilde{X}_{1}+\widetilde{X}_{1}\widetilde{X}_{a}\right)\widetilde{X}_{1}^{b-1}
	\\&=\ldots\\&=
...
$\square$
\end{flushright}
\subsubsection{The steady state of $H^{'}$}\label{subsectionSSdual}
...
\begin{equation}\label{ABS}
		\begin{split}
			\Psi^{'}(\bm{s})=\sum_{x=1}^{L}\sum_{c_{x}=0}^{1-\delta_{s_{x},1}}\frac{\Gamma(2+L-\sum_{z=1}^{L}c_{z})}{\Gamma(L+2)}\prod_{y=1}^{L}\left(\lambda_{s_{y}}\left(2+L-y-\sum_{j=y}^{L}c_{y}\right)\right)^{c_{l}}\beta_{s_{y}}^{(1-c_{y})(1-\delta_{s_{y},1})}
		\end{split}
	\end{equation} 
and where
...
\end{equation}
\paragraph{Proof of formula \eqref{ABS-vect}} As a consequence of \eqref{S2-Inverse}, we only need to show how the transformation $\mathcal{S}_{2}^{-1}$ acts on the vector $|\Psi^{''}\rangle$. The matrix $S_{2}^{-1}$, acting on a single site, is given by 
\begin{equation}
   S_{2}^{-1}=\mathbbm{1}+\sum_{a=2}^{N}\beta_{a}e_{a1}
\end{equation}
...
\begin{equation}\label{notation2}
    ...
    \begin{equation}\label{InverseS2}
    	\begin{split}
    		\mathcal{S}_{2}^{-1}=\left(S_{2}^{\otimes L}\right)^{-1}&=\sum_{s_{1}=1}^{N}\ldots\sum_{s_{L}=1}^{N}\left\{e_{s_{1},\ldots,s_{L}}^{s_{1},\ldots,s_{L}}+\sum_{q_{1}=1}^{L}(1-\delta_{s_{q_{1}},1})e_{s_{1},\ldots,s_{q_{1}},\ldots,s_{L}}^{s_{1},\ldots,1,\ldots,s_{L}}\beta_{s_{q_{1}}}\right. 
    ...
    \begin{equation}
    	S_{2}^{-1}=\sum_{s_{1}=1}^{N}\left\{e_{s_{1}}^{s_{1}}+(1-\delta_{s_{1},1})\sum_{q_{1}=1}^{1}\beta_{s_{q_{1}}}e_{s_{q_{1}}}^{1}\right\}
    \end{equation}
    Assuming that \eqref{InverseS2} is true for arbitrary $L\in \mathbb{N}$, we prove that it holds for $L+1$
\begin{align*}
    	\left(S_{2}^{\otimes L}\right)^{-1}\otimes S_{2}^{-1}&=\sum_{s_{1}=1}^{N}\ldots\sum_{s_{L}=1}^{N}\left\{e_{s_{1},\ldots,s_{L}}^{s_{1},\ldots,s_{L}}++\sum_{q_{1}=1}^{L}(1-\delta_{s_{q_{1}},1})e_{s_{1},\ldots,s_{q_{1}},\ldots,s_{L}}^{s_{1},\ldots,1,\ldots,s_{L}}\beta_{s_{q_{1}}}\right. 
    	\\&+\left. \sum_{q_{1},q_{2}=1\,:\,q_{1}\neq q_{2}}^{L}(1-\delta_{s_{q_{1}},1})(1-\delta_{s_{q_{2}},1})e_{s_{1},\ldots,s_{q_{1}},\ldots,s_{q_{2}}\ldots,s_{L}}^{s_{1},\ldots,1,\ldots,1,\ldots,s_{L}}\beta_{s_{q_{1}}}\beta_{s_{q_{2}}}\right.
    	\\&+\left.
    	\ldots+	\sum_{q_{1},\ldots,q_{L-1}=1\,:\,q_{i}\neq q_{j}\,\text{if}\,i\neq j}^{L}\left(\prod_{t=1}^{L-1}(1-\delta_{s_{q_{t}},1})\right)e_{s_{1},\ldots,s_{q_{L-1}s_{L}}}^{1,\ldots,1,s_{L}}
    	\right. \\&+ \left. 
    	...
    	\\&=
    	\left(S_{2}^{\otimes L+1}\right)^{-1}
\end{align*}
    \begin{comment}
     
    \begin{equation}
        S_{2}^{-1}=\sum_{s_{1}=1}^{N}e_{s_{1}}^{s_{1}}+Q
    \end{equation}
    where
    \begin{equation}
...
\begin{equation}
	\left\{\left(S_{2}^{-1}\otimes S_{2}^{-1}\right)_{s_{1},s_{2}}^{q_{1},q_{2}}\right\}_{s_{1},s_{2}\in \{1,\ldots,N\}}^{q_{1},q_{2}\in \{1,\ldots,N\}}
\end{equation}
where the subscripts $s_{1},s_{2}$ denote the raw and the superscript $q_{1},q_{2}$ the column. Therefore, the raw identified by $s_{1},s_{2}$ reads
\begin{equation}
	\left(S_{2}^{-1}\otimes S_{2}^{-1}\right)_{s_{1},s_{2}}=e_{s_{1},s_{2}}^{s_{1},s_{2}}+(1-\delta_{s_{2},1})\beta_{s_{2}}e_{s_{1},s_{2}}^{s_{1},1}+(1-\delta_{s_{1},1})\beta_{s_{1}}e_{s_{1},s_{2}}^{1,s_{2}}+(1-\delta_{s_{1},1})(1-\delta_{s_{2},1})\beta_{s_{1}}\beta_{s_{2}}e_{s_{1},s_{2}}^{1,1}
\end{equation}
\end{comment}
The product $\mathcal{S}_{2}^{-1}|\Psi^{''}\rangle$ gives a vector $|\Psi^{'}\rangle$ whose component are denoted by $\Psi^{'}(\bm{s})$. 
To find these elements we need to fix the rows correspondent to $\bm{s}$ in the matrix $\mathcal{S}_{2}^{-1}$. Consider a string $\bm{s}=(s_{1},\ldots,s_{L})$ with $s_{1},\ldots,s_{L}\in \{1,\ldots,N\}$, then the corresponding row reads
\begin{equation}
	\begin{split}
	(\mathcal{S}_{2}^{-1})_{\bm{s}}=\left(S_{2}^{-1}\right)^{\otimes L}_{\bm{s}}&=\,e_{s_{1},\ldots,s_{L}}^{s_{1},\ldots,s_{L}}+\sum_{q_{1}=1}^{L}(1-\delta_{s_{q_{1}},1})e_{s_{1},\ldots,s_{q_{1}},\ldots,s_{L}}^{s_{1},\ldots,1,\ldots,s_{L}}\beta_{s_{q_{1}}}\\&+\sum_{q_{1},q_{2}=1\,:\,q_{1}\neq q_{2}}^{L}(1-\delta_{s_{q_{1}},1})(1-\delta_{s_{q_{2}},1})e_{s_{1},\ldots,s_{q_{1}},\ldots,s_{q_{2}}\ldots,s_{L}}^{s_{1},\ldots,1,\ldots,1,\ldots,s_{L}}\beta_{s_{q_{1}}}\beta_{s_{q_{2}}}
  \\&+
...
\begin{equation}\label{ABS_intermediate}
	\begin{split}
		\Psi^{'}(\mathbf{s})&=(\mathcal{S}_{2}^{-1})_{\bm{s}}|\Psi^{''}\rangle\\&= \Psi^{''}(\mathbf{s})+\sum_{q_{1}= 1}^{L}(1-\delta_{s_{q_{1}},1})\Psi^{''}(s_{1},\ldots,\overbrace{1}^{s_{q_{1}}},\ldots,s_{L})\beta_{s_{q_{1}}}\\&+
		\sum_{q_{1},q_{2}= 1}^{L}(1-\delta_{s_{q_{1}},1})(1-\delta_{s_{q_{2}},1})\Psi^{''}(s_{1},\ldots,\overbrace{1}^{s_{q_{1}}},\ldots,\overbrace{1}^{s_{q_{2}}},\ldots,s_{L})\beta_{a_{1}}\beta_{a_{2}}\\&+\ldots+\Psi^{''}(1,\ldots,1)\left(\prod_{x=1}^{L}(1-\delta_{s_{x},1})\right)\prod_{x=1}^{L}\left(\beta_{s_{x}}\right)^{1-\delta_{s_{x},1}}
	\end{split}
\end{equation}
...
\end{equation}
and replacing $\Psi^{'}(\bm{s})$ compute above in \eqref{ABS_intermediate} we obtain 


%Because of the exponent $(1-\delta_{s_{x},1})$, the element $\Psi^{''}(\bm{s})$ has, up the term $\frac{\Gamma(2+\sum_{i=1}^{L}\delta_{s_{x},1})}{\Gamma(L+2)}$, a contribution of a factor $\lambda_{s_{x}}\left(2+L-x-\sum_{j=x}^{L}(1-\delta_{s_{x},1})\right)$ if the site is non empty, and a contribution of a factor "$1$" if the site is empty, it is convenient to introduce the $c_{i}$ of defined in \eqref{CiEquation}. Then, equation  \eqref{ABS_intermediate} reads 
...
Finally, considering all possible occupancy $\bm{s}$, we obtain \eqref{ABS-vect}.
\begin{flushright}
    $\square$
\end{flushright}
...
\begin{equation}\label{steadyStateH-coeff}
    \begin{split}
        \Psi(\bm{s})&=\sum_{x=1}^{L}\sum_{c_{x}=0}^{1-\delta_{s_{x},1}}\frac{\Gamma(2+L-\sum_{z=1}^{L}c_{z})}{\Gamma(L+2)}\prod_{y=1}^{L}\left(\lambda_{s_{y}}\left(2+L-y-\sum_{j=y}^{L}c_{j}\right)\right)^{c_{y}}\beta_{s_{y}}^{(1-c_{y})(1-\delta_{s_{y},1})}
        \\&+
...
\\&
\sum_{x=1\,:\, x\neq q_{1},\ldots,q_{b}}^{L}\sum_{c_{x}=0}^{1-\delta_{s_{x},1}}\sum_{p=1}^{b}\sum_{c_{q_{p}}=0}^{1}\frac{\Gamma(2+L-\sum_{z=1}^{L}c_{z})}{\Gamma(2+L)}
...
\paragraph{Proof of formula \eqref{steadyStateH}} As a consequence of \eqref{S1-Inverse}, we only need to show how the transformation $\mathcal{S}_{1}^{-1}$ acts on the vector $|\Psi^{'}\rangle$. The matrix $S_{2}^{-1}$, acting on a single site, is given by 
\begin{equation}\label{S1-L1}
	S_{1}^{-1}=\sum_{s_{1}=1}^{N}e_{s_{1}}^{s_{1}}+(-1)\delta_{s_{1},1}\sum_{d_{1}=2}^{N}e_{1}^{d_{1}}
\end{equation}
Using the notation introduced in \eqref{notation1} and \eqref{notation2} we have that 
\begin{equation}
	\begin{split}
		\mathcal{S}_{1}^{-1}=\left(S_{1}^{\otimes L}\right)^{-1}&=\sum_{s_{1}=1}^{N}\ldots\sum_{s_{L}=1}^{N}\left\{e_{s_{1},\ldots,s_{L}}^{s_{1},\ldots,s_{L}}\right.
		\\&
		+\left.\sum_{b=1}^{L}(-1)^{b}\sum_{q_{1}=1}^{L}\sum_{q_{2}=q_{1}+1}^{L}\ldots\sum_{q_{b}=q_{b-1}+1}^{L}\left(\prod_{r=1}^{b}\delta_{s_{q_{r}},1}\right)\sum_{r=1}^{b}\sum_{d_{q_{r}}=2}^{N}e_{\ldots,s_{x},\ldots,\underbrace{1}_{q_{r}},\ldots}^{\ldots,s_{x},\ldots,\,d_{q_{r}}\,,\ldots}\right\} 
	\end{split}
\end{equation}
...
where $e_{\ldots,s_{x},\ldots,\underbrace{1}_{q_{r}},\ldots}^{\ldots,s_{x},\ldots,\,d_{q_{r}}\,,\ldots}$ denotes the matrix $e_{s_{1},\ldots,s_{L}}^{s_{1},\ldots,s_{L}}$ where the subscripts $s_{q_{1}},\ldots,s_{q_{b}}$ have been replaced by "1" and the superscripts $s_{q_{1}},\ldots,s_{q_{b}}$ have been replaced by $d_{q_{1}},\ldots,d_{q_{b}}$. \\
\end{comment}
The product $\mathcal{S}_{1}^{-1}|\Psi^{'}\rangle$ gives a vector whose component are denoted by $\Psi(\bm{s})$. 
...
\begin{equation}
    \begin{split}
        \mathcal{S}_{1}^{-1}{\bm{s}}|\Psi{'}\rangle&=\Psi{'}(s_{1},\ldots,s_{L})+ 
        \\&
        +\sum_{b=1}^{L}(-1)^{b}\sum_{q_{1}=1}^{L}\sum_{q_{2}=q_{1}+1}^{L}\ldots\sum_{q_{b}=q_{b-1}+1}^{L}\left(\prod_{r=1}^{b}\delta_{s_{q_{r}},1}\right)\sum_{r=1}^{b}\sum_{d_{q_{r}}=2}^{N}\Psi{'}(\ldots,s_{x},\ldots,d_{q_{r}},\ldots)
    \end{split}
\end{equation}
...
\begin{equation}
    \begin{split}
  &\Psi{'}(\ldots,s_{x},\ldots,d_{q_{r}},\ldots)=\sum_{x=1\,:\, x\neq q_{1},\ldots,q_{l}}^{L}\sum_{c_{x}=0}^{1-\delta_{s_{x},1}}\sum_{p=1}^{b}\sum_{c_{q_{p}}=0}^{1}\frac{\Gamma(2+L-\sum_{z=1}^{L}c_{z})}{\Gamma(2+L)}
\\&  
...
In this section we write a formula for the stationary steady state correlations between $m$- points of the chain, each of them occupied by a non empty species, i.e. a species with label from $2$ to $N$. This is motivated by interest of understanding how actual particles of the chain are correlated among each others and because the holes depends on the occupation of all other species, as a consequence of the constraint $n_{0}^{x}=\nu-\sum_{a=2}^{N}n_{a}^{x}$ for arbitrary $x\in \{1,\ldots,L\}$. Consider $m\in \{1,\ldots,L\}$ points with coordinates given by $\bm{y}=(y_{1},\ldots,y_{m})$, such that $y_{i}<y_{i+1}$ $\forall i\in \{1,\ldots,m-1\}$. In each of these points there is a particle of species $s_{x}\in \{2,\ldots,N\}$. Then, with respect to the non-equilibrium stationary distribution, the correlations among these particles are given by
...
\begin{equation}
\langle n^{y_{1}}_{s_{1}}\cdots n^{y_{m}}_{s_{m}}\rangle_{\text{stat}}=\sum_{c_{1},\ldots,c_{m}=0}^{1}
f(c_{1},\ldots,c_{m})f(c_{1},\ldots,c_{m})\sum_{t_{1}=0}^{c_{1}}\sum_{t_{m}=1}^{c_{m}}\prod_{q=1}^{m}(-1)^{c_{q}-t_{q}}\alpha_{s_{q}}^{t_{q}}\beta_{s_{q}}^{1-t_{q}}g_{q}(y_{q},c_{q},\ldots,c_{m})
\end{equation}
%\begin{equation}
...
\begin{equation}\label{Prob-m}
	\begin{split}
		\langle n_{s_{1}}^{y_{1}}\cdots n_{s_{m}}^{y_{m}}\rangle_{\text{stat}}&
...
This corresponds to the steady state probability of a chain of length $m$ with each coordinates $y_{i}$ occupied by $s_{x}\neq 1$, i.e. 
\begin{equation}
	\mathbb{P}_{stat}\left(n_{s_{1}}^{y_{1}}=1,\ldots,n_{s_{m}}^{y_{m}}=1\right)=\Psi\left(s_{1},\ldots,s_{m}\right)
\end{equation}
where $\Psi(\bm{s})$ is the element of the vector \eqref{steadyStateH} computed for this chain $m$.
As already remarked in \eqref{LinkABS-corr}, for non-empty configurations we have:
\begin{equation}\label{Corr-mABS}
	\Psi(s_{1},\ldots,s_{m})=\Psi^{'}(s_{1},\ldots,s_{m})
\end{equation}
Thus, by applying \eqref{ABS_intermediate} we have 
\begin{equation}
	\Psi(s_{1},\ldots,s_{m})=\sum_{i=1}^{L}\sum_{c_{i}=0}^{1}\frac{\Gamma(2+L-\sum_{a=1}^{m}c_{k})}{\Gamma(2+L)}\prod_{k=1}^{m}\lambda_{s_{k}}^{c_{k}}\beta_{s_{k}}^{1-c_{k}}\left(2+L-y_{k}-\sum_{j=k}^{m}c_{j}\right)^{c_{k}}
\end{equation}
Introducing $f(c_{1},\ldots,c_{m})$ and $g_{a}(y_{a},c_{a},\ldots,c_{m})$ we have the formulas for arbitrary correlations.
\begin{flushright}
    $\square$
\end{flushright}
...
\begin{equation}
	...
	\end{split}
\end{equation}
Therefore, we obtain the second cumulant, i.e. the second truncated correlation as 
\begin{equation}\label{two-pts-corr}
	\begin{split}
		\mathbb{E}[n_{y_{1}}^{s_{1}}n_{y_{2}}^{s_{2}}]-\mathbb{E}[n_{y_{1}}^{s_{1}}]\mathbb{E}[n_{y_{2}}^{s_{2}}]=-\frac{y_{1}(L-y_{2}+1)}{L(L+1)^{2}}\left(\alpha_{s_{1}}-\beta_{s_{1}}\right)\left(\alpha_{s_{2}}-\beta_{s_{2}}\right)
	\end{split}
\end{equation}
...
\begin{align}\label{three-pts-corr}
        \langle n_{s_{1}}^{y_{1}}n_{s_{2}}^{y_{2}}n_{s_{3}}^{y_{3}}\rangle_{\text{stat}}&=\beta_{s_{1}}\beta_{s_{2}}\beta_{s_{3}}+\frac{(L-y_{1})}{(L+1)}(\alpha_{s_{1}}-\beta_{s_{1}})\beta_{s_{2}}\beta_{s_{3}}
        \\&+\nonumber
        \frac{(L-y_{2})}{(L+1)}(\alpha_{s_{2}}-\beta_{s_{2}})\beta_{s_{1}}\beta_{s_{3}}+\frac{(L-y_{3})}{(L+1)}(\alpha_{s_{3}}-\beta_{s_{3}})\beta_{s_{1}}\beta_{s_{2}}
        \\&+\nonumber
...
\end{equation} 
for $ x\in\{1,\ldots,L\}\quad a\in\{2,3\}$.
It is endowed with boundary conditions $\rho_{a}^{0},\rho_{\overline{a}}^{0}$ on the left and $\rho_{a}^{L+1},\rho_{\overline{a}}^{L+1}$ and where $\overline{a}=3$ if $a=2$ and $\overline{a}=2$ if $a=3$. \\The generator of this model consists in a sum of three Markov generators: a stirring, a stirring-mutation and a pure mutation one (see section 4 of \cite{casini2022uphill}). For the lattice $\mathbb{Z}$, self-duality has been showed and the duality function is exactly \eqref{elementsABS} in case of hard-core exclusion (see section 5 of \cite{casini2022uphill}). In view of what have been proved in the previous section of this work, some questions arise naturally:
\begin{enumerate}
...
\end{enumerate}
The aim of this section is to give an answer to these questions. We first define a more general version with arbitrary number of species and arbitrary occupation on a general finite graph, checking that in the case of a chain we obtain average occupation evolution equations with the same structure as \eqref{2SpeciesRD}. Second we construct the Lie algebraic description and we use it to prove absorbing duality in the general case. 
\subsection{The process on a finite graph}
Considering again a connected graph $G=(V,\mathcal{E})$ with set of vertices $V$ and set of edges $\mathcal{E}$, we introduce the generator
...
\begin{equation}
	\sum_{a=1}^{N}\rho_{a}^{x}=\nu
\end{equation}
\textbf{Remark}: observe that the site generator $\mathcal{L}_{x}^{rd}$ is exactly the one of the multi-species stirring process \eqref{siteGenerator}. We wrote it with the uppercase 'rd' in order to underline the fact that we need a specific choice of the reservoirs parameters $\alpha_{a}^{x}$, as specified above. 
...
\begin{equation}
	...
	\end{split}
\end{equation}
where we call $q$ such that $g_{i}^{-1}(d)=q$. Let us observe that, by the definition of the map $g_{c}(\cdot)$ and for the fact that $1\leq c\leq N-2$, $q\neq d$ and $q\neq 1$ .
Moreover, we have
\begin{equation}
...
\end{equation}
where $\mathbb{E}_{\mu}$ denotes the expectation with respect to the law of the Markov process $(\bm{n})_{t\geq 0}$ initialized with arbitrary initial measure $\mu$. They evolve according to the Master's equation
\begin{equation}
	\frac{d \rho_{a}(x,t)}{dt}=\mathbb{E}_{\mu}\left[\mathcal{L}^{rd}n_{a}^{x}(t)\right]
\end{equation} 
Therefore, using the action of $\mathcal{L}^{rd}$ expressed by \eqref{DifferenceEquation} and \eqref{actionBDLine} we obtaina system of $N-1$ difference-differential equation where the discrete Laplacians of all species are coupled and linear reaction terms are present. For these reasons, this model is a generalization to $N$ species and to maximal occupation $\nu$ of the one studied in \cite{casini2022uphill}. This answers the first question of the beginning of this section. 
...
\begin{equation}
	\Lambda^{x}_{rev}\sim \text{Multinomial}\left(\twoj,\rho_{1},\ldots,\rho_{N}\right)\quad \text{where}\quad \rho_{a}=\frac{1}{N}
\end{equation}
namely
\begin{equation}
	\Lambda_{rev}^{x}(n^{x})=\frac{\nu!}{\prod_{a=1}^{N}n_{a}^{x}!}\left(\frac{1}{N}\right)^{\sum_{a=1}^{N}n_{a}^{x}}
\end{equation}
...
\end{equation}
that is true if and only if $p_{a}=p$ $\forall a\in \{2,\ldots,N\}$.\\
By similar computation it is possible to show that the same result is obtained imposing the detailled balance for the transition $\bm{n}\to \bm{n}-\delta_{a}^{x}+\delta_{b}^{x}$. \\
Recalling that, to have reversibility, the reservoirs parameters must fulfil $\rho_{a}^{x}=\rho_{a}$ $\forall x \in V$ and $\forall a\in \{1,\ldots,N\}$, we obtain the result. 
\subsection{Duality}
...
The dual process is defined by its generator, which reads
\begin{equation}\label{DualGenerator}
	\widetilde{\mathcal{L}}=\sum_{(x,y)\in \mathcal{E}}\omega_{x,y}\mathcal{L}_{x,y}^{rd}+\sum_{x\in V}\Gamma_{x}\widetilde{\mathcal{L}}_{x}^{rd}
\end{equation}
where 
...
\begin{equation}
	H^{rd}=\sum_{x,y\in \mathcal{E}}\omega_{x,y}\left(\sigma_{11}H_{x,y}+\sigma_{12}\sum_{c=1}^{N-2}H_{x,y}^{c}+(\Upsilon-2\sigma_{12})H_{x,y}^{m}\right)+\sum_{x\in V}\Gamma_{x}H_{x}
\end{equation}
where $H_{x,y}$ and $H_{x}$ are \eqref{edgeHamiltonian} and \eqref{siteHamiltonian} respectively, 
\begin{equation}
	H_{x,y}^{c}=\sum_{a,b=0}^{N}\left(E_{g_{c}(a)b}^{c}\otimes E_{g_{c}(b)a}^{y}-E_{aa}^{x}\otimes E_{bb}^{y}\right)
\end{equation}
and 
\begin{equation}
	H_{x,y}^{m}=\sum_{a,b=1}^{N}\left(E_{ba}^{x}\otimes \mathbbm{1}^{y}-E_{aa}^{x}\otimes \mathbbm{1}^{y}\right)
\end{equation}
Similarly, we define the dual Hamiltonian $\widetilde{H}^{rd}$. For the edge dynamics, the original and the dual Hamiltonians do coincide, while the site one it is again $\widetilde{H}_{x}$ defined in \eqref{siteDualHamiltonian}. \\ We aim to show
\begin{equation}\label{DualityRelationRD}
	(H^{rd})^{T}D=D\widetilde{H}^{rd}
\end{equation}
Therefore, we need to prove that
\begin{align}
	(H_{x,y}^{rd})^{T}D&=DH_{x,y}^{rd}\label{edgeDualityRelationRD}\\
	H_{x}^{T}D&=D\widetilde{H}_{x}D\label{siteDualityRelationRD}
\end{align}
where the duality matrix $D$ is again \eqref{dualityMatrix}. As already pointed out, $H_{x}$ and $\widetilde{H}_{x}$ have the same structure (up to a specific choice of the reservoirs parameters), while $H_{x,y}^{rd}$ has two more terms $\sum_{c=1}^{N-2}H_{x,y}^{c}$ and $H_{x,y}^{m}$. Therefore, we only need to show that 
\begin{align}
	&(H_{x,y}^{c})^{T}D=DH_{x,y}^{c}\qquad \forall c\in \{1,\ldots,N-2\}\label{cDualityRelation}\\
	&(H_{x,y}^{m})^{T}D=DH_{x,y}^{m}\label{mDualityRelation}
\end{align}
Since the matrix $R_{x}$ is the same, we shall show that $H_{x,y}^{c}$ and $H_{x,y}^{m}$ are symmetries of $\exp{E^{x}}\exp{E^{y}}$. The first result will follow from the commutator between $H_{x,y}^{c}$ and $\sum_{d=2}^{N}(E_{d1}^{x})\otimes \sum_{k=2}^{N}E_{b,1}^{x}$. Using \eqref{eq:comgl}, the bilinearity and associativity of the Kronecker product and the bilinearity of the brackets we obtain 
\begin{align*}
...
The second result will follow from the commutator between $H_{x,y}^{m}$ and $\sum_{d=2}^{N}(E_{d1}^{x})\otimes \sum_{k=2}^{N}E_{b,1}^{x}$. The prof is similar and gives
\begin{equation}
...
where the edge Hamiltonian is
\begin{equation}
H_{x,y}=\sum_{a,l=1}^{N}\left(e_{al}\otimes e_{la}-e_{ll}\otimes e_{aa}\right)
 \end{equation}
 and where the site Hamiltonian is
 \begin{equation}
