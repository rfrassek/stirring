...
are the absorption probabilities for the non-integrable chain, i.e. the probabilities that $s_{a}$ particles of species $a$ are absorbed at $0$. Here we have denoted $|\xi_{a}|=\sum_{x=1}^{L}\xi_{a}^{x}$.
 ...
 The absorption probabilities for higher point correlations fulfil  difference equations.
  \begin{remark}
	\fra{Considering arbitrary coordinates $(x_{k})_{k\in\{1,\ldots,m\}}$ (not necessary different) and considering proper dual configurations, the technique explained above can be used to compute, in function of the absorption probabilities, the $m$-mixed factorial moments under the non-equilibrium stationary distribution.}
\end{remark}
